...
Actor::Impl::Impl() : plugin(nullptr), songcompare{SONG_INFO_TYPE_NULL} {
  // nothing
}
...
VisVideoDepth Actor::Impl::get_supported_depths() {
  ...
  return actor_plugin ? actor_plugin->vidoptions.depth
                      : VISUAL_VIDEO_DEPTH_NONE;
}

bool Actor::available(std::string const &name) {
  return LV::PluginRegistry::instance()->has_plugin(VISUAL_PLUGIN_TYPE_ACTOR,
                                                    name);
}
...
Actor::Actor(std::string const &name) : m_impl{new Impl}, m_ref_count{1} {
  if (!available(name)) {
...
VisPluginData *Actor::get_plugin() { return m_impl->plugin; }

bool Actor::realize() { return visual_plugin_realize(m_impl->plugin); }

VideoPtr const &Actor::get_video() { return m_impl->video; }
...
bool Actor::video_negotiate(VisVideoDepth run_depth, bool noevent,
                            bool forced) {
  ...
  m_impl->get_actor_plugin()->requisition(m_impl->plugin, &run_width,
                                          &run_height);

  // Check to make sure requested run depth is supported. If not, pick the
  // highest.

  auto supported_depths = get_supported_depths();
  m_impl->run_depth =
      forced ? run_depth
             : visual_video_depth_get_highest_nogl(supported_depths);
  ...
  if (output_depth != VISUAL_VIDEO_DEPTH_GL) {
    ...
    visual_log(VISUAL_LOG_DEBUG,
               "Conversions skipped in OpenGL rendering mode");
  }
...
void Actor::set_video(VideoPtr const &video) { m_impl->video = video; }
...
void Actor::run(Audio const &audio) {
  ...
  if (!visual_songinfo_compare(&m_impl->songcompare, actor_plugin->songinfo) ||
      m_impl->songcompare.get_elapsed() !=
          actor_plugin->songinfo->get_elapsed()) {
  ...
  if (video->get_depth() != VISUAL_VIDEO_DEPTH_GL) {
    ...
    if (to_convert) {
      ...
      // Render first
      actor_plugin->render(m_impl->plugin, to_convert.get(),
                           const_cast<Audio *>(&audio));
      ...
      if (to_scale) {
        ...
        // Render, then scale
        actor_plugin->render(m_impl->plugin, to_scale.get(),
                             const_cast<Audio *>(&audio));
        video->scale(to_scale, VISUAL_VIDEO_SCALE_NEAREST);
      } else {
        // Setup any palette
        ...
        actor_plugin->render(m_impl->plugin, video.get(),
                             const_cast<Audio *>(&audio));
      }
    }
  } else {
