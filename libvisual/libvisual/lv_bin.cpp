...
public:
  ...
  VisVideoDepth depthforced; /* Contains forced depth value, for the actmorph so
                                we've got smooth transformations */
  VisVideoDepth
      depthforcedmain; /* Contains forced depth value, for the main actor */
...
Bin::Impl::Impl()
    : use_morph(false), morphing(false), morphtime(4, 0),
      depthpreferred(VISUAL_BIN_DEPTH_HIGHEST),
      depthflag(VISUAL_VIDEO_DEPTH_NONE), depthold(VISUAL_VIDEO_DEPTH_NONE),
      depth(VISUAL_VIDEO_DEPTH_NONE), depthchanged(false), depthfromGL(false),
...
void Bin::Impl::set_actor(ActorPtr const &new_actor) { actor = new_actor; }

void Bin::Impl::set_input(InputPtr const &new_input) { input = new_input; }

Bin::Bin() : m_impl(new Impl) {
  // empty
}
...
ActorPtr const &Bin::get_actor() const { return m_impl->actor; }

InputPtr const &Bin::get_input() const { return m_impl->input; }
...
void Bin::set_morph(std::string const &morph_name) {
  ...
  if (visual_video_depth_is_supported(depthflag,
                                      m_impl->actvideo->get_depth()) <= 0) {
    m_impl->morph.reset();
    return;
  }
}

MorphPtr const &Bin::get_morph() const { return m_impl->morph; }
...
bool Bin::connect(std::string const &actor_name,
...
void Bin::sync(bool noevent) {
  ...
  if (m_impl->morphing && m_impl->use_morph &&
    ...
    visual_log(VISUAL_LOG_DEBUG, "phase1 m_impl->privvid %p",
               (void *)m_impl->privvid.get());
  ...
  visual_log(
      VISUAL_LOG_DEBUG,
      "one last video pitch check %d depth old %d forcedmain %d noevent %d",
      video->get_pitch(), m_impl->depthold, m_impl->depthforcedmain, noevent);
...
void Bin::set_video(VideoPtr const &video) { m_impl->actvideo = video; }

void Bin::set_supported_depth(VisVideoDepth depthflag) {
  m_impl->depthflag = depthflag;
}

VisVideoDepth Bin::get_supported_depth() const { return m_impl->depthflag; }
...
VisVideoDepth Bin::get_depth() const { return m_impl->depth; }
...
void Bin::switch_actor(std::string const &actor_name) {
  visual_log(VISUAL_LOG_DEBUG, "switching to a new actor: %s, old actor: %s",
             actor_name.c_str(),
             visual_plugin_get_info(m_impl->actor->get_plugin())->plugname);
  ...
  if (visual_video_depth_is_supported(depthflag, VISUAL_VIDEO_DEPTH_GL)) {
    ...
    visual_log(VISUAL_LOG_INFO, "video depth (from fixate): %d",
               video->get_depth());

    /* FIXME check if there are any unneeded depth transform environments and
     * drop these */
    visual_log(VISUAL_LOG_DEBUG,
               "checking if we need to drop something: depthforcedmain: %d "
               "actvideo->depth %d",
    ...
    if (m_impl->depthforcedmain != m_impl->actvideo->get_depth()) {
      ...
      visual_log(VISUAL_LOG_DEBUG,
                 "[[[[optionally a bogus transform environment, dropping]]]]");
    }
    ...
    if (m_impl->actvideo->get_depth() > video->get_depth() &&
      ...
      visual_log(VISUAL_LOG_INFO,
                 "old depth is higher, video depth %d, bin depth %d",
      ...
      visual_log(VISUAL_LOG_INFO,
                 "new depth is higher, or equal: video depth %d, depth %d bin "
                 "depth %d",
                 video->get_depth(), depth, m_impl->depth);

      visual_log(VISUAL_LOG_DEBUG,
                 "depths i can locate: actvideo: %d   bin: %d     bin-old: %d",
                 m_impl->actvideo->get_depth(), m_impl->depth,
                 m_impl->depthold);

      m_impl->depthforced = video->get_depth();
      m_impl->depthforcedmain = m_impl->depth;

      visual_log(VISUAL_LOG_DEBUG, "depthforcedmain in switch by name: %d",
                 m_impl->depthforcedmain);
...
void Bin::switch_actor(ActorPtr const &actor) {
  ...
  /* Starting the morph, but first check if we don't have anything todo with
   * openGL */
  if (m_impl->use_morph &&
    ...
    visual_log(VISUAL_LOG_DEBUG, "Pointer actvideo->pixels %p",
               m_impl->actvideo->get_pixels());
    if (m_impl->actor->get_video()->get_depth() != VISUAL_VIDEO_DEPTH_GL &&
        m_impl->actvideo->get_pixels()) {
      visual_mem_set(m_impl->actvideo->get_pixels(), 0,
                     m_impl->actvideo->get_size());
    }
  }

  visual_log(VISUAL_LOG_DEBUG,
             "Leaving, actor->video->depth: %d actmorph->video->depth: %d",
             m_impl->actor->get_video()->get_depth(),
             m_impl->actmorph->get_video()->get_depth());
...
void Bin::switch_finalize() {
  ...
  visual_log(VISUAL_LOG_DEBUG,
             " - in finalize - fscking depth from actvideo: %d %d",
             m_impl->actvideo->get_depth(), m_impl->actvideo->get_bpp());
  ...
  m_impl->depthforcedmain = m_impl->actvideo->get_depth();
  visual_log(VISUAL_LOG_DEBUG, "m_impl->depthforcedmain in finalize %d",
             m_impl->depthforcedmain);
...
void Bin::use_morph(bool use) { m_impl->use_morph = use; }

void Bin::switch_set_time(Time const &time) { m_impl->morphtime = time; }
...
void Bin::run() {
  ...
  if (m_impl->morphing) {
    if (m_impl->use_morph &&
      ...
      m_impl->morph->realize();
      m_impl->morph->run(audio, m_impl->actor->get_video(),
