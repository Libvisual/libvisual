...
Palette::Palette(unsigned int ncolors) : colors(ncolors) {
  // empty
}
...
void Palette::allocate_colors(unsigned int ncolors) { colors.resize(ncolors); }
...
void Palette::blend(Palette const &src1, Palette const &src2, float rate) {
  ...
  for (unsigned int i = 0; i < colors.size(); i++) {
    colors[i].r =
        src1.colors[i].r + ((src2.colors[i].r - src1.colors[i].r) * rate);
    colors[i].g =
        src1.colors[i].g + ((src2.colors[i].g - src1.colors[i].g) * rate);
    colors[i].b =
        src1.colors[i].b + ((src2.colors[i].b - src1.colors[i].b) * rate);
  }
}
