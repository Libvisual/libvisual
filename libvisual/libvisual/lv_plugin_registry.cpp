...
namespace {
typedef std::unordered_map<PluginType, PluginList, std::hash<int>>
    PluginListMap;
...
PluginRef *load_plugin_ref(fs::path const &plugin_path) {
  ...
  if (!module) {
    visual_log(VISUAL_LOG_ERROR, "Cannot load plugin (%s)",
               plugin_path_string.c_str());
    return nullptr;
  }

  auto plugin_version =
      static_cast<int *>(module->get_symbol(VISUAL_PLUGIN_VERSION_TAG));
  ...
  auto get_plugin_info = reinterpret_cast<PluginGetInfoFunc>(
      module->get_symbol("get_plugin_info"));

  if (!get_plugin_info) {
    visual_log(VISUAL_LOG_ERROR,
               "Cannot get function that returns plugin info");
    return nullptr;
  }
...
PluginRegistry::PluginRegistry() : m_impl(new Impl) {
...
PluginRef const *PluginRegistry::find_plugin(PluginType type,
...
bool PluginRegistry::has_plugin(PluginType type,
                                std::string const &name) const {
...
VisPluginInfo const *
PluginRegistry::get_plugin_info(PluginType type,
                                std::string const &name) const {
...
PluginList
PluginRegistry::Impl::get_plugins_from_dir(fs::path const &dir) const {
  PluginList list;
  list.reserve(30);
