...
LV_API void visual_param_value_init(VisParamValue *self, VisParamType type,
                                    void *value);
LV_API void visual_param_value_copy(VisParamValue *value, VisParamValue *src);
LV_API void visual_param_value_set(VisParamValue *value, VisParamType type,
                                   void *new_value);
...
#define _LV_DEFINE_PARAM_VALUE_SET(func, ctype, name, marshal)                 \
  static inline void visual_param_value_set_##func(VisParamValue *self,        \
                                                   ctype value) {              \
    visual_return_if_fail(self != NULL);                                       \
    visual_param_value_set(self, VISUAL_PARAM_TYPE_##name,                     \
                           _LV_PARAM_MARSHAL_##marshal(value));                \
  }
...
#define _LV_DEFINE_PARAM_VALUE_GET(func, ctype, name, member, defvalue)        \
  static inline ctype visual_param_value_get_##func(VisParamValue *self) {     \
    ...
    visual_return_val_if_fail(self->type == VISUAL_PARAM_TYPE_##name,          \
                              defvalue);                                       \
