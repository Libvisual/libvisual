...
#if defined(VISUAL_OS_LINUX) || defined(VISUAL_OS_SOLARIS) ||                  \
    defined(VISUAL_OS_AIX)

  int ncpus = sysconf(_SC_NPROCESSORS_ONLN);

  return ncpus != -1 ? ncpus : 1;

#elif defined(VISUAL_OS_NETBSD) || defined(VISUAL_OS_FREEBSD) ||               \
    defined(VISUAL_OS_OPENBSD) || defined(VISUAL_OS_DARWIN)
...
static void print_cpu_info(void) {
  ...
  visual_log(VISUAL_LOG_DEBUG, "CPU: ARM LDREX_STREX %d",
             cpu_caps.hasLDREX_STREX);
#endif /* VISUAL_ARCH_X86 || VISUAL_ARCH_X86_64 */
}

__attribute__((no_sanitize("address"))) void visual_cpu_initialize() {
