...
public:
  ...
  Param(std::string const &name, std::string const &description,
        VisParamType type, void *default_value, Closure *validator);
  ...
  Closure *add_callback(VisParamChangedFunc func, void *priv,
                        VisDestroyFunc destroy_func);
...
Closure::Closure(void *func_, void *data_, VisDestroyFunc destroy_func_)
    : func{reinterpret_cast<void (*)()>(func_)}, data{data_},
...
ParamList::Impl::Impl() : event_queue{nullptr} {
  // nothing
}
...
ParamList::ParamList() : m_impl{new Impl} {
  // nothing
}
...
ParamList::ParamList(std::initializer_list<Param *> params) : m_impl{new Impl} {
  for (auto param : params) {
    add(param);
  }
}

ParamList::ParamList(ParamList &&list) : m_impl{std::move(list.m_impl)} {
  // nothing
}
...
EventQueue *ParamList::get_event_queue() const { return m_impl->event_queue; }

void ParamList::add(Param &&param) { add(&param); }
...
Param::Param(std::string const &name_, std::string const &description_,
             VisParamType type_, void *default_value_, Closure *validator_)
    : parent(nullptr), name(name_), description(description_),
...
Closure *Param::add_callback(VisParamChangedFunc func, void *priv,
                             VisDestroyFunc destroy_func) {
  visual_return_val_if_fail(func != nullptr, nullptr);

  auto closure =
      new Closure{reinterpret_cast<void *>(func), priv, destroy_func};
...
bool Param::remove_callback(Closure *to_remove) {
  ...
  auto entry = std::find_if(changed_handlers.begin(), changed_handlers.end(),
...
VisClosure *visual_closure_new(void *func, void *data,
                               VisDestroyFunc destroy_func) {
  return new LV::Closure{func, data, destroy_func};
}

void visual_closure_free(VisClosure *self) { delete self; }

// VisParamList implementation

VisParamList *visual_param_list_new(void) { return new LV::ParamList; }

void visual_param_list_free(VisParamList *self) { delete self; }

void visual_param_list_set_event_queue(VisParamList *self,
                                       VisEventQueue *event_queue) {
...
void visual_param_list_add_array(VisParamList *self, VisParam **params,
...
VisParam *visual_param_new(const char *name, const char *description,
...
void visual_param_free(VisParam *self) { delete self; }
