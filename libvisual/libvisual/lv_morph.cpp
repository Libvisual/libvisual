...
Morph::Impl::Impl() : plugin{nullptr}, progress{0}, morphpal{nullptr} {
  // nothing
}
...
VisPluginData *Morph::get_plugin() { return m_impl->plugin; }

bool Morph::available(std::string const &name) {
  return LV::PluginRegistry::instance()->has_plugin(VISUAL_PLUGIN_TYPE_MORPH,
                                                    name);
}
...
Morph::Morph(std::string const &name) : m_impl(new Impl), m_ref_count(1) {
  if (!available(name)) {
...
bool Morph::realize() { return visual_plugin_realize(m_impl->plugin); }
...
void Morph::set_video(VideoPtr const &video) { m_impl->dest = video; }

void Morph::set_time(Time const &time) { m_impl->morphtime = time; }
...
bool Morph::run(Audio const &audio, VideoPtr const &src1,
                VideoPtr const &src2) {
  ...
  if (morph_plugin->palette) {
    morph_plugin->palette(m_impl->plugin, m_impl->progress,
                          const_cast<Audio *>(&audio), m_impl->morphpal,
                          src1.get(), src2.get());
  } else {
  ...
  morph_plugin->apply(m_impl->plugin, m_impl->progress,
                      const_cast<Audio *>(&audio), m_impl->dest.get(),
                      src1.get(), src2.get());
