...
public:
  ...
  void upload_to_channel(std::string const &name, BufferConstPtr const &samples,
...
void sample_buffer_mix(BufferPtr const &dest, BufferPtr const &src,
                       float multiplier) {
...
void Audio::Impl::upload_to_channel(std::string const &name,
...
AudioChannel::AudioChannel(std::string const &name_) : name(name_) {}

AudioChannel::~AudioChannel() {
  // empty
}

void AudioChannel::add_samples(BufferConstPtr const &samples,
                               Time const &timestamp) {
...
Audio::Audio(Audio &&rhs) : m_impl{std::move(rhs.m_impl)} {
  // empty
}
...
bool Audio::get_sample(BufferPtr const &buffer,
...
void Audio::get_sample_mixed_simple(BufferPtr const &buffer,
...
void Audio::get_sample_mixed(BufferPtr const &buffer, bool divide,
                             unsigned int channels, ...) {
  va_list args;
...
void Audio::get_spectrum(BufferPtr const &buffer, std::size_t samplelen,
...
void Audio::get_spectrum_for_sample(BufferPtr const &buffer,
                                    BufferConstPtr const &sample,
                                    bool normalised) {
...
void Audio::input(BufferPtr const &buffer, VisAudioSampleRateType rate,
  ...
  auto sample_count =
      buffer->get_size() / visual_audio_sample_format_get_size(format);

  auto converted_buffer = Buffer::create(sample_count * sizeof(float));

  AudioConvert::convert_samples(
      converted_buffer, VISUAL_AUDIO_SAMPLE_FORMAT_FLOAT, buffer, format);
  ...
  case VISUAL_AUDIO_SAMPLE_CHANNEL_STEREO: {
    ...
    AudioConvert::deinterleave_stereo_samples(
        samples1, samples2, converted_buffer, VISUAL_AUDIO_SAMPLE_FORMAT_FLOAT);
