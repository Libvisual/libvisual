...
/* NOTE: The depth find helper code in lv_actor depends on an arrangment from
 * low to high */
/**
...
typedef enum {
  ...
  VISUAL_VIDEO_DEPTH_ENDLIST =
      32, /**< Used to mark the end of the depth list. */
  VISUAL_VIDEO_DEPTH_ALL = VISUAL_VIDEO_DEPTH_8BIT | VISUAL_VIDEO_DEPTH_16BIT |
                           VISUAL_VIDEO_DEPTH_24BIT | VISUAL_VIDEO_DEPTH_32BIT |
                           VISUAL_VIDEO_DEPTH_GL /**< All graphical depths. */
} VisVideoDepth;

/**
 * Enumerate that defines video rotate types, used with the
 * visual_video_rotate_*() functions.
 */
typedef enum {
...
/**
 * Enumerate that defines the video mirror types, used with the
 * visual_video_mirror_*() functions.
 */
typedef enum {
...
typedef enum {
  ...
  VISUAL_VIDEO_COMPOSE_TYPE_SURFACE,  /**< One alpha channel for the complete
                                         surface. */
  VISUAL_VIDEO_COMPOSE_TYPE_SURFACECOLORKEY, /**< Use surface alpha on colorkey.
                                              */
  VISUAL_VIDEO_COMPOSE_TYPE_CUSTOM /**< Custom compose function (looks up on the
                                      source VisVideo. */
} VisVideoComposeType;
...
public:
  ...
  static VideoPtr wrap(void *buffer, bool owner, int width, int height,
                       VisVideoDepth depth, int pitch = 0);

  static VideoPtr create_sub(VideoConstPtr const &src, Rect const &srect);

  static VideoPtr create_sub(Rect const &drect, VideoConstPtr const &src,
                             Rect const &srect);
   ...
   *
   * @note Use this only when the desired pitch is not equal to width * bytes
   * per pixel.
   *
  ...
  /**
   * Checks if this Video has the same attributes as another Video, ignoring
   * pitch comparisons.
   *
   * @param src Video to compare against
   *
  ...
  VisVideoComposeFunc get_compose_function(VideoConstPtr const &src,
                                           bool alpha);
   ...
   * @param alpha set to true if alpha channel is to be checked
   */
  void blit(Rect const &drect, VideoConstPtr const &src, Rect const &srect,
            bool alpha);

  void compose(VideoConstPtr const &src, int x, int y,
               VisVideoComposeFunc func);
  void compose(Rect const &drect, VideoConstPtr const &src, Rect const &srect,
               VisVideoComposeFunc func);

  void blit_scale(Rect const &drect, VideoConstPtr const &src,
                  Rect const &srect, bool alpha,
                  VisVideoScaleMethod scale_method);
  void compose_scale(Rect const &drect, VideoConstPtr const &src,
  ...
  static VideoPtr create_scale_depth(VideoConstPtr const &src, int width,
...
inline void intrusive_ptr_add_ref(Video const *video) { video->m_ref_count++; }
...
LV_API VisVideo *visual_video_new_with_buffer(int width, int height,
                                              VisVideoDepth depth);
LV_API VisVideo *visual_video_new_wrap_buffer(void *buffer, int owner,
...
LV_API int visual_video_compare_attrs_ignore_pitch(VisVideo *src1,
                                                   VisVideo *src2);
...
LV_API VisVideo *visual_video_new_sub_by_values(VisVideo *src, int x, int y,
                                                int width, int height);
LV_API VisVideo *visual_video_new_sub_with_boundary(VisRectangle *drect,
                                                    VisVideo *src,
                                                    VisRectangle *srect);
LV_API VisVideo *visual_video_new_sub_all(VisVideo *src);

LV_API void visual_video_set_compose_type(VisVideo *video,
                                          VisVideoComposeType type);
...
LV_API void visual_video_set_compose_function(VisVideo *video,
                                              VisVideoComposeFunc compfunc);

LV_API VisVideoComposeFunc visual_video_get_compose_function(VisVideo *dest,
                                                             VisVideo *src,
                                                             int alpha);

LV_API void visual_video_blit_area(VisVideo *dest, VisRectangle *drect,
                                   VisVideo *src, VisRectangle *srect,
                                   int alpha);
LV_API void visual_video_compose_area(VisVideo *dest, VisRectangle *drect,
                                      VisVideo *src, VisRectangle *srect,
                                      VisVideoComposeFunc func);
LV_API void visual_video_blit_scale_area(VisVideo *dest, VisRectangle *drect,
                                         VisVideo *src, VisRectangle *srect,
                                         int alpha,
                                         VisVideoScaleMethod scale_method);
LV_API void visual_video_compose_scale_area(VisVideo *dest, VisRectangle *drect,
                                            VisVideo *src, VisRectangle *srect,
                                            VisVideoScaleMethod scale_method,
                                            VisVideoComposeFunc compfunc);

LV_API void visual_video_blit(VisVideo *dest, VisVideo *src, int x, int y,
                              int alpha);
LV_API void visual_video_compose(VisVideo *dest, VisVideo *src, int x, int y,
                                 VisVideoComposeFunc compfunc);

LV_API void visual_video_fill_alpha(VisVideo *video, uint8_t density);
LV_API void visual_video_fill_alpha_area(VisVideo *video, uint8_t density,
                                         VisRectangle *rect);
LV_API void visual_video_fill_color(VisVideo *video, VisColor *color);
LV_API void visual_video_fill_color_area(VisVideo *video, VisColor *color,
                                         VisRectangle *rect);
...
LV_API void visual_video_rotate(VisVideo *dest, VisVideo *src,
                                VisVideoRotateDegrees degrees);
LV_API void visual_video_mirror(VisVideo *dest, VisVideo *src,
                                VisVideoMirrorOrient orient);
LV_API void visual_video_scale(VisVideo *dest, VisVideo *src,
                               VisVideoScaleMethod scale_method);

LV_API void visual_video_scale_depth(VisVideo *dest, VisVideo *src,
                                     VisVideoScaleMethod scale_method);

LV_API VisVideo *visual_video_scale_depth_new(VisVideo *src, int width,
                                              int height, VisVideoDepth depth,
                                              VisVideoScaleMethod scale_method);
 ...
 */
LV_API VisVideoDepth visual_video_depth_get_prev(int depthflag,
                                                 VisVideoDepth depth);
 ...
 */
LV_API VisVideoDepth visual_video_depth_get_next(int depthflag,
                                                 VisVideoDepth depth);
 ...
 *
 * @return VisVideoDepth enum value, or VISUAL_VIDEO_DEPTH_NONE if depth is
 * unsupported
 */
