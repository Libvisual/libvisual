...
public:
   ...
   *
   * @param[out] buffer  buffer to hold the retrieved samples (32-bit floating
   * point PCM)
   ...
   * @note Output samples will be truncated to fit the user-supplied buffer.
   *
   * @param[out] buffer buffer to hold the mixed samples (32-bit floating point
   * PCM)
  ...
  /**
   * Returns samples downmixed by weighted summing or averaging a set of
   * channels.
   *
   * @note Output samples will be truncated to fit the user-supplied buffer.
   *
   ...
   * @param channels    number of channels
   * @param ...         list of channel names (each of type const char *),
   * followed by a list of respective weights (each of type double)
   */
  void get_sample_mixed(BufferPtr const &buffer, bool divide,
   ...
   *
   * @note The output spectrum will be truncated to fit the user-supplied
   * buffer.
   *
   ...
   */
  void get_spectrum(BufferPtr const &buffer, std::size_t sample_count,
                    std::string const &channel_name, bool normalised);
   ...
   * @param normalised  normalise ampltitudes to [0.0, 1.0]
   */
  static void get_spectrum_for_sample(BufferPtr const &buffer,
                                      BufferConstPtr const &samples,
                                      bool normalised);

  static void get_spectrum_for_sample(BufferPtr const &buffer,
                                      BufferConstPtr const &samples,
                                      bool normalised, float multiplier);
   ...
   */
  void input(BufferPtr const &buffer, VisAudioSampleRateType rate,
   ...
   */
  void input(BufferPtr const &buffer, VisAudioSampleRateType rate,
             VisAudioSampleFormatType format, std::string const &channel_name);
...
LV_API int visual_audio_get_sample(VisAudio *audio, VisBuffer *buffer,
                                   const char *channelid);
LV_API void visual_audio_get_sample_mixed_simple(VisAudio *audio,
                                                 VisBuffer *buffer,
                                                 unsigned int channels, ...);
LV_API void visual_audio_get_sample_mixed(VisAudio *audio, VisBuffer *buffer,
                                          int divide, unsigned int channels,
                                          ...);

LV_API void visual_audio_get_spectrum(VisAudio *audio, VisBuffer *buffer,
                                      int samplelen, const char *channelid,
                                      int normalised);
LV_API void
visual_audio_get_spectrum_multiplied(VisAudio *audio, VisBuffer *buffer,
                                     int samplelen, const char *channelid,
                                     int normalised, float multiplier);
LV_API void visual_audio_get_spectrum_for_sample(VisBuffer *buffer,
                                                 VisBuffer *sample,
                                                 int normalised);
LV_API void visual_audio_get_spectrum_for_sample_multiplied(VisBuffer *buffer,
                                                            VisBuffer *sample,
                                                            int normalised,
                                                            float multiplier);

LV_API void visual_audio_input(VisAudio *audio, VisBuffer *buffer,
                               VisAudioSampleRateType rate,
                               VisAudioSampleFormatType format,
                               VisAudioSampleChannelType channeltype);

LV_API void visual_audio_input_channel(VisAudio *audio, VisBuffer *buffer,
                                       VisAudioSampleRateType rate,
                                       VisAudioSampleFormatType format,
                                       const char *channelid);
