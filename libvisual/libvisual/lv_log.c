...
void visual_log_set_verbosity(VisLogSeverity level) { verbosity = level; }

VisLogSeverity visual_log_get_verbosity() { return verbosity; }

void visual_log_set_handler(VisLogSeverity severity, VisLogHandlerFunc func,
                            void *priv) {
  LogHandler *handler;
...
void _lv_log(VisLogSeverity severity, const char *file, int line,
...
static void output_to_stderr(VisLogSeverity severity, const char *msg,
                             VisLogSource const *source) {
  fprintf(stderr, "%s %s:%d:%s: %s\n", log_prefixes[severity], source->file,
          source->line, source->func, msg);
  fflush(stderr);
}
