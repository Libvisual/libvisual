...
public:
   ...
   * @param height Height of rectangle
   */
  Rect(int width_, int height_) : x(0), y(0), width(width_), height(height_) {}
   ...
   */
  void denormalize_points(float const *fxlist, float const *fylist,
   ...
   * @param y  Integer variable to store the absolute Y coordinate
   */
  void denormalize_point_neg(float fx, float fy, int32_t &x_,
                             int32_t &y_) const {
   ...
   * @param size   number of points
   */
  void denormalize_points_neg(float const *fxlist, float const *fylist,
                              int32_t *xlist, int32_t *ylist,
                              unsigned int size) const;
};
...
LV_API void visual_rectangle_set(VisRectangle *rect, int x, int y, int width,
                                 int height);
...
LV_API int visual_rectangle_contains_rect(VisRectangle *dest,
                                          VisRectangle *src);
LV_API void visual_rectangle_clip(VisRectangle *dest, VisRectangle *within,
                                  VisRectangle *src);

LV_API void visual_rectangle_normalize(VisRectangle *rect);
LV_API void visual_rectangle_normalize_to(VisRectangle *dest,
                                          VisRectangle *src);

LV_API void visual_rectangle_denormalize_point(VisRectangle *rect, float fx,
                                               float fy, int32_t *x,
                                               int32_t *y);
LV_API void visual_rectangle_denormalize_points(VisRectangle *rect,
                                                const float *fxlist,
                                                const float *fylist,
                                                int32_t *xlist, int32_t *ylist,
                                                unsigned int size);

LV_API void visual_rectangle_denormalize_point_neg(VisRectangle *rect, float fx,
                                                   float fy, int32_t *x,
                                                   int32_t *y);
LV_API void
visual_rectangle_denormalize_points_neg(VisRectangle *rect, const float *fxlist,
                                        const float *fylist, int32_t *xlist,
                                        int32_t *ylist, unsigned int size);
