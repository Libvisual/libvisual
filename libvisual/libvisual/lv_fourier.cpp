...
enum DFTMethod { DFT_METHOD_BRUTE_FORCE, DFT_METHOD_FFT };
...
DFTCache::Entry const &DFTCache::get_entry(DFTMethod method,
                                           unsigned int sample_count) {
...
DFT::DFT(DFT &&rhs) : m_impl{std::move(rhs.m_impl)} {
  // nothing
}
...
void DFT::perform(float *output, float const *input) {
  ...
  visual_math_simd_complex_scaled_norm(
      output, m_impl->real.data(), m_impl->imag.data(),
...
void DFT::log_scale_standard(float *output, float const *input,
                             unsigned int size) {
  visual_return_if_fail(output != nullptr);
  visual_return_if_fail(input != nullptr);
...
void DFT::log_scale_custom(float *output, float const *input, unsigned int size,
...
DFT::Impl::Impl(unsigned int samples_out_, unsigned int samples_in_)
    : sample_count(samples_in_), spectrum_size(sample_count / 2 + 1),
      samples_out(std::min(samples_out_, spectrum_size)),
      method(best_method(sample_count)), real(sample_count),
      imag(sample_count) {
  // empty
}
