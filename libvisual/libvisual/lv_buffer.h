...
public:
   ...
   *
   * @note Use this to wrap and optionally manage memory blocks allocated by C
   * functions.
   *
  ...
  /**
   * Returns the pointer to the managed memory block, starting at a given
   * offset.
   *
...
LV_API void *visual_buffer_get_data_offset(VisBuffer *buffer,
                                           visual_size_t offset);
...
LV_API void visual_buffer_copy_to_data(VisBuffer *src, void *dest,
                                       visual_size_t size);

LV_API void visual_buffer_put(VisBuffer *dest, VisBuffer *src,
                              visual_size_t offset);
LV_API void visual_buffer_put_data(VisBuffer *dest, const void *data,
                                   visual_size_t size, visual_size_t offset);

LV_API void visual_buffer_fill(VisBuffer *buffer, uint8_t value);
LV_API void visual_buffer_fill_with_pattern(VisBuffer *buffer, const void *data,
                                            visual_size_t size);
