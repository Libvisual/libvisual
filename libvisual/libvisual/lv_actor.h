...
public:
  ...
  /**
   * Configures actor to automatically work with the video target set with
   * set_video().
   *
   * @note This method setups any necessary scaling and depth conversion
   * necessary.
   *
   * @see set_video()
   *
   * @param run_depth Desired colour depth used for rendering. Use
   * VISUAL_VIDEO_DEPTH_NONE for auto selection
...
inline void intrusive_ptr_add_ref(Actor const *actor) { actor->m_ref_count++; }
 ...
 *
 * The requisition() method is called to query and configure the preferred
 ...
 */
typedef void (*VisPluginActorRequisitionFunc)(VisPluginData *plugin, int *width,
                                              int *height);

/**
 * Function signature and type of the Actor palette() method.
 *
 * The palette() method is called to obtain the colour palette used for 8-bit
 * rendering modes.
 *
 * @param plugin Plugin object
 *
 ...
 * @param audio  Audio data to visualise
 */
typedef void (*VisPluginActorRenderFunc)(VisPluginData *plugin, VisVideo *video,
                                         VisAudio *audio);
...
struct _VisActorPlugin {
  VisPluginActorRequisitionFunc
      requisition; /**< Returns the preferred rendering dimensions */
  VisPluginActorPaletteFunc
      palette; /**< Returns the colour palette of the actor */
...
LV_API int visual_actor_video_negotiate(VisActor *actor,
                                        VisVideoDepth run_depth, int noevent,
                                        int forced);
