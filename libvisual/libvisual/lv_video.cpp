...
Video::Impl::Impl()
    : width(0), height(0), depth(VISUAL_VIDEO_DEPTH_NONE), bpp(0), pitch(0),
      buffer(Buffer::create()), parent(),
...
Video::Video() : m_impl(new Impl), m_ref_count(1) {
  // empty
}

VideoPtr Video::create() { return VideoPtr(new Video, false); }
...
VideoPtr Video::create_sub(VideoConstPtr const &src, Rect const &area) {
  ...
  if (!vrect.contains(area)) {
    visual_log(VISUAL_LOG_DEBUG,
               ...
               "source area (%d, %d, %d, %d)",
               area.x, area.y, area.width, area.height, vrect.x, vrect.y,
  ...
  self->set_attrs(area.width, area.height, src->m_impl->pitch,
                  src->m_impl->depth);
...
VideoPtr Video::create_sub(Rect const &drect, VideoConstPtr const &src,
                           Rect const &srect) {
...
VideoPtr Video::wrap(void *data, bool owner, int width, int height,
                     VisVideoDepth depth, int pitch) {
  // FIXME: Support this
  if (owner) {
    visual_log(VISUAL_LOG_WARNING, "Memory leak: Ownership of wrapped pixel "
                                   "buffer is not supported at the moment");
  }
...
VideoPtr Video::create_scale_depth(VideoConstPtr const &src, int width,
                                   int height, VisVideoDepth depth,
...
bool Video::allocate_buffer() {
  ...
  if (get_pixels()) {
    if (m_impl->buffer->is_allocated()) {
      ...
      visual_log(
          VISUAL_LOG_ERROR,
          "Not allocating buffer for Video with an external screen buffer");
      return false;
    }
  }
...
bool Video::validate() const {
  ...
  if (m_impl->width <= 0) {
    visual_log(VISUAL_LOG_ERROR, "Video has a zero or negative width (%d).",
               m_impl->width);
    return false;
  }

  if (m_impl->height <= 0) {
    visual_log(VISUAL_LOG_ERROR, "Video has a zero or negative height (%d).",
               m_impl->height);
    return false;
  }

  if (m_impl->pitch <= 0) {
    visual_log(VISUAL_LOG_ERROR, "Video has a zero or negative pitch (%d).",
               m_impl->pitch);
    return false;
  }
  ...
  if (m_impl->width != m_impl->extents.width) {
    visual_log(VISUAL_LOG_ERROR,
               "Width (%d) and width of extents (%d) do not agree.",
  ...
  if (m_impl->height != m_impl->extents.height) {
    visual_log(VISUAL_LOG_ERROR,
               "Height (%d) and height of extents (%d) do not agree.",
  ...
  if (m_impl->parent) {
    ...
    if (!parent_impl->extents.contains(m_impl->extents)) {
      visual_log(VISUAL_LOG_ERROR,
                 "Sub-video is not fully contained by parent.");
      return false;
    }
  }
  ...
  if (m_impl->depth != VISUAL_VIDEO_DEPTH_GL) {
    ...
    if (m_impl->bpp != visual_video_depth_bpp(m_impl->depth) / 8) {
      visual_log(VISUAL_LOG_ERROR,
                 "Depth (%s) is not consistent with BPP (%d).",
    ...
    for (int y = 0; y < m_impl->height; y++) {
      auto const pixel_row_ptr =
          static_cast<uint8_t const *>(get_pixels()) + y * m_impl->pitch;

      if (m_impl->pixel_rows[y] != pixel_row_ptr) {
        visual_log(VISUAL_LOG_ERROR,
                   "Pixel row pointer table is wrong at y=%d.", y);
        return false;
      }
    }
  }
...
bool Video::has_same_content(VideoConstPtr const &video) const {
  ...
  if (m_impl->depth == VISUAL_VIDEO_DEPTH_GL ||
      video->m_impl->depth == VISUAL_VIDEO_DEPTH_GL) {
    return false;
  }
  ...
  // Determine the size (in bytes) of the blocks in each row to compare. This
  // must exclude padding. It must also exclude pixels outside of a subvideo's
  // extents.
  std::size_t const content_bytes_per_row = m_impl->width * m_impl->bpp;

  for (int y = 0; y < m_impl->height; y++) {
    if (std::memcmp(m_impl->pixel_rows[y], video->m_impl->pixel_rows[y],
                    content_bytes_per_row)) {
      return false;
    }
  }
...
void Video::set_palette(Palette const &palette) { m_impl->palette = palette; }

void Video::set_palette(Palette &&palette) {
  m_impl->palette = std::move(palette);
}

Palette const &Video::get_palette() const { return m_impl->palette; }

Palette &Video::get_palette() { return m_impl->palette; }
...
int Video::get_width() const { return m_impl->width; }

int Video::get_height() const { return m_impl->height; }
...
int Video::get_pitch() const { return m_impl->pitch; }
...
VisVideoDepth Video::get_depth() const { return m_impl->depth; }

int Video::get_bpp() const { return m_impl->bpp; }
...
std::size_t Video::get_size() const { return m_impl->pitch * m_impl->height; }

BufferPtr Video::get_buffer() const { return m_impl->buffer; }

void *Video::get_pixels() const { return m_impl->buffer->get_data(); }

void *Video::get_pixel_ptr(int x, int y) const {
  return static_cast<uint8_t *>(get_pixels()) + y * m_impl->pitch +
         x * m_impl->bpp;
}

Rect const &Video::get_extents() const { return m_impl->extents; }

void Video::set_extents(Rect area) { m_impl->extents = area; }
...
void Video::set_compose_surface(uint8_t alpha) { m_impl->alpha = alpha; }

void Video::set_compose_function(VisVideoComposeFunc compose_func) {
  m_impl->compose_func = compose_func;
}

VisVideoComposeFunc Video::get_compose_function(VideoConstPtr const &src,
                                                bool alpha) {
...
void Video::blit(Rect const &drect, VideoConstPtr const &src, Rect const &srect,
                 bool alpha) {
...
void Video::compose(Rect const &drect, VideoConstPtr const &src,
                    Rect const &srect, VisVideoComposeFunc compose_func) {
  auto ndrect = drect;
...
void Video::blit_scale(Rect const &drect, VideoConstPtr const &src,
                       Rect const &srect, bool alpha,
                       VisVideoScaleMethod scale_method) {
...
void Video::compose_scale(Rect const &drect, VideoConstPtr const &src,
                          Rect const &srect, VisVideoScaleMethod scale_method,
  ...
  auto svid = create();
  svid->set_attrs(drect.width, drect.height, src->m_impl->bpp * drect.width,
                  src->m_impl->depth);
...
void Video::compose(VideoConstPtr const &src, int x, int y,
                    VisVideoComposeFunc compose_func) {
...
void Video::rotate(VideoConstPtr const &src, VisVideoRotateDegrees degrees) {
  ...
  case VISUAL_VIDEO_ROTATE_NONE:
    if (m_impl->width == src->m_impl->width &&
        m_impl->height == src->m_impl->height)
      blit(src, 0, 0, false);
    return;
...
void Video::convert_depth(VideoConstPtr const &src) {
  /* We blit overlay it instead of just visual_mem_copy because the pitch can
   * still be different */
  if (m_impl->depth == src->m_impl->depth) {
    blit(src, 0, 0, false);
    return;
  ...
  if (m_impl->depth == VISUAL_VIDEO_DEPTH_8BIT ||
      src->m_impl->depth == VISUAL_VIDEO_DEPTH_8BIT) {
  ...
  if (src->m_impl->depth == VISUAL_VIDEO_DEPTH_8BIT) {
    ...
    default:
      visual_log(VISUAL_LOG_ERROR,
                 "Invalid depth conversion requested (%d -> %d)",
...
void Video::scale(VideoConstPtr const &src, VisVideoScaleMethod method) {
  ...
  /* If the dest and source are equal in dimension and scale_method is nearest,
   * do a blit overlay */
...
void Video::scale_depth(VideoConstPtr const &src,
                        VisVideoScaleMethod scale_method) {
  if (m_impl->depth != src->m_impl->depth) {
    auto dtransform = create();
    dtransform->set_attrs(m_impl->width, m_impl->height,
                          m_impl->width * m_impl->bpp, m_impl->depth);
...
VisVideoDepth visual_video_depth_get_next(int depthflag, VisVideoDepth depth) {
  visual_return_val_if_fail(visual_video_depth_is_sane(depth),
