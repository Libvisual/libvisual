...
VideoPtr bitmap_load_bmp(std::istream &fp) {
  ...
  if (bi_bitcount != 1 && bi_bitcount != 4 && bi_bitcount != 8 &&
      bi_bitcount != 24) {
  ...
  auto video =
      Video::create(bi_width, bi_height, visual_video_depth_from_bpp(depth));

  if (palette)
    video->set_palette(*palette);

  /* Set to the beginning of image data, note that MickeySoft likes stuff upside
   * down .. */
