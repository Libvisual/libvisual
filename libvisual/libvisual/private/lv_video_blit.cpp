...
void VideoBlit::blit_overlay_noalpha(Video *dest, Video *src) {
  ...
  /* src and dest are completely equal, do one big mem copy instead of a per
   * line mem copy. Also check if the pitch is equal to it's width * bpp, this
   * is because of subregions. */
  if (dest->compare_attrs(src) &&
      (src->m_impl->pitch == (src->m_impl->width * src->m_impl->bpp))) {
...
void VideoBlit::blit_overlay_surfacealpha(Video *dest, Video *src) {
  ...
  if (dest->m_impl->depth == VISUAL_VIDEO_DEPTH_8BIT) {
    ...
    for (int y = 0; y < src->m_impl->height; y++) {
      ...
      destbuf +=
          dest->m_impl->pitch - (dest->m_impl->width * dest->m_impl->bpp);
      srcbuf += src->m_impl->pitch - (src->m_impl->width * src->m_impl->bpp);
    }
