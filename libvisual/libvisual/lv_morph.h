...
inline void intrusive_ptr_add_ref(Morph const *morph) { morph->m_ref_count++; }
 ...
 *
 * The palette() method is called to obtain the colour palette used in 8-bit
 * rendering modes.
 *
 ...
 * @param src2     Second source Video
 */
typedef void (*VisPluginMorphPaletteFunc)(VisPluginData *plugin, float progress,
                                          VisAudio *audio, VisPalette *palette,
 ...
 * @param src2     Second source Video
 */
typedef void (*VisPluginMorphApplyFunc)(VisPluginData *plugin, float progress,
...
struct _VisMorphPlugin {
  VisPluginMorphPaletteFunc
      palette; /**< Returns the color palette (only for 8-bit morphs) */
...
LV_API int visual_morph_run(VisMorph *morph, VisAudio *audio, VisVideo *src1,
                            VisVideo *src2);
