...
#define VISUAL_PLUGIN_API_VERSION_VALIDATOR                                    \
  LV_C_LINKAGE LV_PLUGIN_EXPORT const int __lv_plugin_libvisual_api_version =  \
...
typedef enum {
  VISUAL_PLUGIN_FLAG_NONE =
      0, /**< Used to indicate the absence of special flags */
  VISUAL_PLUGIN_FLAG_REENTRANT =
      1 /**< Indicate that plugin is safe for multiple instantiation */
} VisPluginFlags;
 ...
 *
 * The init() method is called to initialise the plugin state and pre-allocate
 * needed resources for the Plugin to run.
 *
 ...
 *
 * The cleanup() method is called to clean up the plugin state and free any
 * allocated resources.
 *
 * @param plugin Plugin object
 */
 ...
 * @return TRUE on success, FALSE on failure
 */
typedef int (*VisPluginEventsFunc)(VisPluginData *plugin,
 ...
 * Loads a Plugin.
 *
 * @note The loaded plugin will need to be initialised with
 ...
 */
LV_API char const *plugin_get_next_by_name(PluginList const &list,
                                           char const *name);
 ...
 */
LV_API char const *plugin_get_prev_by_name(PluginList const &list,
                                           char const *name);
