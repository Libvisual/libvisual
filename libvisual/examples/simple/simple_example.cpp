...
enum CycleDirection { CYCLE_DIR_PREV, CYCLE_DIR_NEXT };
...
SimpleExample::SimpleExample()
    : m_actor_name(ACTOR_PLUGIN_NAME), m_input_name(INPUT_PLUGIN_NAME),
...
LV::VideoPtr SimpleExample::create_display(int width, int height,
                                           VisVideoDepth depth) {
  m_screen.reset();
  ...
  if (depth == VISUAL_VIDEO_DEPTH_GL) {
    ...
    m_sdl_screen = SDL_SetVideoMode(width, height,
                                    visual_video_depth_from_bpp(depth), flags);
  }
...
void SimpleExample::morph_to_actor(std::string const &actor_name,
                                   std::string const &morph_name) {
