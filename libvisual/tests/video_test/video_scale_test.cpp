...
LV::VideoPtr lv_video_wrap_sdl_surface(SDL_Surface *surface) {
  return LV::Video::wrap(
      surface->pixels, false, surface->w, surface->h,
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    SDL_Surface *sdl_screen =
        SDL_SetVideoMode(DISPLAY_WIDTH, DISPLAY_HEIGHT, 24, sdl_video_flags);
    if (!sdl_screen) {
    ...
    auto const image_file_path =
        (argc > 1) ? argv[1] : "../images/landscape.png";
    ...
    while (running) {
      ...
      while (SDL_PollEvent(&event)) {
        ...
        case SDL_VIDEORESIZE:
          sdl_screen = SDL_SetVideoMode(event.resize.w, event.resize.h, 24,
                                        sdl_video_flags);
          if (!sdl_screen) {
