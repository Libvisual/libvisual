...
class VideoConvertDepthBench : public LV::Tools::Benchmark {
public:
  VideoConvertDepthBench(unsigned int width, unsigned int height,
                         VisVideoDepth src_depth, VisVideoDepth dst_depth)
      : Benchmark{"VideoConvertDepthBench"}, m_src{LV::Video::create(
                                                 width, height, src_depth)},
...
std::unique_ptr<VideoConvertDepthBench> make_benchmark(int &argc,
                                                       char **&argv) {
  ...
  if (argc > 2) {
    ...
    if (src_depth == VISUAL_VIDEO_DEPTH_NONE ||
        dst_depth == VISUAL_VIDEO_DEPTH_NONE) {
      throw std::invalid_argument("Invalid bit depths specified");
    }

    if (src_depth == dst_depth) {
      throw std::invalid_argument(
          "Bit depths are the same, Nothing to test!\n");
    }
  ...
  return std::make_unique<VideoConvertDepthBench>(width, height, src_depth,
                                                  dst_depth);
}
