...
class ActorBench : public LV::Tools::Benchmark {
public:
  ActorBench(std::string const &actor_name, unsigned int width, unsigned height,
             VisVideoDepth depth, bool forced_depth)
      : Benchmark{"ActorBench"}, m_actor{LV::Actor::load(actor_name)} {
    (void)width;
    (void)height;
...
std::unique_ptr<ActorBench> make_benchmark(int &argc, char **&argv) {
  ...
  return std::make_unique<ActorBench>(actor_name, width, height, depth,
                                      forced_depth);
}
