...
Display::Display(std::string const &driver_name) : m_impl(new Impl) {
  m_impl->driver.reset(
      DisplayDriverFactory::instance().make(driver_name, *this));

  if (!m_impl->driver) {
    throw std::runtime_error("Failed to load display driver '" + driver_name +
                             "'. Valid driver set? (\"--driver\" parameter)");
  }
}

Display::~Display() {}

LV::VideoPtr Display::get_video() const { return m_impl->driver->get_video(); }

LV::VideoPtr Display::create(VisVideoDepth depth,
                             VisVideoAttrOptions const *vidoptions,
                             unsigned int width, unsigned int height,
                             bool resizable) {
  visual_log(VISUAL_LOG_INFO, "Attempting to create display (%dx%d %s)", width,
             height, visual_video_depth_name(depth));
...
void Display::lock() { m_impl->driver->lock(); }

void Display::unlock() { m_impl->driver->unlock(); }
...
bool Display::is_fullscreen() const { return m_impl->driver->is_fullscreen(); }
