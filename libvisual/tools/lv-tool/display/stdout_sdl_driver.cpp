...
class StdoutSDLDriver : public DisplayDriver {
public:
  StdoutSDLDriver(Display &display)
      : m_display(display), m_screen(0), m_screen_video(0),
        m_requested_depth(VISUAL_VIDEO_DEPTH_NONE), m_last_width(0),
        m_last_height(0), m_resizable(false), m_active(false),
        m_running(false) {}

  virtual ~StdoutSDLDriver() { close(); }
  ...
  virtual LV::VideoPtr create(VisVideoDepth depth,
    ...
    if (!SDL_WasInit(SDL_INIT_VIDEO)) {
      if (SDL_Init(SDL_INIT_VIDEO) == -1) {
        visual_log(VISUAL_LOG_ERROR, "Unable to init SDL VIDEO: %s",
                   SDL_GetError());
        return nullptr;
      }
    }
    ...
    if (depth == VISUAL_VIDEO_DEPTH_GL) {
      ...
      if (vidoptions) {
        for (unsigned int i = VISUAL_GL_ATTRIBUTE_NONE;
             i < VISUAL_GL_ATTRIBUTE_LAST; i++) {
    ...
    m_screen_video = LV::Video::wrap(m_screen->pixels, false, m_screen->w,
                                     m_screen->h, depth, m_screen->pitch);
  ...
  virtual void set_fullscreen(bool fullscreen, bool autoscale) {
    if (fullscreen) {
      ...
      if ((m_screen->flags & SDL_FULLSCREEN)) {
        ...
        if (autoscale) {
          create(m_requested_depth, nullptr, m_last_width, m_last_height,
                 m_resizable);
        }
      }
    }
  ...
  virtual void update_rect(LV::Rect const &rect) {
    ...
    if (m_requested_depth == VISUAL_VIDEO_DEPTH_GL) {
      ...
      // Read pixels
      glReadPixels(rect.x, rect.y, rect.width, rect.height, GL_BGR,
                   GL_UNSIGNED_BYTE, raw_buffer1.data());

      // Manually flip image on the CPU
      // glReadPixels() returns a vertically-inverted image and there's no way
      // to control this
  ...
  virtual void drain_events(VisEventQueue &eventqueue) {
    ...
    if (((SDL_GetAppState() & SDL_APPACTIVE) == 0) && m_active) {
      m_active = false;
      visual_event_queue_add(&eventqueue, visual_event_new_visibility(FALSE));
    } else if (((SDL_GetAppState() & SDL_APPACTIVE) != 0) && !m_active) {
      m_active = true;
      visual_event_queue_add(&eventqueue, visual_event_new_visibility(TRUE));
    }
    ...
    while (SDL_PollEvent(&event)) {
      ...
      switch (event.type) {
      case SDL_KEYUP:
        visual_event_queue_add(&eventqueue, visual_event_new_keyboard(
      ...
      case SDL_VIDEORESIZE:
        visual_event_queue_add(
            &eventqueue,
            visual_event_new_resize(event.resize.w, event.resize.h));

        create(m_display.get_video()->get_depth(), nullptr, event.resize.w,
               event.resize.h, m_resizable);
        break;

      case SDL_MOUSEMOTION:
        visual_event_queue_add(
            &eventqueue,
            visual_event_new_mousemotion(event.motion.x, event.motion.y));
        break;

      case SDL_MOUSEBUTTONDOWN:
        visual_event_queue_add(
            &eventqueue,
            visual_event_new_mousebutton(event.button.button, VISUAL_MOUSE_DOWN,
      ...
      case SDL_MOUSEBUTTONUP:
        visual_event_queue_add(
            &eventqueue,
            visual_event_new_mousebutton(event.button.button, VISUAL_MOUSE_UP,
                                         event.button.x, event.button.y));
        break;

      case SDL_QUIT:
        visual_event_queue_add(&eventqueue, visual_event_new_quit());
        break;
