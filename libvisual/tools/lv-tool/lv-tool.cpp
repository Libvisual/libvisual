...
const std::unordered_set<std::string> actors_to_skip{"gdkpixbuf", "gstreamer",
                                                     "lv_analyzer"};
...
class Libvisual {
public:
  Libvisual(int &argc, char **&argv) { LV::System::init(argc, argv); }

  ~Libvisual() { LV::System::destroy(); }
};
...
void print_plugin_info(VisPluginInfo const &info) {
  printf("Plugin: \"%s\" (%s)\n"
         ...
         "\tAuthor: %s\n\tVersion: %s\tLicense: %s\n"
         "\t%s - %s\n\n",
         info.name, info.plugname, info.url ? info.url : "<n/a>", info.author,
         info.version, info.license, info.about, info.help);
}

/** print help for plugins */
void print_plugin_help() {
  printf("===== INPUTS =====\n");
  auto const &inputs = LV::PluginRegistry::instance()->get_plugins_by_type(
  ...
  printf("===== ACTORS =====\n");
  auto const &actors = LV::PluginRegistry::instance()->get_plugins_by_type(
  ...
  printf("===== MORPHS =====\n");
  auto const &morphs = LV::PluginRegistry::instance()->get_plugins_by_type(
...
void print_help(std::string const &name) {
  printf("Usage: %s [options]\n\n"
         ...
         "\t--verbose\t\t-v\t\tIncrease log verbosity (may be used multiple "
         "times)\n"
         "\t--dimensions <wxh>\t-D <wxh>\tRequest dimensions from display "
         "driver (no guarantee) [%dx%d]\n"
         "\t--depth <depth> \t-c <depth>\tSet output colour depth (automatic "
         "by default)\n"
         ...
         "\t--fps <n>\t\t-f <n>\t\tLimit output to n frames per second (if "
         "display driver supports it) [%d]\n"
         "\t--framecount <n>\t-F <n>\t\tOutput n frames, then exit.\n"
         "\t--switch <n>\t\t-S <n>\t\tSwitch actor after n frames.\n"
         "\t--exclude <actors>\t-x <actors>\tProvide a list of actors to "
         "exclude.\n"
         "\n",
         name.c_str(), width, height, driver_name.c_str(), input_name.c_str(),
         DEFAULT_ACTOR_GL, DEFAULT_ACTOR_NONGL, morph_name.c_str(), frame_rate);
...
int parse_args(int argc, char *argv[]) {
  ...
  while ((argument = getopt_long(argc, argv, "hpvD:d:i:a:m:f:s:F:S:x:c:",
                                 loptions, &index)) >= 0) {
    ...
    case 'D': {
      if (std::sscanf(optarg, "%ux%u", &width, &height) != 2) {
        std::cerr << "Invalid dimensions: '" << optarg
                  << "'. Use <width>x<height> (e.g. 640x480)\n";
        return -1;
      }
      break;
    }
    ...
    case 'c': {
      if (std::sscanf(optarg, "%u", &color_depth) != 1 ||
        ...
        std::cerr << "Invalid depth: '" << optarg
                  << "'. Use integer value (e.g. 24)\n";
        return -1;
      }
      break;
    }
...
int main(int argc, char **argv) {
  try {
    // print warm welcome
    std::cerr << visual_truncate_path(argv[0], 1) << " - " << PACKAGE_STRING
    ...
    if (!bin.connect(actor_name, input_name)) {
      throw std::runtime_error("Failed to start pipeline with actor '" +
                               actor_name + "' and input '" + input_name + "'");
    }
    ...
    uint64_t const frame_period_us =
        frame_rate > 0 ? VISUAL_USECS_PER_SEC / frame_rate : 0;
    ...
    while (running) {
      ...
      if (frame_rate > 0) {
        if (frames_drawn > 0) {
          draw_frame =
              (LV::Time::now() - last_frame_time).to_usecs() >= frame_period_us;
        }
      }
      ...
      if (draw_frame) {
        ...
        if (actor_switch_after_frames > 0 &&
            frames_drawn >=
                actor_switch_after_frames + actor_switch_framecount) {
      ...
      auto pluginqueue =
          visual_plugin_get_event_queue(bin.get_actor()->get_plugin());
