 ...
 *
 * 	@param obj Runnable Object containing @a name.
...
/**
 *	Bind a Runnable Object variable to a user supplied pointer.
 *	
 *	@param obj Runnable Object
...
{
	AvsRunnableVariable *var = avs_runnable_variable_find(obj, name);
...
int avs_runnable_variable_bind(AvsRunnable *obj, char *name, AvsNumber *value)
{
	...
	if (!var)
		var = avs_runnable_variable_create(obj, name, -1);
