...
static inline AvsRunnableVariable * create_result(ILAGlobalData *vm)
{
	AvsRunnableVariable *var = avs_blob_pool_push(vm->rvpool);
...
static inline AvsRunnableVariable * create_constant(ILAGlobalData *vm, AvsNumber value)
{
	AvsRunnableVariable *var = avs_blob_pool_push(vm->rvpool);
...
static inline AvsRunnableVariable * find_constant(ILAGlobalData *vm, AvsNumber constant)
{
...
{
	ILARunnableData *rd = VM_RUNNABLEDATA(obj->pasm);
	ILAInstructionContext *ix;
	ILAInstruction *v, *vx;
...
static inline void free_instruction(ILAGlobalData *gd, ILAInstruction *insn)
{
	/* Free instruction */
	if (insn->fncount)
...
static inline void free_instruction_tree(ILAGlobalData *gd, ILAInstruction *insn)
{
	ILAInstruction *next;
...
static void free_context(ILAGlobalData *gd, ILAInstructionContext *ix)
{
	free_instruction_tree(gd, ix->base);
}

static void rollback(ILAGlobalData *gd)
{
	ILAInstructionContext *ix;
	int context, count;
...
{
	ILAGlobalData *vm = VM_GLOBALDATA(ctx->pdata);
...
ASSEMBLER_RUNNABLE_INIT(vm_object_init)
{
	...
	ILARunnableData *vm = visual_mem_malloc0(sizeof(ILARunnableData));
	memset(vm, 0, sizeof(ILARunnableData));
	obj->pasm = vm;
...
{
	ILARunnableData *vm = VM_RUNNABLEDATA(obj->pasm);
...
ASSEMBLER_CLEANUP(vm_cleanup)
{
	...
	visual_object_unref(VISUAL_OBJECT(vm->rvpool));
 ...
 */
int avs_il_assembler_init(AvsILAssemblerContext *ctx)
{
	avs_debug(print("IL assembler: Initialize..."));
	ILAGlobalData *vm = visual_mem_malloc0(sizeof(ILAGlobalData));
	memset(vm, 0, sizeof(ILAGlobalData));
	ctx->pdata = vm;
	...
	/* Allocate blob pools */
	vm->rvpool = avs_blob_pool_new0(AvsRunnableVariable);
