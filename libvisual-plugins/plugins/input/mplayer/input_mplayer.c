 ...
 *
 * Copyright (C) 2004, 2005, 2006 Gustavo Sverzut Barbieri
...
#ifndef SHARED_FILE
#define SHARED_FILE                                                            \
  ".mplayer/mplayer-af_export" /**< default file name, relative to $HOME */
#endif                         /* SHARED_FILE */
...
const VisPluginInfo *get_plugin_info(void) {
  static VisInputPlugin input = {.upload = inp_mplayer_upload};
...
static int inp_mplayer_init(VisPluginData *plugin) {
  ...
  priv->sharedfile = visual_mem_malloc0(
      sizeof(char) * (strlen(SHARED_FILE) + strlen(getenv("HOME")) + 2));
  ...
  if (priv->fd < 0) {
     ...
     */
    visual_log(VISUAL_LOG_CRITICAL, "Could not open file '%s': %s",
               priv->sharedfile, strerror(errno));
    return FALSE;
  }

  priv->mmap_area =
      mmap(0, sizeof(mplayer_data_t), PROT_READ, MAP_SHARED, priv->fd, 0);
  ...
  if ((priv->mmap_area->nch != 2) || (priv->mmap_area->bs != 2048)) {
  ...
  priv->mmap_area = mremap(priv->mmap_area, sizeof(mplayer_data_t),
                           sizeof(mplayer_data_t) + priv->mmap_area->bs, 0);
  if ((intptr_t)priv->mmap_area == -1) {
    visual_log(VISUAL_LOG_CRITICAL,
               "Could not mremap() area from file '%s' "
               " (%p from %" VISUAL_SIZE_T_FORMAT " to %" VISUAL_SIZE_T_FORMAT
               " bytes): %s",
               priv->sharedfile, (void *)priv->mmap_area,
               sizeof(mplayer_data_t),
               sizeof(mplayer_data_t) + priv->mmap_area->bs, strerror(errno));
    return FALSE;
  }
...
static void inp_mplayer_cleanup(VisPluginData *plugin) {
  ...
  if (priv->loaded == 1) {
    ...
    if (priv->fd > 0) {
      if (close(priv->fd) != 0) {
        visual_log(VISUAL_LOG_CRITICAL,
                   "Could not close file descriptor %d: %s", priv->fd,
                   strerror(errno));
      }
      priv->fd = -1;
    } else {
      visual_log(VISUAL_LOG_CRITICAL, "Wrong file descriptor %d", priv->fd);
    }

    if (munmap(mmap_area, mmap_count) != 0) {
      visual_log(VISUAL_LOG_CRITICAL, "Could not munmap() area %p+%d. %s",
...
static int inp_mplayer_upload(VisPluginData *plugin, VisAudio *audio) {
  ...
  VisBuffer *buffer = visual_buffer_new_wrap_data(
      (uint8_t *)priv->mmap_area + sizeof(mplayer_data_t), 2048, FALSE);
