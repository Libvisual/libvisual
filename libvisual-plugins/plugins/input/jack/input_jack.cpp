...
VisPluginInfo const *get_plugin_info() {
  static VisInputPlugin input = {inp_jack_upload};
...
int inp_jack_init(VisPluginData *plugin) {
  ...
  // server if needed
  priv->client =
      jack_client_open("Libvisual", options, &status, JACK_SERVER_NAME);
  if (!priv->client) {
    visual_log(VISUAL_LOG_ERROR,
               "Cannot connect to JACK server: status = 0x%2.0x", status);
    return FALSE;
  }
  ...
  priv->input_port = jack_port_register(
      priv->client, "input", JACK_DEFAULT_AUDIO_TYPE, JackPortIsInput, 0);
  if (!priv->input_port) {
  ...
  const char **ports = jack_get_ports(priv->client, NULL, NULL,
...
void inp_jack_cleanup(VisPluginData *plugin) {
  JackPrivate *priv =
      static_cast<JackPrivate *>(visual_plugin_get_private(plugin));
...
int inp_jack_upload(VisPluginData *plugin, VisAudio *audio) {
  ...
  LV::BufferPtr buffer = LV::Buffer::wrap(
      priv->buffer.data(), priv->buffer.size() * sizeof(int16_t), false);
...
int process_callback(jack_nframes_t nframes, void *arg) {
  ...
  jack_default_audio_sample_t *in = static_cast<jack_default_audio_sample_t *>(
      jack_port_get_buffer(priv->input_port, nframes));
