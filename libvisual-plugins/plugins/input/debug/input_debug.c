...
#define UPLOAD_PERIOD_USECS                                                    \
  (((uint64_t)OUTPUT_SAMPLES * VISUAL_USECS_PER_SEC) / OUTPUT_RATE)
...
const VisPluginInfo *get_plugin_info(void) {
  static VisInputPlugin input = {.upload = inp_debug_upload};
...
static int inp_debug_upload(VisPluginData *plugin, VisAudio *audio) {
  ...
  int64_t sleep_time =
      (int64_t)UPLOAD_PERIOD_USECS - (int64_t)visual_time_to_usecs(diff_time);
  if (sleep_time > 0) {
