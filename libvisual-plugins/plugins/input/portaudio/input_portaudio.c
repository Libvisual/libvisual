...
const VisPluginInfo *get_plugin_info(void) {
  static VisInputPlugin input = {.upload = inp_portaudio_upload};

  static VisPluginInfo info = {.type = VISUAL_PLUGIN_TYPE_INPUT,
                               ...
                               .help = N_("Use this plugin to capture PCM data "
                                          "from the PortAudio record device"),
...
int inp_portaudio_init(VisPluginData *plugin) {
  ...
  const PaError open_error =
      Pa_OpenStream(&priv->stream, &input_parameters, NULL, SAMPLE_RATE, FRAMES,
                    paClipOff, NULL, NULL);
  visual_return_val_if_fail(open_error == paNoError, FALSE);

  // Allocate buffer
  const visual_size_t buffer_size_bytes =
      FRAMES * CHANNELS * (SAMPLE_FORMAT_BITS / 8);
...
int inp_portaudio_upload(VisPluginData *plugin, VisAudio *audio) {
  ...
  for (;;) {
    ...
    if (frames_to_read > FRAMES) {
      frames_to_read =
          FRAMES; // because that's all that priv->buffer has space for
    }

    const PaError read_error =
        Pa_ReadStream(priv->stream, priv->buffer, frames_to_read);
    if (read_error != paNoError) {
      return TRUE;
    }

    const visual_size_t bytes_to_write =
        frames_to_read * CHANNELS * (SAMPLE_FORMAT_BITS / 8);

    VisBuffer *const buffer =
        visual_buffer_new_wrap_data(priv->buffer, bytes_to_write, FALSE);
    visual_return_val_if_fail(priv != NULL, FALSE);

    visual_audio_input(audio, buffer, SAMPLE_RATE_TYPE_LV, SAMPLE_FORMAT_LV,
                       CHANNELS_TYPE);
