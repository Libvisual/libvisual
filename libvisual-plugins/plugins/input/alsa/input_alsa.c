...
const VisPluginInfo *get_plugin_info(void) {
  static VisInputPlugin input = {.upload = inp_alsa_upload};
  ...
  static VisPluginInfo info = {
      ...
      .help =
          N_("Use this plugin to capture PCM data from the ALSA record device"),
...
int inp_alsa_init(VisPluginData *plugin) {
  ...
  if ((err = snd_pcm_open(&priv->chandle, inp_alsa_var_cdevice,
    ...
    visual_log(VISUAL_LOG_ERROR, "Record open error: %s", snd_strerror(err));
    return FALSE;
  }
  ...
  if (snd_pcm_hw_params_any(priv->chandle, hwparams) < 0) {
    visual_log(VISUAL_LOG_ERROR, "Cannot configure this PCM device");
  ...
  if (snd_pcm_hw_params_set_rate_near(priv->chandle, hwparams, &exact_rate,
                                      &dir) < 0) {
  ...
  tmp = (int)ceil((1000.0 * 1000.0 * inp_alsa_var_frames_target) /
                  inp_alsa_var_samplerate); // in micro seconds
  if (snd_pcm_hw_params_set_period_time_near(priv->chandle, hwparams, &tmp,
                                             &dir) < 0) {
  ...
  tmp = (int)ceil((1000.0 * 1000.0 * inp_alsa_var_frames_target) /
                  inp_alsa_var_samplerate); // in micro seconds
  if (snd_pcm_hw_params_set_buffer_time_near(priv->chandle, hwparams, &tmp,
                                             &dir) < 0) {
...
int inp_alsa_upload(VisPluginData *plugin, VisAudio *audio) {
  ...
  const snd_pcm_uframes_t frames_wanted =
      sizeof(data) / sizeof(data[0]) / inp_alsa_var_channels;
  ...
  do {
    ...
    if (rcnt > 0) {
      ...
      buffer = visual_buffer_new_wrap_data(
          data, rcnt * sizeof(data[0]) * inp_alsa_var_channels, FALSE);
