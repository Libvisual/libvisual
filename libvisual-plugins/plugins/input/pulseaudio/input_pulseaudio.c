...
pa_sample_spec sample_spec = {
    .format = SAMPLE_FORMAT_PA, .rate = SAMPLE_RATE, .channels = CHANNELS};
...
const VisPluginInfo *get_plugin_info(void) {
  static VisInputPlugin input = {.upload = inp_pulseaudio_upload};
...
static int inp_pulseaudio_init(VisPluginData *plugin) {
  ...
  pa_mainloop_api *const mainloop_api =
      pa_threaded_mainloop_get_api(priv->mainloop);
  ...
  priv->input_stream =
      pa_stream_new(priv->context, "Recording", &sample_spec, NULL);
...
static int inp_pulseaudio_upload(VisPluginData *plugin, VisAudio *audio) {
  ...
  // `priv->chunks_written` is monotonically increasing in another thread in
  // parallel, so we make a snapshot to work with a single consistent value
  // below. Also, plain reads to 64bit are not atomic on 32bit platforms, so we
  // add protection. This is attomic `priv->chunks_written =
  // frozen_chunks_written`.
  uint64_t frozen_chunks_written =
      atomic_load_explicit(&priv->chunks_written, memory_order_acquire);
  ...
  while (priv->chunks_read < target_chunks_read) {
    ...
    VisBuffer *const visbuffer =
        visual_buffer_new_wrap_data(data, CHUNK_SIZE_BYTES, FALSE);
    visual_return_val_if_fail(visbuffer != NULL, FALSE);
    visual_audio_input(audio, visbuffer, SAMPLE_RATE_TYPE_LV, SAMPLE_FORMAT_LV,
                       CHANNELS_TYPE_LV);
    priv->chunks_read++;
...
static void on_input_stream_data(pa_stream *p, size_t nbytes, void *userdata) {
  ...
  // Copy all readable bytes from `source` to the right place in
  // `priv->pcm_data`.
  while (nbytes > 0) {
    void *const target = (void *)priv->pcm_data[priv->chunks_written % CHUNKS] +
                         priv->chunk_write_offset_bytes;
    ...
    if (priv->chunk_write_offset_bytes + round_nbytes < CHUNK_SIZE_BYTES) {
      ...
      atomic_store_explicit(&priv->chunks_written, new_chunks_written,
