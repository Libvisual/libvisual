...
bool Corona::setUpSurface(int width, int height) {
  ...
  m_deltafield =
      (unsigned char **)malloc(m_width * m_height * sizeof(unsigned char *));
  ...
  m_particles =
      (Particle *)visual_mem_realloc(m_particles, sizeof(Particle) * newsize);
...
void Corona::setPointDelta(int x, int y) {
  ...
  int dx =
      (int)((ds * m_swirl.pull * cos(ang) - tx) * m_width) + LV::rand() % 5 - 2;
  int dy = (int)((ds * m_swirl.pull * sin(ang) - ty) * m_height) +
           LV::rand() % 5 - 2;
  if (x + dx < 0)
    dx = -dx - x;
  if (x + dx >= m_width)
    dx = 2 * m_width - 2 * x - dx - 1;
  if (y + dy < 0)
    dy = -dy - y;
  if (y + dy >= m_height)
    dy = 2 * m_height - 2 * y - dy - 1;
...
void Corona::genReflectedWaves(double loop) {
  ...
  for (int i = 0; i < m_real_height - m_height; ++i) {
    double fincr =
        (3.1415 / 2.0) * (1.0 - (fwidth - REFL_MIN_WIDTH) / REFL_MAX_WIDTH);
    floop += fincr;
  ...
  if (visual_cpu_has_mmx()) {
    ...
    while (n -= 4) {
      __asm __volatile("\n\t movd %[dest], %%mm0"
                       ...
                       "\n\t psrlw $2, %%mm4"
                       "\n\t packuswb %%mm6, %%mm4"
                       "\n\t movd %%mm4, %[dest]" ::[dest] "m"(*(ptr)),
                       [pix2] "m"(*(ptr - m_width)), [pix3] "m"(*(ptr - 1)),
                       [pix4] "m"(*(ptr + 1)), [pix5] "m"(*(ptr + m_width)));
      ptr += 4;
    }
