...
static void lv_flower_requisition(VisPluginData *plugin, int *width,
                                  int *height);
...
static void lv_flower_render(VisPluginData *plugin, VisVideo *video,
                             VisAudio *audio);
static VisPalette *lv_flower_palette(VisPluginData *plugin);

/* Main plugin stuff */
const VisPluginInfo *get_plugin_info(void) {
  static VisActorPlugin actor = {.requisition = lv_flower_requisition,
  ...
  static VisPluginInfo info = {
      ...
      .author = N_("Original by: Antti Silvast <asilvast@iki.fi>, Port by: "
                   "Dennis Smit <ds@nerds-incorporated.org>"),
      .version = "0.1",
...
static int lv_flower_init(VisPluginData *plugin) {
  ...
  priv->flower.continuity =
      (visual_random_context_float(priv->rcxt) - 0.5) * 16.0;
...
static VisPalette *lv_flower_palette(VisPluginData *plugin) { return NULL; }
...
static void lv_flower_render(VisPluginData *plugin, VisVideo *video,
  ...
  visual_audio_get_sample_mixed_simple(
      audio, pcmbuf, 2, VISUAL_AUDIO_CHANNEL_LEFT, VISUAL_AUDIO_CHANNEL_RIGHT);
  ...
  if (visual_timer_is_past2(priv->t, 15, 0)) {
    priv->flower.tension_new =
        (-visual_random_context_float(priv->rcxt)) * 12.0;
    priv->flower.continuity_new =
        (visual_random_context_float(priv->rcxt) - 0.5) * 32.0;
...
#define DIF 5.0
    ...
    for (i = 0; i < priv->nof_bands; i++) {
      ...
      y = ((DIF - 2.0) * y + (i == 0 ? 0 : temp_bars[i - 1]) +
           (i == (NOTCH_BANDS - 1) ? 0 : temp_bars[i + 1])) /
          DIF;
