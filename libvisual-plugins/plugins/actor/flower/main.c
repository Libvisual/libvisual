...
static void splineTCP(FlowerInternal *flower, float u, Vect *control,
                      Vect *result) {

  (*result)[0] =
      (2 * u * u * u - 3 * u * u + 1) * control[1][0] +
      (-2 * u * u * u + 3 * u * u) * control[2][0] +
      (u * u * u - 2 * u * u + u) *
          (0.5 * (1 - flower->tension) *
           ((1 + flower->bias) * (1 - flower->continuity) *
                (control[1][0] - control[0][0]) +
            (1 - flower->bias) * (1 + flower->continuity) *
                (control[2][0] - control[1][0]))) +
      (u * u * u - u * u) * (0.5 * (1 - flower->tension) *
                             ((1 + flower->bias) * (1 + flower->continuity) *
                                  (control[2][0] - control[1][0]) +
                              (1 - flower->bias) * (1 - flower->continuity) *
                                  (control[3][0] - control[2][0])));
      ...
      (u * u * u - 2 * u * u + u) *
          (0.5 * (1 - flower->tension) *
           ((1 + flower->bias) * (1 - flower->continuity) *
            ...
            (1 - flower->bias) * (1 + flower->continuity) *
                (control[2][1] - control[1][1]))) +
      (u * u * u - u * u) * (0.5 * (1 - flower->tension) *
                             ((1 + flower->bias) * (1 + flower->continuity) *
                                  (control[2][1] - control[1][1]) +
                              (1 - flower->bias) * (1 - flower->continuity) *
                                  (control[3][1] - control[2][1])));
...
static void spline3DMorph(FlowerInternal *flower, float factor,
                          float poikkeama) {
#define NBTW 12
  int i, j, k;
  ...
  for (i = 0; i < (c - 1); i++) {
    ...
    set_vector(vertices[1], 1.0 * rf[i * 3 + 3], -rf[i * 3 + 3 + 2],
               rf[i * 3 + 3 + 1]);
    copy_vector(normals[1], n);
    set_vector(vertices[2], 1.0 * rf[i * 3 + 3], +rf[i * 3 + 3 + 2],
               rf[i * 3 + 3 + 1]);
...
static void render_flower(FlowerInternal *flower) {
  ...
  flower->continuity =
      F * flower->continuity_new + (1.0 - F) * flower->continuity;
  flower->bias = F * flower->bias_new + (1.0 - F) * flower->bias;

  for (t = 0; t < 12; t++) {
    glRotatef(360.0 / 12.0, 0.0, 0.0, 1.0);
    spline3DMorph(
        flower, sin(time * 0.001 * flower->spd) * 0.5 + 0.5,
...
void render_flower_effect(FlowerInternal *flower) {
  ...
  glLoadIdentity();
  gluPerspective(60.0, (float)flower->width / (float)flower->height, 0.01,
                 1135.0);
