...
class LCDGraphicWrapper {
    ...
    void ResizeBefore(int rows, int cols) { wrappedObject_->ResizeBefore(rows,
cols); } void ResizeAfter() { wrappedObject_->ResizeAfter(); } void
LayoutChangeBefore() { wrappedObject_->LayoutChangeBefore(); } void
LayoutChangeAfter() { wrappedObject_->LayoutChangeAfter(); }
...
public:
  ...
  void GraphicDraw();
  void GraphicInit(const int rows, const int cols, const int yres,
                   const int xres, const int layers,
                   bool clear_on_layout_change = true);
  void GraphicClear();
  void GraphicFill();
  ...
  void GraphicRender(int layer, int row, int col, RGBA fg, RGBA bg,
                     const char *txt, bool bold, int offset,
                     std::string layout);
  ...
  void DrawSpecialChar(const int row, const int col, const int height,
                       const int width, const int layer, const RGBA fg,
                       const RGBA bg, SpecialChar ch, std::string layout);
};
