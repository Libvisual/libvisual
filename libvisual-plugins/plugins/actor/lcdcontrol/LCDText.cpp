...
void LCDText::TextInit(int rows, int cols, int yres, int xres, int _goto,
                       int chars, int char0, int layers) {
  int n = rows * cols;
  GOTO_COST = _goto;
  CHARS = chars;
  CHAR0 = char0;
  ...
  for (int l = 0; l < layers; l++) {
    LayoutFB[l] = (unsigned char *)malloc(sizeof(char) * cols *
                                          rows); // new unsigned char[n];
    memset(LayoutFB[l], ' ', n);
    TransitionFB[l] = (unsigned char *)malloc(sizeof(char) * cols *
                                              rows); // new unsigned char[n];
...
void LCDText::TextSetSpecialChars() {
  for (int i = 0; i < (int)special_chars.size(); i++) {
    if (i > CHARS) {
      LCDError(
          "Too many chars to process. Expected %d, got %" VISUAL_SIZE_T_FORMAT
          ".",
          CHARS, special_chars.size());
      break;
    }
...
void LCDText::LayoutChangeAfter() { TextSetSpecialChars(); }

void LCDText::TextBlit(int row, int col, int height, int width) {
  ...
  for (lr = row; lr < LROWS && lr < row + height; lr++) {
    ...
    for (lc = col; lc < LCOLS && lc < col + width; lc++) {
      ...
      for (p1 = dc, p2 = p1, eq = 0, lc++; lc < LCOLS && lc < col + width;
           lc++) {
        /* transform layout to display column */
        dc = lc;
        /* sanity check */
...
void LCDText::TextClearChars() { special_chars.clear(); }
...
void LCD::TextGifDraw(WidgetGif *widget) {
    ...
    for(int row = 0; row < widget->GetRows() * lcdText->YRES; row++ ) {
        ...
        for(int col = 0; col < widget->GetCols() * lcdText->XRES; col++ ) {
            ...
            RGBA pxl = widget->GetBitmap()[row * widget->GetCols() *
lcdText->XRES + col]; if( !(pxl.R == widget->GetBackground().redQuantum() / 256
&& pxl.G == widget->GetBackground().greenQuantum() / 256 && pxl.B ==
widget->GetBackground().blueQuantum() / 256) && pxl.A == 255) { tmp[n][rr] =
tmp[n][rr] ^ 1<<(lcdText->XRES - 1 - cc);
            }
        }
    }
    ...
    for(int i = 0; i < (int)tmp.size() && i < (int)widget->GetChars().size();
i++ ){ if((!lcdText->IsTransitioning() || (lcdText->IsTransitioning() &&
widget->GetLayoutBase() == lcdText->GetTransitionLayout()))) {
...
void TextVisualizationPeakDraw(WidgetVisualization *widget) {
  ...
  for (int y = 0; y < height && row + y < lcdText->LROWS; y++) {
    int val = (int)(((double)widget->GetHistory()[y][0] / (double)SHRT_MAX) *
                    (width / 2 - 1));
...
void VLineFromBuffer(LCDText *lcd, unsigned char *dst, unsigned char *src,
                     int _x, int _y1, int _y2) {
  int x = _x;
  int y1 = _y1;
  int y2 = _y2;
...
void LCDText::TransitionTentacle() {
  ...
  for (int i = 0; i < visitor_->GetLCD()->LROWS; i++)
    if (TextRealBlit)
      TextRealBlit((LCDText *)visitor_->GetLCD(), i, 0, DisplayFB + i * LCOLS,
                   LCOLS);
