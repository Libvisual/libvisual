...
WidgetHistogram::WidgetHistogram(LCDCore *v, std::string n,
  ...
  expression_ =
      new Property(v, section, "expression", new Json::Value("return 0"));
...
void WidgetHistogram::SetupChars() {
  ...
  for (int c = 0; c < (int)lcd->YRES; c++) {
    ...
    for (int i = lcd->YRES - 1; i >= 0; i--) {
      buffer[lcd->YRES - i - 1] =
          (i < c ? pow(2.0, (double)lcd->XRES) - 1 - gap_ : 0);
    }
    ...
    if (lcd->CHARS < (int)lcd->special_chars.size()) {
      update_ = -1;
      LCDError("Widget %s - unable to allocate special chars", name_.c_str());
      return;
    }
