...
void PluginLCD::Transition(int i) { visitor_->Transition(i); }
...
string PluginLCD::AddWidget(string layout, int row, int col, int layer,
                            string object) {
  return string(visitor_->AddWidget(layout, row, col, layer, object).c_str());
}
...
void PluginLCD::SendData(int row, int col, int layer, string val) {
  /*
      ...
      if(type_ == LCD_TEXT) {
          ...
          lcd->GraphicRender(layer, row, col, fg_, bg_, val.c_str(), 0, 0,
     GetCurrentLayout().c_str()); lcd->GraphicBlit(0, 0, lcd->LROWS,
     lcd->LCOLS);
      }
  */
}
...
void PluginLCD::AddSpecialChar(SpecialChar matrix) {
  if (type_ == LCD_TEXT) {
    ...
    if (size < lcd->CHARS) {
      lcd->special_chars.push_back(SpecialChar(matrix.Size()));
...
int PluginLCD::GetXres() { return visitor_->GetLCD()->XRES; }
