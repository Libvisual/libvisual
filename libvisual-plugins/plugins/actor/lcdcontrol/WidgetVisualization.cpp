...
int VISUALIZATION_CHARS[6][9] = {
    {31, 31, 31, 31, 31, 31, 31, 31, -1}, {21, 14, 31, 14, 31, 14, 31, 10, -1},
    {0, 14, 4, 14, 4, 14, 4, 0, -1},      {0, 0, 4, 14, 14, 4, 0, 0, -1},
    {0, 0, 0, 4, 4, 0, 0, 0, -1},         {14, 14, 14, 14, 14, 14, 14, 14, -1}};

WidgetVisualization::WidgetVisualization(LCDCore *v, std::string n,
                                         Json::Value *section, int row, int col,
                                         int layer)
  ...
  if (val->asString() == "E")
    ...
    LCDError(
        "Widget %s has unknown direction '%s'; Use (E)ast or (W)est. Using E.",
  ...
  morph_timer_ = v->timers_->AddTimer(visualization_morph_update, this,
                                      morph_timeout_, true);
...
void WidgetVisualization::DoParams() {
  // params
  Json::Value *actorVal =
      visitor_->CFG_Fetch_Raw(section_, std::string("params.") + actor_plugin_);
  if (actorVal) {
...
void WidgetVisualization::Resize(int rows, int cols, int old_rows,
                                 int old_cols) {
...
void WidgetVisualization::SetupChars() {
          ...
          for(std::map<std::string, Widget *>::iterator ii=widgets1.begin();
              ...
              if(ii->second->GetWidgetBase() == widget_base_ &&
                  ...
                  for(int i = 0; i < size; i++ ) {
                      ch_[i] = ((WidgetVisualization
     *)widgets2[ii->first])->GetChars()[i];
                  }
                  has_chars_ = true;
                  return;
              }
  ...
  */
  ...
  if ((int)lcd->special_chars.size() >= lcd->CHARS - size) {
    update_ = -1;
    LCDError("Widget %s - unable to allocate special chars. CHARS: %d, "
             "chars_cache: %" VISUAL_SIZE_T_FORMAT ", size: %d",
  ...
  for (int c = 0; c < size; c++) {
    if (style_ != STYLE_PCM)
      lcd->special_chars.push_back(
          SpecialChar(VISUALIZATION_CHARS[c], lcd->YRES));
    else
...
void WidgetVisualization::UpdatePeak() {
  /*
      ...
      //int ret = xmmsc_visualization_chunk_get(client_.getConnection(), vis_,
     buffer, 0, 0);
