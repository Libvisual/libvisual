...
LCDGraphic::LCDGraphic(LCDCore *v)
    : FG_COL(0x00, 0x00, 0x00, 0xFF), BG_COL(0xFF, 0xFF, 0xFF, 0xFF),
      BL_COL(0xFF, 0xFF, 0xFF, 0x00), NO_COL(0x00, 0x00, 0x00, 0x00) {

  visitor_ = v;

  Json::Value *val =
      v->CFG_Fetch_Raw(v->CFG_Get_Root(), v->GetName() + ".foreground",
                       new Json::Value("ff000000"));

  if (color2RGBA(val->asCString(), &FG_COL) < 0) {
    LCDError("%s: ignoring illegal color '%s'", v->GetName().c_str(),
             val->asCString());
  }
  delete val;

  val = v->CFG_Fetch_Raw(v->CFG_Get_Root(), v->GetName() + ".background",
                         new Json::Value("ffffffff"));
  ...
  val = v->CFG_Fetch_Raw(v->CFG_Get_Root(), v->GetName() + ".basecolor",
                         new Json::Value("00000000"));
  ...
  val = v->CFG_Fetch(v->CFG_Get_Root(), v->GetName() + ".fill",
                     new Json::Value(0));
  fill_ = val->asInt();
  delete val;
  ...
  val = v->CFG_Fetch(v->CFG_Get_Root(), v->GetName() + ".inverted",
                     new Json::Value(0));
  INVERTED = val->asInt();
  delete val;
...
VisVideo *LCDGraphic::GetVideo(int layer) {
  VisVideo *video =
      visual_video_new_with_buffer(LCOLS, LROWS, VISUAL_VIDEO_DEPTH_32BIT);
...
void LCDGraphic::GraphicStop() { timer_->Stop(); }

void LCDGraphic::GraphicInit(const int rows, const int cols, const int yres,
                             const int xres, const int layers,
                             const bool clear_on_layout_change) {
...
void LCDGraphic::ResizeBefore(int rows, int cols) { is_resizing_ = true; }

void LCDGraphic::ResizeAfter() { is_resizing_ = false; }
...
void LCDGraphic::GraphicDraw() {
  while (visitor_->IsActive()) {
    if (update_window_.H == 0 || update_window_.W == 0 || is_resizing_ ||
        transitioning_) {
    ...
    // graphic_mutex_.lock();
    GraphicBlit(update_window_.R, update_window_.C, update_window_.H,
                update_window_.W);
...
void LCDGraphic::GraphicWindow(int pos, int size, int max, int *wpos,
                               int *wsize) {
  int p1 = pos;
  int p2 = pos + size;
...
void LCDGraphic::GraphicBlit(const int row, const int col, const int height,
                             const int width) {
  int r, c, h, w;
      ...
      for (int cc = c; cc < c + w; cc++) {
        for (int l = LAYERS - 1; l >= 0; l--) {
          if (LayoutFB[l][rr * LCOLS + cc] != NO_COL)
            DisplayFB[l][rr * LCOLS + cc] = LayoutFB[l][rr * LCOLS + cc];
        }
      }
    }
    if (GraphicRealBlit)
...
void LCDGraphic::GraphicBlitAll() { GraphicBlit(0, 0, LROWS, LCOLS); }

inline RGBA LCDGraphic::GraphicBlend(const int row, const int col,
                                     RGBA **buffer) {
  int l, o;
  RGBA p;
  RGBA ret;
...
void LCDGraphic::GraphicRender(const int layer, const int row, const int col,
                               const RGBA fg, const RGBA bg, const char *txt,
                               bool bold, int offset, std::string layout) {
  int c, r, x, y, len;
  ...
  if (layer < 0 || layer >= (int)LAYERS) {
    LCDError("%s: layer %d out of bounds (0..%d)", visitor_->GetName().c_str(),
             layer, LAYERS - 1);
    return;
  }
...
void LCDGraphic::DrawSpecialChar(const int row, const int col, const int height,
                                 const int width, const int layer,
                                 const RGBA fg, const RGBA bg, SpecialChar ch,
                                 std::string layout) {
...
void LCD::GraphicHistogramDraw(WidgetHistogram *w) {
  ...
  for (int c = 0; c < width / lcd->XRES; c++) {
    ...
    for (int y = 0; y < height && row + y < lcd->LROWS; y++) {
      for (int x = 0;
           x < lcd->XRES && x + col + c * lcd->XRES + offset < lcd->LCOLS;
           x++) {
...
void GraphicVisualizationPeakDraw(WidgetVisualization *widget) {
  ...
  for (int y = 0; y < height && row + y < lcd->LROWS; y++) {
    ...
    val = (int)(((double)widget->GetHistory()[y][1] / (double)SHRT_MAX) *
                (width / 2 - 1)) +
          width / 2;
    ...
    for (int i = 0; i < width; i++) {
      for (int yy = 0; yy < lcd->YRES && row + yy < lcd->LROWS; yy++) {
        for (int xx = 0; xx < lcd->XRES && col + xx < lcd->LCOLS; xx++) {
          ...
          case 4:
          case 5:
            if (VISUALIZATION_CHARS[(int)buffer[y * width + i]][yy] &
                (lcd->XRES - xx - 1))
              fb[n] = fg;
            else
              fb[n] = bg;
...
void GraphicVisualizationPCMDraw(WidgetVisualization *widget) {
  ...
  VisVideo *sub = visual_video_new_with_buffer(
      video->get_width(), video->get_height(), VISUAL_VIDEO_DEPTH_32BIT);
...
void GraphicVisualizationSpectrumDraw(WidgetVisualization *w) {
  ...
  // lcd->graphic_mutex_.lock();
  for (int x = 0; x < width && col + x < lcd->LCOLS; x += lcd->XRES) {
    int val = w->GetHistory()[0][x];

    for (int y = lcd->YRES; y <= height && row + y - lcd->YRES < lcd->LROWS;
         y += lcd->YRES) {
      if (val > (y * base))
        lcd->DrawSpecialChar(row + height - y, col + x, 8, 5, layer, fg, bg,
                             SpecialChar(VISUALIZATION_CHARS[0], 8),
                             w->GetLayoutBase());
      else if (val > ((y - 1) * base))
        lcd->DrawSpecialChar(row + height - y, col + x, 8, 5, layer, fg, bg,
                             SpecialChar(VISUALIZATION_CHARS[1], 8),
                             w->GetLayoutBase());
      else
        lcd->DrawSpecialChar(row + height - y, col + x, 8, 5, layer,
                             lcd->NO_COL, lcd->NO_COL, SpecialChar(8),
                             w->GetLayoutBase());
    }
  }
  // lcd->graphic_mutex_.unlock();
  ...
  if (!lcd->IsTransitioning())
    lcd->GraphicUpdate(row * lcd->YRES, col * lcd->XRES, height * lcd->YRES,
                       width * lcd->XRES);
}
...
void LCDGraphic::AlphaBlendBuffer(RGBA **dest, RGBA **src1, RGBA **src2,
                                  float alpha) {
