...
int GenericSerial::SerialOpen(std::string port, int baud, int flags) {
  ...
  if (fd_ == -1) {
    LCDError("%s: Open(%s) failed: %s", device_name_.c_str(), port.c_str(),
             strerror(errno));
    return -1;
  }

  if (tcgetattr(fd_, &portset) == -1) {
    LCDError("%s: tcgetattr(%s) failed: %s", device_name_.c_str(), port.c_str(),
             strerror(errno));
    return -1;
  }
  ...
  // cfmakeraw(&portset);
  // input modes
  portset.c_iflag &= ~(IGNBRK | BRKINT | PARMRK | INPCK | ISTRIP | INLCR |
                       IGNCR | ICRNL | IXON | IXOFF);
  portset.c_iflag |= IGNPAR;

  // output modes
  portset.c_oflag &= ~(OPOST | ONLCR | OCRNL | ONOCR | ONLRET | OFILL | OFDEL |
  ...
  if (tcsetattr(fd_, TCSANOW, &portset) == -1) {
    LCDError("%s: tcsetattr(%s) failed: %s", device_name_.c_str(), port.c_str(),
             strerror(errno));
    return -1;
  }
...
int GenericSerial::SerialPoll(unsigned char *string, int len) {
  ...
  if (ret < 0 && errno != EAGAIN) {
    LCDError("%s: read(%s) failed: %s", device_name_.c_str(), port_.c_str(),
             strerror(errno));
  }
  for (int i = 0; i < ret; i++) {
    string[i] = buff[i];
...
int GenericSerial::SerialWrite(unsigned char *string, int len) {
  ...
  for (run = 0; run < 10; run++) {
    ...
    if (ret >= 0 || errno != EAGAIN)
      break;
    if (run > 0)
      LCDInfo("%s: write(%s): EAGAIN #%d", device_name_.c_str(), port_.c_str(),
              run);
    usleep(1000);
  }
