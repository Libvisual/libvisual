...
static HASH_ITEM *hash_lookup(HASH *Hash, const char *key, const int do_sort) {
  ...
  if (Hash->sorted) {
    Item = (HASH_ITEM *)bsearch(key, Hash->Items, Hash->nItems,
                                sizeof(HASH_ITEM), hash_lookup_item);
  }
...
int hash_age(HASH *Hash, const char *key) {
  ...
  return (now.tv_sec - timestamp->tv_sec) * 1000 +
...
void hash_set_column(HASH *Hash, const int number, const char *column) {
  ...
  Hash->nColumns++;
  Hash->Columns = (HASH_COLUMN *)realloc(Hash->Columns,
                                         Hash->nColumns * sizeof(HASH_COLUMN));
...
static int hash_get_column(HASH *Hash, const char *key) {
  ...
  Column = (HASH_COLUMN *)bsearch(key, Hash->Columns, Hash->nColumns,
                                  sizeof(HASH_COLUMN), hash_lookup_column);
  if (Column == NULL)
    return -1;
...
static HASH_ITEM *hash_set(HASH *Hash, const char *key, const char *value,
                           const int delta) {
  HASH_ITEM *Item;
  HASH_SLOT *Slot;
  int size;
  ...
  if (Item == NULL) {
    ...
    Hash->nItems++;
    Hash->Items =
        (HASH_ITEM *)realloc(Hash->Items, Hash->nItems * sizeof(HASH_ITEM));
    ...
    if (Item->nSlot < delta) {
      Item->nSlot = delta;
      Item->Slot =
          (HASH_SLOT *)realloc(Item->Slot, Item->nSlot * sizeof(HASH_SLOT));
    }
  }
