...
Json::Value *CFG::CFG_Fetch_Raw(Json::Value *section, std::string key,
  ...
  if (pos > -1) {
    ...
    if (!subSection)
      return defval;
    Json::Value *root_ =
        CFG_Fetch_Raw(subSection, key.substr(pos + 1, key.length()), defval);
    delete subSection;
    return root_;
  }
  ...
  Json::Value *val =
      new Json::Value(section->get(key, defval ? *defval : Json::Value()));
...
Json::Value *CFG::CFG_Fetch(Json::Value *section, std::string key,
  ...
  Json::Value *val =
      CFG_Fetch_Raw(section, key, defval ? new Json::Value(*defval) : defval);
