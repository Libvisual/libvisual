...
AVSGfxColorCycler *avs_gfx_color_cycler_new(VisPalette *pal) {
  ...
  visual_object_initialize(VISUAL_OBJECT(cycler), TRUE,
                           avs_gfx_colorcycler_dtor);
...
int avs_gfx_color_cycler_set_mode(AVSGfxColorCycler *cycler,
...
VisColor *avs_gfx_color_cycler_run(AVSGfxColorCycler *cycler) {
  ...
  if (cycler->type == AVS_GFX_COLOR_CYCLER_TYPE_TIME) {
    ...
    usec_elapsed =
        ((double)elapsed.tv_sec) * VISUAL_USEC_PER_SEC + elapsed.tv_usec;
    usec_morph = ((double)cycler->morphtime.tv_sec) * VISUAL_USEC_PER_SEC +
                 cycler->morphtime.tv_usec;
...
/* Line functions */
int avs_gfx_line_non_naieve_floats(VisVideo *video, float x0, float y0,
                                   float x1, float y1, VisColor *col) {
  return avs_gfx_line_non_naieve_ints(video, video->width * x0,
                                      video->height * y0, video->width * x1,
                                      video->height * y1, col);
}

int avs_gfx_line_non_naieve_ints(VisVideo *video, int x0, int y0, int x1,
                                 int y1, VisColor *col) {
...
int avs_gfx_line_floats(VisVideo *video, float x0, float y0, float x1, float y1,
                        VisColor *col) {
  return avs_gfx_line_ints(video, video->width * x0, video->height * y0,
                           video->width * x1, video->height * y1, col);
}

int avs_gfx_line_ints(VisVideo *video, int x0, int y0, int x1, int y1,
                      VisColor *col) {
