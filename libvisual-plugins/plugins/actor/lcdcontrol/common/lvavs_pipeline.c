...
int pipeline_from_preset(LVAVSPipelineContainer *container,
                         LVAVSPresetContainer *presetcont);
int pipeline_container_realize(LVAVSPipelineContainer *container);
int pipeline_container_negotiate(LVAVSPipelineContainer *container,
                                 VisVideo *video);
int pipeline_container_propagate_event(LVAVSPipelineContainer *container,
                                       VisEvent *event);
int pipeline_container_run(LVAVSPipelineContainer *container, VisVideo *video,
                           VisAudio *audio);
...
static int lvavs_pipeline_element_dtor(VisObject *object) {
  ...
  default:
    visual_log(VISUAL_LOG_WARNING,
               "Tried to destroy invalid LVAVSPipelineElement type");
...
LVAVSPipelineElement *
lvavs_pipeline_element_new(LVAVSPipelineElementType type) {
  LVAVSPipelineElement *element;

  element = visual_mem_new0(LVAVSPipelineElement, 1);

  /* Do the VisObject initialization */
  visual_object_initialize(VISUAL_OBJECT(element), TRUE,
                           lvavs_pipeline_element_dtor);
...
LVAVSPipelineContainer *lvavs_pipeline_container_new() {
  ...
  /* Do the VisObject initialization */
  visual_object_initialize(VISUAL_OBJECT(container), TRUE,
                           lvavs_pipeline_container_dtor);

  LVAVS_PIPELINE_ELEMENT(container)->type =
...
int lvavs_pipeline_negotiate(LVAVSPipeline *pipeline, VisVideo *video) {
  pipeline_container_negotiate(LVAVS_PIPELINE_CONTAINER(pipeline->container),
                               video);
...
int lvavs_pipeline_run(LVAVSPipeline *pipeline, VisVideo *video,
  ...
  if (visual_audio_get_sample(audio, &tmp, VISUAL_AUDIO_CHANNEL_LEFT)) {
    VisBuffer *pcmbuf1 =
        visual_buffer_new_wrap_data(data[0][0], sizeof(float) * size, 0);
    visual_audio_sample_buffer_mix(pcmbuf1, tmp, TRUE, 1.0);
    visual_buffer_unref(pcmbuf1);
  }

  VisBuffer *spmbuf1 =
      visual_buffer_new_wrap_data(data[1][0], sizeof(float) * size, 0);
  ...
  if (visual_audio_get_sample(audio, &tmp, VISUAL_AUDIO_CHANNEL_LEFT)) {
    VisBuffer *pcmbuf2 =
        visual_buffer_new_wrap_data(data[0][1], sizeof(float) * size, 0);
    visual_audio_sample_buffer_mix(pcmbuf2, tmp, TRUE, 1.0);
    visual_buffer_unref(pcmbuf2);
  }

  VisBuffer *spmbuf2 =
      visual_buffer_new_wrap_data(data[1][1], sizeof(float) * size, 0);
...
#pragma omp parallel for private(i)
  ...
  #pragma omp parallel for private(i)
      ...
      pipeline->isBeat = visual_audio_is_beat_with_data(audio,
  VISUAL_BEAT_ALGORITHM_PEAK, visdata, BEAT_MAX_SIZE/2);
  */

  pipeline_container_run(LVAVS_PIPELINE_CONTAINER(pipeline->container), video,
                         audio);
...
int pipeline_from_preset(LVAVSPipelineContainer *container,
  ...
  while ((pelem = visual_list_next(presetcont->members, &le)) != NULL) {
    ...
    case LVAVS_PRESET_ELEMENT_TYPE_PLUGIN:
      ref =
          visual_plugin_find(visual_plugin_get_registry(), pelem->element_name);

      if (ref == NULL) {
        visual_log(VISUAL_LOG_CRITICAL, "Requested plugin %s not in registry",
                   pelem->element_name);
      ...
      if (ref->info == NULL) {
        visual_log(VISUAL_LOG_CRITICAL, "Could not get VisPluginInfo for %s",
                   pelem->element_name);
      ...
      if (strcmp(ref->info->type, VISUAL_PLUGIN_TYPE_ACTOR) == 0) {
        ...
        visual_object_set_private(VISUAL_OBJECT(element->data.actor->plugin),
                                  pipeline);
        ...
        visual_object_set_private(VISUAL_OBJECT(element->data.morph->plugin),
                                  pipeline);

      } else if (strcmp(ref->info->type, VISUAL_PLUGIN_TYPE_TRANSFORM) == 0) {

        element =
            lvavs_pipeline_element_new(LVAVS_PIPELINE_ELEMENT_TYPE_TRANSFORM);
        element->data.transform = visual_transform_new(pelem->element_name);
        visual_object_set_private(
            VISUAL_OBJECT(element->data.transform->plugin), pipeline);
    ...
    case LVAVS_PRESET_ELEMENT_TYPE_CONTAINER:
      ...
      LVAVS_PIPELINE_ELEMENT(cont)->params =
          LVAVS_PIPELINE_ELEMENT(container)->params;
      LVAVS_PIPELINE_ELEMENT(cont)->pipeline =
          LVAVS_PIPELINE_ELEMENT(container)->pipeline;
    ...
    default:
      visual_log(VISUAL_LOG_CRITICAL,
                 "Invalid LVAVSPresetElementType in LVAVSPresetElement");
...
int pipeline_container_realize(LVAVSPipelineContainer *container) {
  ...
  while ((element = visual_list_next(container->members, &le)) != NULL) {
    ...
    case LVAVS_PIPELINE_ELEMENT_TYPE_TRANSFORM:
      ...
      visual_param_container_copy_match(
          visual_plugin_get_params(
              visual_transform_get_plugin(element->data.transform)),
          element->params);
...
int pipeline_container_negotiate(LVAVSPipelineContainer *container,
  ...
  while ((element = visual_list_next(container->members, &le)) != NULL) {
    ...
    case LVAVS_PIPELINE_ELEMENT_TYPE_ACTOR:
      ...
      visual_actor_video_negotiate(element->data.actor, VISUAL_VIDEO_DEPTH_NONE,
                                   FALSE, FALSE);
...
int pipeline_container_propagate_event(LVAVSPipelineContainer *container,
  ...
  while ((element = visual_list_next(container->members, &le)) != NULL) {
    ...
    case LVAVS_PIPELINE_ELEMENT_TYPE_ACTOR:
      ...
      pluginqueue = visual_plugin_get_eventqueue(
          visual_actor_get_plugin(element->data.actor));
    ...
    case LVAVS_PIPELINE_ELEMENT_TYPE_CONTAINER:
      ...
      pipeline_container_propagate_event(LVAVS_PIPELINE_CONTAINER(element),
                                         event);
...
static void set_blendin(int v, int *mode) {
  *mode &= ~(31 << 8);
  *mode |= (v & 31) << 8;
}
static int blendout(int mode) { return ((mode >> 16) & 31) ^ 1; }
static void set_blendout(int v, int *mode) {
  *mode &= ~(31 << 16);
  *mode |= ((v ^ 1) & 31) << 16;
}

static int render_now(LVAVSPipelineContainer *container, VisVideo *video,
                      VisAudio *audio, int s) {
  LVAVSPipeline *pipeline = LVAVS_PIPELINE_ELEMENT(container)->pipeline;
...
int pipeline_container_run(LVAVSPipelineContainer *container, VisVideo *video,
  ...
  if (video->width != pipeline->dummy_vid->width ||
      video->height != pipeline->dummy_vid->height ||
      video->depth != pipeline->dummy_vid->depth) {
    ...
    pipeline->last_vid = visual_video_scale_depth_new(
        video, video->width, video->height, video->depth,
        VISUAL_VIDEO_COMPOSITE_TYPE_SRC);
    ...
    for (i = 0; i < 16; i++) {
      ...
      vid = visual_video_scale_depth_new(video, video->width, video->height,
                                         video->depth,
                                         VISUAL_VIDEO_COMPOSITE_TYPE_NONE);
  ...
  */
  ...
  if (!is_preinit) {
    ...
    case 9:
      ...
      break;
    case 10:
      mmx_adjblend_block(pipeline->blendtable, o, tfb, o, x,
                         pipeline->use_inblendval);
      break;
    case 11:
    ...
    case 12:
      /*
                                      {
                                              int
         *buf=(int*)getGlobalBuffer(w,h,bufferin,0); if (!buf) break; while
         (x--)
                                              {
                                                      *o=BLEND_ADJ(*tfb++,*o,
         depthof(*buf, ininvert)); o++; buf++;
                                              }
                                      }
      */
      break;
  ...
  if (!is_preinit) {
    if (s)
      visual_mem_copy(framebuffer, fbout, w * h * sizeof(int));
