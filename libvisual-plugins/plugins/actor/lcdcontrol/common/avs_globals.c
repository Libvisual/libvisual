...
/*
AvsGlobalBuffer *avs_get_global_buffer(AvsGlobalProxy *obj, int w, int h, int n,
int do_alloc)
{
    if(n < 0 || n >= MAXBUF)
        return NULL;
    ...
    if(!obj->buffers[n].buffer || obj->buffers[n].w != w || obj->buffers[n].h !=
h)
    {
...
AvsGlobalProxy *avs_global_proxy_new(int w, int h, int depth) {
  ...
  visual_object_initialize(VISUAL_OBJECT(proxy->multidelay), 0,
                           multidelay_dtor);
...
int avs_global_proxy_resize(AvsGlobalProxy *proxy, int w, int h, int depth) {
  ...
  proxy->framebuffer = visual_video_scale_depth_new(tmp, w, h, depth,
                                                    VISUAL_VIDEO_SCALE_NEAREST);
  visual_video_free_buffer(tmp);
  tmp = proxy->fbout;
  proxy->fbout = visual_video_scale_depth_new(tmp, w, h, depth,
                                              VISUAL_VIDEO_SCALE_NEAREST);
  visual_video_free_buffer(tmp);
  return 0;
}
