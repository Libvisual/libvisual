...
// *lvavs_preset_container_from_xml_node(LVAVSPresetContainer *cont, xmlNodePtr
// node);

/*static int preset_convert_from_wavs (LVAVSPresetContainer *presetcont,
AVSContainer *cont);
...
LVAVSPresetElement *wavs_convert_remap (AVSElement *avselem, const char
*plugname);
*/
  ...
  for (cur = cur->xmlChildrenNode; cur; cur = cur->next)
  {
      if (xmlIsBlankNode (cur) || cur->type != XML_ELEMENT_NODE)
        ...
        if(strcmp((char *)prop, "string") == 0) {
                ...
                visual_param_entry_set_integer(param, (int)strtol(content, NULL,
0)); } else if( strcmp((char *)prop, "color") == 0) { int r,g,b; char *s =
content+1; r = strtoul (s, &s, 0); if (r > 255 || ! (s = strchr (s, ',')))
                  continue;
                g = strtoul (s+1, &s, 0);
                if (g > 255 || ! (s = strchr (s, ',')))
                  continue;
...
LVAVSPreset *lvavs_preset_new_from_preset (char *filename)
{
        ...
        {
                xmlFreeDoc(doc);
                fprintf(stderr, "Bad opening node in preset file %s. Looking for
'pipeline_preset' but got '%s'.", filename, cur->name); return NULL;
        }
        ...
        for (cur = cur->children; cur; cur = cur->next)
        {
                if(xmlStrcmp(cur->name, (const xmlChar *)"container_main") == 0)
                {
                        xmlNodePtr child;
                        LVAVSPresetContainer *cont =
lvavs_preset_container_new(); LVAVS_PRESET_ELEMENT(cont)->pcont = pcont;
...
/*
LVAVSPresetContainer *lvavs_preset_container_from_xml_node(LVAVSPresetContainer
*cont, xmlNodePtr node) { xmlNodePtr child; LVAVSPresetElement *element; int i;
        for(child = node->children; child; child = child->next)
        {
                if (xmlIsBlankNode (child) || child->type != XML_ELEMENT_NODE)
                        continue;
                if(xmlStrcmp(child->name, (xmlChar *)"container_child") == 0) {
                        LVAVSPresetContainer *cont2 =
lvavs_preset_container_new(); VisParamContainer *pcont =
visual_param_container_new(); LVAVS_PRESET_ELEMENT(cont2)->pcont = pcont;
                        visual_param_container_add_many(pcont,
container_params); visual_list_add(cont->members, cont2);
                ...
                element =
lvavs_preset_element_new(LVAVS_PRESET_ELEMENT_TYPE_PLUGIN, (char*)child->name);
        ...
        while ((avselem = visual_list_next (cont->members, &le)) != NULL) {
                ...
                switch (avselem->type) {
                        case AVS_ELEMENT_TYPE_MAIN:
                                visual_list_add (presetcont->members,
wavs_convert_main_new (avselem));

                                break;

                        case AVS_ELEMENT_TYPE_RENDER_RING:
                                visual_list_add (presetcont->members,
wavs_convert_ring_new (avselem));

                                break;

                        case AVS_ELEMENT_TYPE_TRANS_CHANNELSHIFT:
                                visual_list_add (presetcont->members,
wavs_convert_channelshift_new (avselem));

                                break;

                        default:
                                if (avselem->type >=
AVS_ELEMENT_TYPE_RENDER_SIMPLESPECTRUM && avselem->type < AVS_ELEMENT_TYPE_LAST)
{

                                        visual_list_add (presetcont->members,
wavs_convert_remap (avselem, id_to_name_map[avselem->type]));
...
LVAVSPresetElement *wavs_convert_ring_new (AVSElement *avselem)
{
        ...
        sourceplace = visual_param_entry_get_integer (visual_param_container_get
(pcontw, "source and place"));

        visual_param_entry_set_integer (visual_param_container_get (pcont,
"place"), sourceplace >> 4); visual_param_entry_set_integer
(visual_param_container_get (pcont, "source"), (sourceplace & 0x0f) / 4);

        element = lvavs_preset_element_new (LVAVS_PRESET_ELEMENT_TYPE_PLUGIN,
"avs_ring"); element->pcont = pcont;
