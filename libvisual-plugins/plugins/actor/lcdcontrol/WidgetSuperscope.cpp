...
int lv_superscope_render(WidgetSuperscope *priv, VisVideo *video,
                         VisAudio *audio);
...
int scope_load_runnable(SuperScopePrivate *priv, ScopeRunnable runnable,
                        char *buf) {
  /*
...
WidgetSuperscope::WidgetSuperscope(LCDCore *v, std::string n,
  ...
  video_ =
      visual_video_new_with_buffer(width, height, VISUAL_VIDEO_DEPTH_32BIT);
...
int lv_superscope_render(SuperScopePrivate *priv, VisVideo *video,
                         VisAudio *audio) {
  ...
  if ((priv->channel_source & 3) >= 2) {
    for (x = 0; x < size; x++) {
      pcmbuf[x] = pipeline->audiodata[ws ^ 1][0][x] / 2 +
                  pipeline->audiodata[ws ^ 1][1][x] / 2;
    }
  } else {
    for (x = 0; x < size; x++) {
