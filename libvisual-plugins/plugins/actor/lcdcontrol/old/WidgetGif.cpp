...
WidgetGif::WidgetGif(LCDCore *v, std::string name, Json::Value *config, int row,
                     int col, int layer)
...
void WidgetGif::Update() {
  ...
  for (int row = ypoint_; row + ypoint_ < rows; row++) {
    for (int col = xpoint_; col + xpoint_ < cols; col++) {
      if (visible_->P2INT()) {
        Magick::Color pixel = framePtr_->pixelColor(col, row);
        // LCDError("pixel %d %d %d", pixel.redQuantum() / 256,
        // pixel.greenQuantum() / 256, pixel.blueQuantum() / 256);
  ...
  for (std::map<std::string, Widget *>::iterator ii = widgets1.begin();
    ...
    if (ii->second->GetWidgetBase() == widget_base_ &&
      ...
      for (int i = 0; i < rows_ * cols_; i++) {
        if (i >= (int)lcd->CHARS) {
          LCDError("1) GIF too large: %s, %d", name_.c_str(), lcd->CHARS);
          if (update_)
            delete update_;
