...
// LCDGraphic RealBlit
void DrvSDLBlit(LCDGraphic *lcd, const int row, const int col, const int height,
                const int width) {
  ((DrvSDL *)lcd)->DrvBlit(row, col, height, width);
}

// Constructor
DrvSDL::DrvSDL(std::string name, LCDControl *v, Json::Value *config, int layers)
  ...
  Json::Value *val =
      CFG_Fetch(config, name + ".cols", new Json::Value(SCREEN_W));
  cols_ = val->asInt();
  delete val;
  ...
  if (!(drvFB =
            (RGBA *)malloc(rows_ * pixels.y * cols_ * pixels.x * sizeof(RGBA))))
  ...
  surface_ =
      SDL_SetVideoMode(cols_ * pixels.x, rows_ * pixels.y, 32, SDL_RESIZABLE);

  SDL_WM_SetCaption("LCDControl", "LCDControl");

  if (surface_ == NULL) {
    LCDError("DrvSDL: Unable to set %dx%d video: %s", cols_, rows_,
             SDL_GetError());
  }
...
void DrvSDL::WriteGif() {
  ...
  int num = 0;
  for (std::list<Magick::Blob>::iterator it = image_.begin();
       it != image_.end(); it++) {
    Magick::Image image;
    image.magick("RGB");
    image.depth(32);
    ...
    Magick::PixelPacket *pixel_cache =
        image.getPixels(0, 0, cols_ * pixels.x, rows_ * pixels.y);
    for (int n = 0; n < rows_ * pixels.y * cols_ * pixels.x; n++) {
      ...
      *gif_pixel =
          Magick::ColorRGB(rgb[0] / 256.0, rgb[1] / 256.0, rgb[2] / 256.0);
    }
    image.syncPixels();
    gif.push_back(image);
    LCDInfo("Image %d", num++);
  ...
  for_each(gif.begin(), gif.end(),
           Magick::animationDelayImage(ani_speed_ / 10));
...
void DrvSDL::DrvUpdateGif() {
  ...
  image_.push_back(
      Magick::Blob(surface_->pixels, rows_ * pixels.y * surface_->pitch));
...
// Deinit driver
void DrvSDL::TakeDown() { Disconnect(); }

// Configuration setup
void DrvSDL::CFGSetup() { LCDCore::CFGSetup(); }
...
void DrvSDL::DrvBlit(const int row, const int col, const int height,
                     const int width) {
  mutex_.lock();
...
int DrvSDL::IsFullScreen() { return (surface_->flags & SDL_FULLSCREEN) > 0; }
...
void DrvSDL::Resize(const int rows, const int cols) {
  mutex_.lock();
  RGBA *tmp = (RGBA *)realloc(drvFB, rows * cols * sizeof(RGBA));
  if (!tmp) {
    LCDError("DrvSDL: Unable to reallocate buffer");
    return;
  }
  drvFB = tmp;
  emit static_cast<LCDEvents *>(visitor_->GetWrapper())
      ->_ResizeBefore(rows, cols);
  LockSDL();
