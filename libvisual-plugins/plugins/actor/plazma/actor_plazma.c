...
static void act_plazma_requisition(VisPluginData *plugin, int *width,
                                   int *height);
...
static void act_plazma_render(VisPluginData *plugin, VisVideo *video,
                              VisAudio *audio);

const VisPluginInfo *get_plugin_info(void) {
  static VisActorPlugin actor = {.requisition = act_plazma_requisition,
  ...
  static VisPluginInfo info = {
      ...
      .plugname = "plazma",
      .name = "Plazma plugin",
      .author = N_("Original by: Pascal Brochart <p.brochart@libertysurf.fr>, "
                   "Port by: Dennis Smit <ds@nerds-incorporated.org>"),
      .version = "0.0.1",
...
static int act_plazma_init(VisPluginData *plugin) {
  ...
  visual_param_list_add_many(
      params,
      visual_param_new_integer("bass_sensitivity", N_("Bass sensitivity"), 0,
                               visual_param_in_range_integer(0, INT_MAX)),
      visual_param_new_bool("plasma_effect", N_("Plasma effect"), TRUE, NULL),
      visual_param_new_bool("3d_effect_option", N_("3D effect option"), FALSE,
                            NULL),
      visual_param_new_bool("lines", N_("Lines"), TRUE, NULL),
      visual_param_new_bool("spectrum", N_("Spectrum"), TRUE, NULL),
      visual_param_new_bool("3d_effect", N_("3D effect"), TRUE, NULL),
      visual_param_new_float("rotation_speed", N_("Rotation speed"), 0.4, NULL),
      NULL);
...
static void act_plazma_render(VisPluginData *plugin, VisVideo *video,
  ...
  visual_audio_get_sample_mixed_simple(
      audio, pcmback, 2, VISUAL_AUDIO_CHANNEL_LEFT, VISUAL_AUDIO_CHANNEL_RIGHT);
