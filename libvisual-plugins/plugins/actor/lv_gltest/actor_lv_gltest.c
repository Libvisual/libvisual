...
static int xranges[] = {0,  1,  2,  3,  5,   7,   10,  14, 20,
                        28, 40, 54, 74, 101, 137, 187, 255};
...
static void lv_gltest_requisition(VisPluginData *plugin, int *width,
                                  int *height);
...
static void lv_gltest_render(VisPluginData *plugin, VisVideo *video,
                             VisAudio *audio);
static VisPalette *lv_gltest_palette(VisPluginData *plugin);

static void draw_bars(GLtestPrivate *priv);
static void draw_rectangle(GLtestPrivate *priv, GLfloat x1, GLfloat y1,
                           GLfloat z1, GLfloat x2, GLfloat y2, GLfloat z2);
static void draw_bar(GLtestPrivate *priv, GLfloat x_offset, GLfloat z_offset,
                     GLfloat height, GLfloat red, GLfloat green, GLfloat blue);
static void draw_bars(GLtestPrivate *priv);

/* Main plugin stuff */
const VisPluginInfo *get_plugin_info(void) {
  static VisActorPlugin actor = {.requisition = lv_gltest_requisition,
  ...
  static VisPluginInfo info = {
      ...
      .author = N_("Original by: Peter Alm, Mikael Alm, Olle Hallnas, Thomas "
                   "Nilsson and 4Front Technologies, Port by: Dennis Smit "
  ...
  VISUAL_VIDEO_ATTR_OPTIONS_GL_ENTRY(actor.vidoptions,
                                     VISUAL_GL_ATTRIBUTE_RED_SIZE, 5);
  VISUAL_VIDEO_ATTR_OPTIONS_GL_ENTRY(actor.vidoptions,
                                     VISUAL_GL_ATTRIBUTE_GREEN_SIZE, 5);
  VISUAL_VIDEO_ATTR_OPTIONS_GL_ENTRY(actor.vidoptions,
                                     VISUAL_GL_ATTRIBUTE_BLUE_SIZE, 5);
  VISUAL_VIDEO_ATTR_OPTIONS_GL_ENTRY(actor.vidoptions,
                                     VISUAL_GL_ATTRIBUTE_DEPTH_SIZE, 16);
  VISUAL_VIDEO_ATTR_OPTIONS_GL_ENTRY(actor.vidoptions,
                                     VISUAL_GL_ATTRIBUTE_DOUBLEBUFFER, 1);
  VISUAL_VIDEO_ATTR_OPTIONS_GL_ENTRY(actor.vidoptions, VISUAL_GL_ATTRIBUTE_RGBA,
                                     1);
...
static VisPalette *lv_gltest_palette(VisPluginData *plugin) { return NULL; }
...
static void lv_gltest_render(VisPluginData *plugin, VisVideo *video,
  ...
  const float seconds_elapsed =
      visual_timer_elapsed_msecs(priv->started_at) / 1000.0f;

  priv->x_angle =
      fmodf(priv->x_initial_angle + priv->x_speed * seconds_elapsed, 360.0);
  priv->y_angle =
      fmodf(priv->y_initial_angle + priv->y_speed * seconds_elapsed, 360.0);
  priv->z_angle =
      fmodf(priv->z_initial_angle + priv->z_speed * seconds_elapsed, 360.0);
...
static void draw_rectangle(GLtestPrivate *priv, GLfloat x1, GLfloat y1,
  ...
  if (y1 == y2) {
    ...
    const GLfloat vertices[] = {
        x1, y1, z1, x2, y1, z1, x2, y2, z2,

        x2, y2, z2, x1, y2, z2, x1, y1, z1,
...
static void draw_bar(GLtestPrivate *priv, GLfloat x_offset, GLfloat z_offset,
  ...
  draw_rectangle(priv, x_offset, height, z_offset, x_offset + width, height,
                 z_offset + 0.1);
  draw_rectangle(priv, x_offset, 0, z_offset, x_offset + width, 0,
                 z_offset + 0.1);

  glColor4f(0.5 * red, 0.5 * green, 0.5 * blue, 1.0f);
  draw_rectangle(priv, x_offset, 0.0, z_offset + 0.1, x_offset + width, height,
                 z_offset + 0.1);
  draw_rectangle(priv, x_offset, 0.0, z_offset, x_offset + width, height,
                 z_offset);

  glColor4f(0.25 * red, 0.25 * green, 0.25 * blue, 1.0f);
  draw_rectangle(priv, x_offset, 0.0, z_offset, x_offset, height,
                 z_offset + 0.1);
  draw_rectangle(priv, x_offset + width, 0.0, z_offset, x_offset + width,
                 height, z_offset + 0.1);
}
...
static void draw_bars(GLtestPrivate *priv) {
  ...
  for (y = 0; y < 16; y++) {
    ...
    for (x = 0; x < 16; x++) {
      x_offset = -1.6 + (x * 0.2);
      draw_bar(priv, x_offset, z_offset, priv->heights[y][x] * 0.2,
               r_base - (x * (r_base / 15.0)), x * (1.0 / 15), b_base);
    }
  }
