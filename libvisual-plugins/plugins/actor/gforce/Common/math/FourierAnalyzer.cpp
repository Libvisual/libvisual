...
void FourierAnalyzer::Transform(short inF[], long inN, long inNumBins,
                                float inBinRange, float outFT[]) {
  float *trig, samp, t;
  int i, j;
  ...
  if (inBinRange != mBinRange || mN != inN || inNumBins != mNumBins) {
    ...
    // Compute all the sin and cos values and store them in the order they'll be
    // needed
    trig = mTrigLookup;
    for (i = 0; i < inN; i++) {
      for (j = 1; j <= inNumBins; j++) {
        t = ((float)(j * i)) * inBinRange * 6.2831853071 / ((float)inN);
        *trig = cos(t) / ((float)inN);
        trig++;
        *trig = sin(t) / ((float)inN);
        trig++;
      }
    }
  }
  ...
  for (i = 0; i < inN; i++) {
    ...
    for (j = 0; j < inNumBins; j++) {
      outFT[j] += samp * (*trig);
      trig++; // *trig here is cos( j * 2pi * (i/inN) )
      mSinFT[j] += samp * (*trig);
      trig++; // *trig here is sin( j * 2pi * (i/inN) )
    }
  }

  // The actual freq component is the magnitude of the sin and cos freq
  // component vector
  for (j = 0; j < inNumBins; j++)
