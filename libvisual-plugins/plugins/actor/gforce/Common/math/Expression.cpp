...
//	Note:	I normally don't write such huge, non-compartmentalized code,
//but speed is important 	Post:	Allocates a register for the return value and
//returns that register
int Expression::Compile(const char *inStr, long inLen, ExpressionDict &inDict,
                        ExprVirtualMachine &inVM) {
  ...
  // Make a pass thru of the string, finding the starts and ends of all root
  // exprs
    ...
    if (parens == 0) {
      ...
      doOp:
        if (pos > startPos && pos + 1 < inLen) {
          ...
          // 4 exprs, given three priority operators, 4 exprs guaruntees we can
          // collapse two into one expr
          if (numExps == 4) {
            __collapseTwo
          }
        }
...
#undef VAL
  ...
  // If there more than one expression
    ...
    // When there'll be no more exprs, we're free to collapse all the exprs into
    // one
    while (numExps > 1) {
      __collapseTwo
    }
...
//	Pre:	Parens balence & all letters are capitalized
//	Note:	I normally don't write such huge, non-compartmentalized code,
but speed is important void Expression::Compile( char* inStr, long inLen, const
Hashtable& inDict, ExprVirtualMachine& inVM ) { bool
hasLetters = false; double*			dblPtr, val; long
highestPriority, priority, i, numExps = 0, startPos = 0; long
fcnCall, collapse, c, pos, parens = 0, firstParen = -1; short
oper[ 4 ]; short			reg[ 4 ];
        ...
        // Make a pass thru of the string, finding the starts and ends of all
root exprs for ( pos = 0; pos < inLen; pos++ ) { c = inStr[ pos ];
                ...
                if ( parens == 0 ) {
                        ...
                        switch ( c ) {
                                case '-':
                                case '+':	priority = 0x0100;	goto
doOp; case '/': case '%': case '*':	priority = 0x0200;	goto doOp; case
'^':	priority = 0x0300;

                                        // Close cur expr
doOp:				if ( pos > startPos && pos + 1 < inLen ) {

                                                // Push registers 0 thru
(numExps-1), the registers in use inVM.MassPush( numExps - 1 );

                                                // Recurse
                                                Compile( inStr + startPos, pos -
startPos, inDict, inVM ); oper[ numExps ] = c | priority; numExps++; startPos =
pos + 1;

                                                // The return value is in reg 0.
Also pop/restore the FP registers in use inVM.Move( numExps - 1 ); inVM.MassPop(
numExps - 2 );

                                                // 4 exprs, given three priority
operators, 4 exprs guaruntees we can collapse two into one expr if ( numExps ==
4 ) {
                                                        __collapseTwo
                                                }
                                        }
        ...
        if ( numExps == 0 ) {
                ...
                if ( firstParen >= 0 && firstParen <= 4 ) {
                        ...
                        // Eval what's inside the parens
                        Compile( inStr + firstParen + 1, inLen - firstParen - 2,
inDict, inVM );

                        // Translate the string of the fcn to a (one byte) code
number fcnCall = *((long*) inStr); switch ( fcnCall ) { #if EG_MAC ||
(defined(UNIX_X) && defined(WORDS_BIGENDIAN)) case 'SQRT':	fcnCall = cSQRT;
break; case 'ATAN':	fcnCall = cATAN;		break; case 'ABS(':
fcnCall = cABS;			break; case 'SIN(':	fcnCall = cSIN;
break; case 'COS(':	fcnCall = cCOS;			break; case 'TAN(':
fcnCall = cTAN;			break; case 'LOG(':	fcnCall = cLOG;
break; case 'EXP(':	fcnCall = cEXP;			break; case 'SQR(':
fcnCall = cSQR;			break; case 'SQWV':	fcnCall = cSQWV;
break; case 'POS(':	fcnCall = cPOS;			break; case 'RND(':
fcnCall = cRND;			break; #elif EG_WIN || (defined(UNIX_X) &&
!defined(WORDS_BIGENDIAN)) case 'TRQS':	fcnCall = cSQRT;		break;
                                case 'NATA':	fcnCall = cATAN;
break; case '(SBA':	fcnCall = cABS;			break; case '(NIS':
fcnCall = cSIN;			break; case '(SOC':	fcnCall = cCOS;
break; case '(NAT':	fcnCall = cTAN;			break; case '(GOL':
fcnCall = cLOG;			break; case '(PXE':	fcnCall = cEXP;
break; case '(RQS':	fcnCall = cSQR;			break; case 'VWQS':
fcnCall = cSQWV;		break; case '(SOP':	fcnCall = cPOS;
break; case '(DNR':	fcnCall = cRND;			break; #endif default:
fcnCall	= 0;			break;
                        }
                        inVM.MathOp( fcnCall );  }
                ...
                // At this point we assume it's an identifier, so we'll look up
its value else { UtilStr temp; temp.Assign( inStr, inLen ); if ( inDict.Get(
&temp, &dblPtr ) ) inVM.Loadi( dblPtr ); } }
        ...
        // If there more than one expression
                ...
                // Finish the current expr
                if ( startPos < inLen ) {
                        Compile( inStr + startPos, inLen - startPos, inDict,
inVM ); numExps++;
                }

                // The return value is in reg 0. Also pop/restore the FP
registers in use inVM.Move( numExps - 1 ); inVM.MassPop( numExps - 2 );

                // When there'll be no more exprs, we're free to collapse all
the exprs into one while ( numExps > 1 ) {
                        __collapseTwo
                }
...
bool Expression::IsDependent(const char *inStr) {
  ...
  while (pos > 0) {
    ...
    // This expr is depndent on inStr when we find a substring match that
    // *isn't* a substring of another identifier
...
bool Expression::Compile(const UtilStr &inStr, ExpressionDict &inDict) {
  ...
  if (parens != 0) {
    ...
    if (len > 0) {
      ...
      int retRegNum =
          Compile(mEquation.getCStr(), mEquation.length(), inDict, *this);
      Move(retRegNum, 0);
    } else {
