...
public:
  ...
  inline void set(PFloat inX, PFloat inY, PFloat inZ) {
    mX = inX;
    mY = inY;
    mZ = inZ;
  }

  inline void scale(PFloat inS, const V3 &inPt) {
    mX = inS * inPt.mX;
    mY = inS * inPt.mY;
    mZ = inS * inPt.mZ;
  }
  inline void scale(PFloat inS) {
    mX *= inS;
    mY *= inS;
    mZ *= inS;
  }
  ...
  inline PFloat dot(const V3 &inV) const {
    return mX * inV.mX + mY * inV.mY + mZ * inV.mZ;
  }

  void transform(const R3Matrix &inMatrix);
  void transform(const R3Matrix &inMatrix, const V3 &inPt);

  // Transforms the vector like the above two, but then applies a perspective
  // transformation on the x and y coords
  void transform(const R3Matrix &inMatrix, float inPersZ);

  inline void applyPerspective() {
    mX /= mZ;
    mY /= mZ;
  }

  inline void subtract(const V3 &inA) {
    mX -= inA.mX;
    mY -= inA.mY;
    mZ -= inA.mZ;
  }
  inline void subtract(const V3 &inA, const V3 &inB) {
    mX = inA.mX - inB.mX;
    mY = inA.mY - inB.mY;
    mZ = inA.mZ - inB.mZ;
  }

  inline void add(PFloat inX, PFloat inY, PFloat inZ) {
    mX += inX;
    mY += inY;
    mZ += inZ;
  }
  inline void add(const V3 &inA, const V3 &inB) {
    mX = inA.mX + inB.mX;
    mY = inA.mY + inB.mY;
    mZ = inA.mZ + inB.mZ;
  }
  inline void add(const V3 &inA) {
    mX += inA.mX;
    mY += inA.mY;
    mZ += inA.mZ;
  }
