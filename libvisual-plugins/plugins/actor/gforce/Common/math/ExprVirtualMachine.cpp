...
#define __addInst(opcode, data16)                                              \
...
void ExprVirtualMachine::Loadi(float inVal, int inReg) {
  ...
  __addInst(OP_LOADIMMED, inReg) mProgram.Append(&inVal, sizeof(float));
}

void ExprVirtualMachine::Loadi(float *inVal, int inReg) {

  __addInst(OP_LOAD, inReg) mProgram.Append(&inVal, sizeof(float *));
}

void ExprVirtualMachine::UserFcnOp(int inReg, ExprUserFcn **inFcn) {

  if (inFcn) {
    __addInst(OP_USER_FCN, inReg) mProgram.Append(&inFcn, sizeof(void *));
  } else
    Loadi(0.0, inReg);
}
...
float ExprVirtualMachine::Execute /*_Inline*/ () {
  float FR0, FR1, FR2, FR3, FR4, FR5, FR6,
  ...
  while (PC < end) {
    ...
    if (opcode == OP_LOADIMMED) {
      ...
      case OP_OPER:
        subop = (inst >> 16) & 0xFF;
        _fetch(r2, v2) _exeOp(v1, v2) break;
      ...
      case OP_WLINEAR:
      case OP_WEIGHT:
        _fetch(r2, v2) float temp = **((float **)PC);
        if (opcode == OP_WEIGHT) {
...
void ExprVirtualMachine::Chain(ExprVirtualMachine &inVM, float *inC1,
                               float *inC2) {
  ...
  // Use the special weight op that combines the two outputs via an embedded
  // addr to a 0 to 1 value Note that the output is moved to register 0
  if (inC2) {
        ...
        while ( PC < end ) {
                ...
                if ( opcode == OP_LOADIMMED || opcode == OP_LOAD ) {
                        ...
                        if ( opcode == OP_MOVE_FR0 ) {
                                reg = *PC & 0xFF;
// Extract the final dest register *start = (*start) | reg;
// Change the load so it loads right into the reg it needs to mProgram.Remove( 1
+ ( ((char*) PC) - base ), 4 ); }	// Delete the move from fr0 inst
                                // ??
                                ...
                                if ( opcode == OP_MOVE_FR0 ) {
                                        reg = *(PC + 1) & 0xFF;
// Extract the final dest register *start = (*start) | reg;
// Change the load so it loads right into the reg it needs to *PC = (*PC) | reg;
// Change the math op so it operates on the proper reg (see above)
                                        mProgram.Remove( 5 + ( ((char*) PC) -
base ), 4 );	// Delete the move from fr0 inst
                                }
                        }  // ??
                }
        }
...
long ExprVirtualMachine::StackReduction( long inStartPC, long inEndPC ) {
        ...
        while ( PC < inEndPC ) {
                ...
                reg &= 0xFF;					// Extract the
dest reg

                // We're only interested in root level pop/pushes (ie, when
fcnDepth == 0) switch ( opcode ) {

                        case OP_MASSPUSH:
                                if ( fcnDepth == 0 ) {
                                        pushLoc = PC;
                                        regsInUse = reg;	// We know
what's in use by what the compiler wanted us to push
                                }
                                fcnDepth++;
                                break;
                ...
                // see what regs are in use--skip over insts not at the root
level if ( fcnDepth == 0 ) { switch ( opcode ) {
                                ...
                                case OP_MASSPOP:
                                        ...
                                        // Get the regs that get sub-used (ie,
used between pushLoc and PC) progLen = mProgram.length(); subRegs =
StackReduction( pushLoc + 4, PC );

                                        // StackReduction() may have elminated
instructions, so adjust our PC PC		-= progLen - mProgram.length();
                                        inEndPC -= progLen - mProgram.length();

                                        // Reassign what regs get pushed then
popped.  We must push the regs that get used in the sub fcn and we use here
                                        regsToPush = subRegs & regsInUse;
                                        if ( regsToPush ) {
                                                inst = (long*) (base + pushLoc);
                                                *inst = OP_MASSPUSH |
regsToPush;		// Reassign the push inst = (long*) (base + PC); *inst =
OP_MASSPOP | regsToPush; }		// Reassign the pop

                                        // If no regs need to get pushed, delete
the pop and push insts else { mProgram.Remove( PC + 1, 4 ); mProgram.Remove(
pushLoc + 1, 4 ); PC		-= 8; inEndPC	-= 8;
                                        }
                                        break;
                        }
                }
