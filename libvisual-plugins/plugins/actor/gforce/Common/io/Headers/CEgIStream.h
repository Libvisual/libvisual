...
public:
  ...
  //	Post:	Reads next token (tokens separated by a space or a
  //CR,LF,TAB,SPACE). 	Note:	For Read( UtilStr& ), true is returned if a new
  //line was reached.
  float ReadFloat();
  long ReadInt();
  ...
  //	Post:	Reads <inBytes> worth of data from <inSource> and resets this
  //stream for that data (at pos = 0)
  //	Note:	These fcns are *only* designed as tools for using CEgIStream as
  //a memory (RAM) stream!
  void Assign(CEgIStream *inSource, long inBytes);
  void Assign(void *inSource, long inBytes);
  void Assign(const UtilStr &inSrce);
  //	Post:	Resets this stream to the beginning of its buffer.  This should
  //*only* in conjuction following 			a call to Assign().
  void ResetBuf();

  //	Pre:	<inSrce> *cannot*, for obvious reasons, be destroyed before this
  //is. 	Post:	All subsequent 'Get...', skip, and Readln fcns will all read
  //from <inSrce> 	Note:	Call this fcn each time you want to reset it.
  // 	Note:	See that the difference between Tie and Assign is that Assign
  // makes an independent copy of the data
...
protected:
  ...
  void fillBuf();
  void invalidateBuf();
  virtual void fillBlock(unsigned long inStartPos, void *destPtr,
                         long &ioBytes);
