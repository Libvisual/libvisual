...
void CEgOStream::PutByte(unsigned char inChar) { PutBlock(&inChar, 1); }
...
// Our readln() is set up so it doesn't matter if LFs are present or not.  Since
// 99% of the time, files will be edited on the platform they were made on, put
// LFs in the windows files.
#if EG_WIN
    PutByte(10);
#endif
  }
