...
CEgIStream::CEgIStream(unsigned short int inBufSize)
    : mIsTied(false), mReadBufSize(inBufSize), mBufPos(0), mPos(0) {
...
bool CEgIStream::Read(UtilStr &outStr) {
  ...
  while (noErr() && c != 13 && c != 9 && c != 32 &&
         c != 10) { // Stop on a CR, LF, TAB, space or error
    outStr.Append((char)c);
    c = GetByte();
  }
  return c == 13 || c == 10;
...
void CEgIStream::Read() {
  ...
  while (noErr() && c != 13 && c != 9 && c != 32 &&
         c != 10) // Stop on a CR, LF, space or error
    c = GetByte();
}
