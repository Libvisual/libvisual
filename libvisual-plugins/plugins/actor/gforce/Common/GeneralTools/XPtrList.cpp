...
void XPtrList::Assign(const XPtrList &inList) { UtilStr::Assign(inList); }
...
long XPtrList::FetchPredIndex(const void *inPtr) const {
  ...
  if (R < 0)
    ...
    // At this point, we know L is the critical element (case: L==R or L
    // contains M from case above)
...
bool XPtrList::RemoveElement(long inIndex) {
  ...
  if (inIndex > 0 && inIndex <= Count()) {
    ...
    if (mOrdering == cOrderNotImportant) {
      s = getCStr();
      *((void **)(s + inIndex * sizeof(void *))) =
          *((void **)(s + length() - sizeof(void *)));
      Trunc(sizeof(void *));
    } else
