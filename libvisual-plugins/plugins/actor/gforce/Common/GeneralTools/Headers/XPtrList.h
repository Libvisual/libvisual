...
// Post:	Returns a num >= 0 iff the first arg is less than or equal to the
// second arg.
//			Returns a num < 0 iff the first arg is greater than the
//second arg.
...
public:
  ...
  //	Post:	Adds the ptr to this' list after (existing) ptr number inN--make
  //inN 0 if you want your ptr to be the 1st element 	Order:	O( 1 )
  //	Note:	If inN is invalid, the closest element is used
  void Add(const void *inPtrToAdd, long inN);

  //	Post:	Adds each element of the incoming list to this list.  This is
  //functionally the same as 			looping thru the size of inPtrList and calling
  //this.Add() for each element.
  ...
  //	Note:	When order is set to be preserved (via cOrderImportant or a
  //specificed sort fcn), elements 			at the end of the list are removed faster
  ...
  //	Post:	Used to fetch any given ptr. If the index exists, <true> is
  //returned and the ptr is copied to *inPtrDest. 	Note:	One based indexing is
  //used 	Order:	O( 1 )
  void *Fetch(long inIndex) const;
  bool Fetch(long inIndex, void **ioPtrDest) const;
  inline bool FetchLast(void **ioPtrDest) const {
    return Fetch(Count(), ioPtrDest);
  }

  // 	Allows easy dynamic array usage.  Simple use any index and XPtrList will
  // expand to meet that size.
  ...
  //	Note:	Since caller has access to changes values, any current sorting
  //fcn is not used
  void *&operator[](long inIndex);

  //	Post:	Moves element at <inIndex> so that it has index 1.
  void MoveToHead(long inIndex);

  //	Post:	Searches for the given ptr in its ptr list and returns the index
  //of the match 			If the item cannot be found, 0 is returned
  ...
  //	Allows the compare fcn to be set.  Causes the current contents to be
  //removed.
...
protected:
  ...
  //	Post:	Returns the (one based) index of the predicessor of <inNum>. If
  //zero is returned, 			then <inNum> is <= than all the elements in the list.
