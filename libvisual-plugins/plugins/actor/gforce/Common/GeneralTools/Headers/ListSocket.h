...
public:
  ...
  //<inAboveCell> < 0 or > N, the closest boundry is used.
  ...
  //	Post:	Used to select cells.  DeleteSelected() and MoveSelected() use
  //cell selections.
  ...
  //	Note:	Returns true if <inCellNum> specifies a valid cell (and valid
  //info is sent back). 	Note:	Depths are such that 0 means root level, etc.
  virtual bool GetCell(long inCellNum, long &outIcnID, long &outDepth,
                       UtilStr &outLabel) = 0;

  //	Post:	Returns the number of cells in the list
  virtual long NumCells() = 0;

  //	Post:	Stub fcn for ListSocket.  Overridde if your table needs this
  //functionality. 	Note:	depth == 0 means root level.
  virtual void SetInsertionPt(long inCellNum, long inDepth);

  //	Post:	Universally manages a click on the <inCell>th cell, using the
  //current insertion 			point as the anchor if nessecary to select cells. 	Note:
  //It returns a list of cell numbers that need refreshing (ie, their select
  //state changed).
  virtual XLongList *ManageClick(long inCell, bool inShift, bool inCmd,
                                 bool inDeselectRest);

  //	Post:	Moves the selection location <inCellDelta> cells forward.  This
  //fcn is useful for handling 			arrow keys in a list for example. 	Note:	If there
  //is nothing selected, the first cell will be selected. 	Note:	It returns a
  //list of cell numbers that need refreshing (ie, their select state changed).
  XLongList *ArrowSelection(int inCellDelta);

  //	Post:	Returns the cell number associated with the user typing in
  //<inGUINum>.  In otherwords, this 			fcn maps the cell labeling to the cell
  //number.
  virtual long GUINumToCellNum(long inGUINum);

  //	Used to see if a given clipboard opertion can be performed (ie, if the
  //menu item can be enabled).
