...
public:
  XLongList(ListOrderingT inOrdering =
                cOrderNotImportant); // See XPtrList.h for ListOrderingT choices
  ...
  inline bool RemoveElement(long inIndex) {
    return mList.RemoveElement(inIndex);
  }
  inline void RemoveAll() { mList.RemoveAll(); }
  inline long Fetch(long inIndex) { return (long)mList.Fetch(inIndex); }
  inline bool Fetch(long inIndex, long *ioPtrDest) const {
    return mList.Fetch(inIndex, (void **)ioPtrDest);
  }
  inline long FindIndexOf(const long inMatch) const {
    return mList.FindIndexOf((void *)inMatch);
  }
  inline long Count() const { return mList.Count(); }

  void Randomize() { mList.Randomize(); }

  //	Post:	Any nums in this list that are in the interval [ inStart, inEnd
  //] are removed. 	Note:	Note how the interval is inclusive.
  void SubtractRange(long inStart, long inEnd);

  //	Post:	Any nums in this list that are not in the interval [ inStart,
  //inEnd ] are removed. 	Note:	Note how the interval is inclusive.
  void ApplyMask(long inStart, long inEnd);

  // 	Allows easy dynamic array usage.  Simply use any index and XLongList
  // will expand to meet that size.
  ...
  //	Post: outRank.Count() == inNumToRank  (ie, only inNumToRank values of
  //the ranking are returned) 	Note: If inNumToRank is invalid, the full ranking
  //is returned 	Note: O( N log N ) running time
