...
class Hashtable {
	...
	public:
		...
		//			the old value is returned, otherwise 0 is returned.
		...
		//	Note: If a matching key is found, true is returned.  If outValue is 0, the value is not returned.
		//	Note: O(1) running time
		...
		//	are less than n entries this this hashtable, 0 is returned.
		...
		//	Removes inKey from this Hashtable.  If inKey is not found, 0 is returned.
		...
		//	Note:	The compare fcn is arbitrary, but if it's it left out (ie. 0) then it sorts by value size, low to high.
		...
		void				Rank( XPtrList& outKeys, CompFunctionT inCompFcn = 0, unsigned long inNumToRank = -1 );
