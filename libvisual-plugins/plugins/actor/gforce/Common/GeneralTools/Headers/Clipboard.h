...
public:
  ...
  //	Post:	Adds a chuck of data to this clipboard, with a flavor tag
  //indicating the kind of data it is.
  void Append(UtilStr &inData, long inDataFlavor);
  void Append(void *inData, long inLen, long inDataFlavor);

  //	Post:	Loads the data at the <inIndex>th slot to <outData> and
  //<outDataFlavor>. 	Post:	<true> is returned iff the fetch was successful
  //	Note:	1 is the first item in the clipboard (ie, "one-indexing" is
  //used)
