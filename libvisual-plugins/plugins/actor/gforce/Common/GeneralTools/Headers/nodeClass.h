...
#define RegisterNode_(ClassName)                                               \
  TNodeRegistrar<ClassName>::RegisterNode(ClassName##T);

typedef nodeClass *(*CreatorFuncT)(nodeClass *inParent);

#define isPLItem(nodePtr) ((nodePtr->GetType() == 90) ? (CPLItem *)nodePtr : 0)
#define Cast(nodePtr, className)                                               \
  ((nodePtr->GetType() == className##T) ? (className *)nodePtr : 0)
...
public:
  nodeClass();
  //	Post:	Constructs a node and places the node at the tail of
  //<inParentPtr>
  ...
  //	Post:	This is how nodeClass "knows" what kind of nodeClass to
  //instantiate when it reads an ID from a stream.
  ...
  //	Post:	All sub nodes (at any depth) that are selected are deleted.  If
  //a node is selected, its contents are placed at the same 			level of this and
  ...
  //	Post:	This node and all sub nodes (and their sub nodes, etc) are
  //un/selected with <inSelected>.
  void SetTreeSelected(bool inSelected);

  //	Post:	Returns an associated unique or hash for the node.
  //	Note:	This is how XList knows what order to sort nodes, if that option
  //is enabled.
  virtual long GetKey() { return 0; }

  //	Post:	The node is detached from it's parent.  Obviously, the caller
  //now assumes ownership of the node.
  void detach();

  //	Post:	This node is inserted in <inPrevNode>'s parent list following
  //<inPrevNode> in the list order
  void insertAfter(nodeClass *inPrevNode);

  //	Post:	If <inAfterNode> <= 0, <inNodeToAdd> is added at this' head.
  //			If 0 < <inAfterNode> <= deepCount(), <inNodeToAdd> is added
  //after the <inAfterNode> item. 			If <inAfterNode> > deepCount(), <inNodeToAdd>
  //is added at this' tail.
  ...
  //	Post:	<inPtr> is detached from it's current parent list and is
  //inserted as the last node in this' sub list
  void addToTail(nodeClass *inPtr);

  //	Post:	<inPtr> is detached from it's current parent list and is
  //inserted at the 1st node in this' sub list
  void addToHead(nodeClass *inPtr);

  //	Post:	Returns the last deep sub node in the deep sub list.
  //			In other words, this fcn equals: this -> findSubNode( this ->
  //deepCount() )
  nodeClass *GetDeepTail() const;

  //	Post:	Returns the highest parent, say p, of this node where (p ->
  //GetDeepTail() == this) is true. 			In other words, this returns the highest
  //parent that has prev node. 	Note:	Returns 0 if no such parent exists.
  nodeClass *GetParentDeepTail(const nodeClass *inCeiling) const;

  //	Post:	this -> CountDepth( GetParentDeepTail( inCeiling ) );
  //			Or, in easy terms, CountOverhang returns the difference in depth
  //between itself 			and the parent node where CountOverhang is zero. 			Or, easier
  //yet,
  /*			1
                          |- 2				2 -> CountOverhang( 1 ) ==
     0 |  |-3              3 -> CountOverhang( 1 ) == 0 |  |-4
     4 -> CountOverhang( 1 ) == 1
                          |
                          |- 5				5 -> CountOverhang( 1 ) ==
     0 |  |-6				6 -> CountOverhang( 1 ) == 1 |    |-7
     7 -> CountOverhang( 1 ) == 2
                          |- 8				8 -> CountOverhang( 1 ) ==
     1				*/
  ...
  //	Post:	Mirrors a GUI drag of the selected items dropped before cell
  //<inAboveCell> at depth <inDepth> 	Note:	If <inDepth> is invalid or
  //<inAboveCell> < 0 or > N, the closest boundry is used. 	Calls:
  //VerifyNode()
  virtual void MoveSelected(long afterItemNum, long inDepth);

  //	Post:	Examines <nodeAdded> to see if it's in a valid location.  If
  //not, it may be moved or deleted.
  virtual void VerifyNode(nodeClass *nodeAdded);

  //	Post:	Delete's all sub nodes in this node
  virtual void deleteContents();

  //	Post:	Travels upward in its hierarchy until it finds a parent without
  //its own parent
  ...
  //	Post:	Returns <true> if this node contains other nodes  (optimized for
  //speed)
  inline bool IsEmpty() const { return mHead == 0; }

  //	Post:	Returns the number of nodes in this node's sub list (in contrast
  //to deepCount() )
  inline long shallowCount() const { return mShallowCount; }

  //	Post:	Returns the total number of nodes in this node's sub list PLUS
  //the number of items in *their* sub lists.
  long deepCount();

  //	Post:	Returns the number of super lists above this node. Ex, if this
  //node didn't have a parent, countDepth() 			would return 0.  If we then put it
  //inside a newly constructed node, countDepth() would return 1.
  long CountDepth(const nodeClass *inCeiling) const;

  //	Post:	Return what node number this node is in its parent's shallow
  //list.  Ex, if you added this node to the head of 			another node,
  //findInstance() would return a 1.  If this node is not in a sub list (ie, it
  //has no parent), 			a 0 is returned.
  long findInstance() const;

  //	Post:	Returns the <inNodeNum>th node in this node's (shallow) sub
  //list. A 0 is returned if <inNodeNum> 			is less than or equal to zero or if
  //greater than the number of items in this node's sub list.
  nodeClass *findNodeNum(long inNodeNum);

  //	Post:	Similar to findNodeNum( long ) except that the node returned is
  //the <inNodeNum>th node in the _entire_ 			_sub_ _tree_ (as opposed to the base
  //sub level).  If we had 2 sub nodes per node and we had 3 levels of 			these,
  //there would be a total of 2^3 leaves and a total of 2^4-1 nodes.
  //findSubNode( 8 ) would return the 			second node at the root level.  The
  //inverse of this function is findSubNode( nodeClass* ) and findNum(). 	Note:
  //If <inNodeNum> does not exist (ie, it's <= 0 or > deepCount()) then 0 is
  //returned.
  virtual nodeClass *findSubNode(long inNodeNum);

  //	Post:	This is the exact inverse function of findSubNode( long ). This
  //fcn returns the deep instance number of <inNodePtr> 			in this node deep sub
  //tree.  If the node was not found in this node's sub tree, 0 is returned.
  //	Note:	If <inNodePtr> is 0, 0 is returned.
  virtual long findSubNode(nodeClass *inNodePtr);

  //	Post:	All sub nodes in <sourceList> are appended in order inside this
  //node.  If <inPutAtHead> is true, the nodes 			in <sourceList> are put at the
  //head of this, and if it is false, the nodes are put at the tail of this.
  void absorbContents(nodeClass *sourceList, int inPutAtHead = true);

  //	Post:	All sub nodes in <sourceList> are moved to the tail of this if
  //they are marked
  void absorbMarked(nodeClass *sourceList);

  //	Post:	All sub nodes in <sourceList> are moved after this node (thus
  //placing them in this node's parent's sub list)
...
public:
  static T *CreateFromStream(nodeClass *inParent) { return new T(inParent); }
