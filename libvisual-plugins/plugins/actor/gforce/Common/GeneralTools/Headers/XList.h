...
public:
  ...
  //	Post:	This inserts <inNodeToAdd> within this node, after the current
  //insertion point (ie, at mInsertionPt and mInsertionDepth). 	Note:	0 for a
  //depth means the root level. 	Note:	The insertion pt is modified/maintained
  //to follow after <inNodeToAdd> was placed
  ...
  //	Post:	Same as nodeClass::findSubNode() except that caching brings
  //running time to O(1) (vs. O(n)) if 			the previous call was an adjacent node
  ...
  //	Post:	Returns the cell number that <inCellNum> is hierarcharaly inside
  //(similar to nodeClass's GetParent()). 	Note:	If the specified cell is at the
  //root level (ie it has no real visible parent) then 0 is returned.
  ...
  virtual bool CheckInsertPt(long &ioNodeNum, long &ioDepth) {
    return nodeClass::CheckInsertPt(ioNodeNum, ioDepth);
  }
  virtual void MoveSelected(long inAboveCell, long inDepth) {
    nodeClass::MoveSelected(inAboveCell, inDepth);
  }
