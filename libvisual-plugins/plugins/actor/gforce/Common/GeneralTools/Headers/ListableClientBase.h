...
public:
  ...
  //	Post:	This fcn should peform a generic export of its particular drag
  //data to <outClip>.
...
protected:
  ...
  //	Post:	Returns the number of pixels the given cell is out of view.  If
  //it's negative 			the cell lies above, if positive, it's below, and zero means
  ...
  //	Post:	Returns the cell range currently visible to the user (ie, the
  //range worth drawing).
  virtual void GetVisibleRect(long &outLeft, long &outTop, long &outRight,
                              long &outBottom) = 0;
