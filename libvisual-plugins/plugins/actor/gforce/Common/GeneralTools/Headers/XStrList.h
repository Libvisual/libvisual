...
public:
  XStrList(XStrListOptsT inOption,
           ListOrderingT inOrdering = cOrderNotImportant);
  virtual ~XStrList();

  // Returns what index the copy of the given string now occupies (1-based
  // indexing).  If 0 is returned, a copy of the string was not added because it
  // was a duplicate (if that flag is on)
  ...
  // Look for an item with a matching name.  If 0 is returned, no item was
  // found.
