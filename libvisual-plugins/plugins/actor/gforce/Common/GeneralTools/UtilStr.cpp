...
void UtilStr::Prepend(char inChar) { Insert(0, &inChar, 1); }
...
void UtilStr::Decapitalize() {
  ...
  for (i = 2; i <= len; i++) {
    ...
    if ((sp >= 'A' && sp <= 'Z') || (sp >= 'a' && sp <= 'z')) {
      if (getChar(i - 1) == 'I' && c == 'I') {
      } // Prevent III from decapitalizing
      else if (c >= 'A' && c <= 'Z')
        setChar(i, c + 32);
    }
  }
...
long UtilStr::Replace(const char *inTarget, const char *inReplacement,
...
void UtilStr::copyTo(unsigned char *pasDestPtr,
                     unsigned char inBytesToCopy) const {
...
int UtilStr::StrCmp(const char *s1, const char *s2, long inN,
                    bool inCaseSensitive) {
  char c1, c2;
...
int UtilStr::compareTo(const unsigned char *inPStr,
                       bool inCaseSensitive) const {
...
long UtilStr::contains(const char *inSrchStr, int inLen, int inStartingPos,
                       bool inCaseSensitive) const {
...
/*
*** The following is some string silmilarity/matching theory...  man, i miss
cornell cs...

Dynamic Programming Algorithm for Sequence Alignment.
Dynamic Programming Algorithms are used for finding shortest paths in graphs,
and in many other optimization problems, but in the comparison or alignment of
strings (as in Biological DNA, RNA and protein sequence analysis, speech
recognition and shape comparison) the following, or similar, is often called
...
M[0, 0] = z                                    -- usually z=0
M[i, 0] = f( M[i-1, 0  ], c(A[i], "_" ) )      -- Boundary conditions - delete
A[i] M[0, j] = f( M[0,   j-1], c("_",  B[j]) )      -- Boundary conditions -
delete B[j]
...
M[i,j] represents the cost (or score) of the partial sequences A[1..i] and
B[1..j], and the algorithm requires that M[i,j] can be calculated from the three
neighbours of M[i,j] - to the north (M[i,j-1]), west (M[i-1,j]), and north west
(M[i-1,j-1]).
...
So what are z, f(), g(), and c()?  By varing them, the returned score comparison
will follow different match criteria/behavior.  Here's 5 sets of assignments:
...
Note that an optimal LCS sequence alignment can be recovered either by retracing
the `max' choices that were made from M[|A|,|B|] to M[0,0], or by using
Hirschberg's (1975) divide and conquer technique.

2)  === Levenshtein Metric or Sellers' Edit Distance
A big edit distance value means the sequences are dissimilar - lots of changes
c(x,y), and indels c(x,"_") and c("_",x).

z = 0
g( ) = min( )
f( ) = +
c(x,x) = 0, c(x,y), c(x,"_"), c("_",x) > 0
The above assumes "simple" gap costs; linear but some other more complex gap
costs can be incorporated with modifications.

3)  === Probability of Alignments
For given probabilities, P(match), P(mismatch), P(insert) and P(delete), the
following dynamic programming algorithm finds a most probable alignment of two
given sequences:
...
c(x,"_") = P(delete) * P(x)
c("_",x) = P(insert) * P(x)
Unfortunately the quantities calculated become very small for strings of
realistic lengths and underflow is the consequence. It is more convenient to
deal with the -log's of probabilities and this also corresponds to a coding or
information theory interpretation - see below.
...
- assuming "simple" gap costs; modify if not. Base-two logs are taken if you
wish to interpret the quantities as bits; some prefer natural logarithms, giving
nits. An alignment can be thought of as a hypothesis of how strings A and B are
related, and it can be compared with the null-theory that they are not related
(Allison et al 1990(a), 1990(b), 1992). Thus, if an alignment gives some real
data compression for the pair of strings, it is an acceptable hypothesis.

5)   === Minimum Message Length R-Theory, Sum Over All Alignments
The r-theory (r for related) is the hypothesis that two strings are related in
some unspecified way. Its (-log2) probability is calculated as above except that

g(,) = logplus(,)
where logplus(-log2(P1), -log2(P2)) = -log2(P1+P2)
One alignment is just one hypothesis of how one string changed into the other.
There may be many optimal alignments and many more suboptimal alignments. Two
alignments are two different or exclusive hypotheses so their probabilities can
be added.

Thus the sum over all alignments gives the -log2 probability of the two strings
being created in a related but unspecified way. The complement of this r-theory
is that the strings are not related. Comparing the message lengths of the
r-theory and the null-theory gives the posterior -log odds-ratio that the two
strings are related (for a given model of string relation of course):
...
We can put a 50:50 prior on being related or unrelated. We do not know the prior
probability of the data, P(A&B), but we can cancel it out to get the posterior
-log-odds ratio of the r-theory and the null-theory.

It is fortunate that the sum of P(A&B|L) over all alignments L can be calculated
in O(|A|*|B|) time for "finite-state" models of relation (based on finite-state
machines) using the dynamic programming algorithm above and variations upon it
(Allison et al 1990(a), 1990(b), 1992).

References.
L. Allison, C. S. Wallace and C. N. Yee. When is a String like a String? AI &
Maths 1990(a) L. Allison, C. S. Wallace and C. N. Yee. Inductive inference over
macro-molecules. TR 90/148 Department of Computer Science, Monash University,
November 1990(b) L. Allison, C. S. Wallace and C. N. Yee. Finite-State Models in
the Alignment of Macromolecules. Jrnl. Molec. Evol. 35 77-89 1992 D. S.
Hirschberg. A Linear Space Algorithm for Computing Maximal Common Subsequences.
Comm. Assoc. Comp. Mach. 18(6) 341-343 1975 V. I. Levenshtein. Binary Codes
Capable of Correcting Deletions, Insertions and Reversals. Soviet Physics
Doklady 10(8) 707-710 1966 and Doklady Akademii Nauk SSSR 163(4) 845-848 1965 P.
Sellers. On the Theory and Computation of Evolutionary Distances. SIAM J. Appl.
Math 26(4) 787-793 1974 Lloyd Allison, Department of Computer Science, Monash
University, Australia 3168
*/

// An implementaion of the Longest Common Subsequece function set for the
// general Sequence Alignment alg described above
#define __MIN(a, b) (((a) < (b)) ? (a) : (b))
#define STACK_TEMP_SIZE 30
#define COST_CONTIGUOUS 1
#define COST_OF_DEL                                                            \
  1 /* Cost of having to delete chars from this to match inStr when preceding  \
       char matches */
#define COST_OF_INS                                                            \
  16 /* Cost of having to insert chars to this to match inStr.  higher cost    \
        than DEL to penalize chars contained in inStr but not in this */
#define COST_OF_CHANGE 17 /* Cose of changing a char in this to match inStr */
#define COST_OF_CASE                                                           \
  1 /* Cose of having to change the case of a char in this to match a char in  \
       inStr */
...
long UtilStr::LCSMatchScore(const char *inStr, long ALen) const {
  ...
  We need to make a 2D table, M[][], that has corner M[ |A| ][ |B| ].  If we
  evaluate M, row by row, we never need to access a row more further than one
  row.  This permits us to evaluate [ |A| ][ |B| ] with only having to allocate
  a single row's worth of elements (vs. allocating all |A| rows).  */
...
bool UtilStr::equalTo( const char* inStr, int inFlags ) const {
        ...
        char		c1 = 1, c2			= 2;
        bool		caseInsensitive 	= inFlags & cCaseInsensitive !=
0;
...
long UtilStr::GetIntValue(const char *inStr, unsigned long inLen,
                          long *outPlacePtr) {
  bool seenNum = false;
  char c;
  long ret = 0, place = 1;
...
double UtilStr::GetFloatValue() const { return GetFloatVal(mBuf + 1, mStrLen); }
  ...
  if (ptr) {
    ...
    while (inLen > 1) { // We stop 1 char early cuz last char should be a "
      ...
      if (c == '"') { // If the flag is detected...
        ...
        c = *ptr;
        if (inLen > 1 &&
            c != '"') { // Ignore double flags (they signify the flag char)
          ascNum.Wipe();
          while (c >= '0' && c <= '9') {
