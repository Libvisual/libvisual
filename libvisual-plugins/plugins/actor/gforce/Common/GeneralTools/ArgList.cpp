...
ArgList::ArgList() { mHeadArg = 0; }

ArgList::~ArgList() { Clear(); }
...
bool ArgList::GetArg(unsigned long inArgID, UtilStr &outStr) const {
  ...
  if (arg) {
    ...
    else // Allows calling for strings if you're not sure the Arg is a long of
         // not
...
void ArgList::SetArgs(const char *curPtr, long inLen) {
  ...
  do { // Loop thru each arg in the str
    ...
    // When we're not inside a string, igrore oddball/whitespace chars (chars <=
    // 32)
