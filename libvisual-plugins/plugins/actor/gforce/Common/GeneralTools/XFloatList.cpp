...
XFloatList::XFloatList(ListOrderingT inOrdering) : mList(inOrdering) {

  if (inOrdering == cSortLowToHigh || inOrdering == cSortHighToLow)
    mList.SetCompFcn(sFloatComparitor, inOrdering == cSortLowToHigh);
}

void XFloatList::FindMeans(long inNumMeans, float outMeans[],
                           float inSigmaScale) const {
  ...
  for (i = 1; i < n - 2; i++) {
    ...
    // If this a local max.  (Note: this could/should be improved for neighbors
    // that are equal)
  ...
  // The means are the averages of the initial (sorted) data, divided by the
  // serparating ranks
  start = 0;
...
void XFloatList::GaussSmooth(float inSigma, long inN, float inSrce[],
                             float inDest[]) {
  ...
  // We have to adjust the center weight so that when sigma falls below 1.0ish
  // so that the sample it takes at x = 0 doesn't represent the val of a
  // gaussian for -.5 to .5 (when sigma is small a gaussian gets very 'high' at
  // x=0 vs. x=+/- .5 when sigma is small
  sum = 0;
  ...
  for (x = maskCtr; x < xEnd; x++) {
    ...
    // For each pixel, apply the mask to its neighbors for a final value of that
    // pixel
    sum = 0;
...
void XFloatList::Rank(XLongList &outRank, long inNumToRank) const {
  ...
  // Duh... still sorted...
    ...
    // To rank, we must sort, with a tag on each element saying where it came
    // from
    p = temp;
    for (i = 1; i <= n; i++) {
      *((float *)p) = *srce;
