...
long Hashtable::sTableSizes[NUM_SIZES] = {
    23,     97,     397,     797,     3203,    6421,     12853,   51437,
    205759, 411527, 1646237, 3292489, 6584983, 13169977, 52679969};
...
void Hashtable::GetKeys(XPtrList &outKeys) {
  ...
  for (unsigned long i = 0; i < mTableSize; i++) {
    entry = *entryP;
    while (entry) {
      outKeys.Add((entry->mHashable) ? entry->mHashable
                                     : (const Hashable *)entry->mKey);
      entry = entry->mNext;
    }
...
void Hashtable::Rank(XPtrList &outKeys, CompFunctionT inCompFcn,
                     unsigned long inNumToRank) {
