...
void EgOSUtils::ShowFileErr(const UtilStr *inName, char *inErrMsg,
                            bool wasReading) {
  UtilStr s;
...
void EgOSUtils::ShowFileErr(const UtilStr *inName, CEgErr &inErr,
                            bool wasReading) {
  UtilStr msg;
...
void EgOSUtils::ShowFileErr(const CEgFileSpec &inFileSpec, CEgErr &inErr,
                            bool wasReading) {
...
bool EgOSUtils::AskSaveAs(const char *inPrompt, const char *inDefaultName,
                          CEgFileSpec &outSpec, long inFileType) {
...
bool EgOSUtils::AskSaveAs(const char *inPrompt, const UtilStr &inDefaultName,
                          CEgFileSpec &outSpec, long inFileType) {
  int doSave = false;
...
// Append the ext mask...
for ( int d = 0; d <= 24; d += 8 ) {			// Go thru each byte in
ID num c = ((inFileType << d) >> 24 ); winFilter.Append( (char) c );
}
winFilter.Append( (char) 0 );					// Windows
exptects an extra NUL to end the filter
...
bool EgOSUtils::AskOpen(const char *inPrompt, CEgFileSpec &outSpec,
                        long inTypeMask) {
  bool didOpen = false;
  UtilStr prompt(inPrompt);
...
// Append the ext mask...
for ( int d = 0; d <= 24; d += 8 ) {			// Go thru each byte in
ID num c = ((inTypeMask << d) >> 24 ); winFilter.Append( (char) c );
}
winFilter.Append( (char) 0 );					// Windows
exptects an extra NUL to end the filter
...
#ifdef EG_WIN
  int ans = ::MessageBox(0, inMsg.getCStr(), "Examgen Message",
...
int EgOSUtils::AskSaveChanges(const UtilStr &inName) {
  ...
  ans = ::MessageBox(0, msg.getCStr(), "Examgen Message",
...
void EgOSUtils::SpinCursor() {
  ...
  else if (time - CLOCKS_PER_SEC / 3 >
           sLastCursorChange) { // Every 1/3 second...
#ifdef EG_MAC
    Handle cursHndl;
...
bool EgOSUtils::GetNextFile(const CEgFileSpec &folderSpec, CEgFileSpec &outSpec,
  ...
  do {
    ...
    if (err == noErr && ((pb.ioFlFndrInfo.fdFlags & fInvisible) ==
                         0) /*&& pb.ioFlFndrInfo.fdCreator == cEGCreator*/) {
  ...
  do {
    ...
    if (ok) {
      ...
      isDir =
          ::GetFileAttributes(fileData.cFileName) & FILE_ATTRIBUTE_DIRECTORY;
      if (isDir == inFolders) {
...
#ifdef EG_WIN
  ::MessageBox(0, inMsg.getCStr(), "Examgen Message",
               MB_ICONEXCLAMATION | MB_OK | MB_SETFOREGROUND | MB_APPLMODAL);
// ZafMessageWindow* w = new ZafMessageWindow( "Message", ZAF_EXCLAMATION_ICON,
// ZAF_DIALOG_OK, ZAF_DIALOG_OK, inMsg.getCStr() ); zafWindowManager -> Add( w
// ); w -> Control();
#endif
}
...
unsigned long EgOSUtils::RevBytes(unsigned long inNum) {
  ...
  return (inNum << 24) | ((inNum & 0xFF00) << 8) | ((inNum & 0xFF0000) >> 8) |
         (inNum >> 24);
}
