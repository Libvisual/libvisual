...
#if EG_WIN
    HRGN rgn = ::CreateRectRgn(mClipRect.left, mClipRect.top,
                               mClipRect.right - 1, mClipRect.bottom - 1);
...
void PixPort::SetClipRect(int32_t inSX, int32_t inSY, int32_t inEX,
                          int32_t inEY) {
...
void PixPort::Init(int inWidth, int inHeight, int inDepth) {
  ...
  // Setup font data
  mWorld = mfl_CreateContext(mBits, mBytesPerPix * 8, mBytesPerRow, mX, mY);
#endif
...
#define __clipRect(inRect)                                                     \
  Rect r = inRect;                                                             \
  __clipPt(r.left, r.top) __clipPt(r.right, r.bottom) int32_t width =          \
      r.right - r.left;                                                        \
...
void PixPort::PreventActivate(GrafPtr inSysWindow) {
  ...
  // Below prevents MacOS from reindexing the colors in our GWorld during
  // CopyBits, thinking the colors in our GWorld index to the standard system
  // colors.  It's a hack, but then again, so is MacOS 8.x
  CTabHandle myTable = (**mBM).pmTable;
  if (inSysWindow) {
    (**myTable).ctSeed =
        (**(*((CGrafPort *)inSysWindow)->portPixMap)->pmTable).ctSeed;
  }
...
// In Win32, everything's upside down
  ...
  // Then o a box blur on the transposed image, effectively blurring the y
  // cords, transposing it to the dest
  if (mBytesPerPix == 2) {
    BoxBlur16((mBits + imgOffset), tempBits, inBoxWidth, width, height,
              mBytesPerRow, mBytesPerPix * height, boxTemp, mBackColor);
    BoxBlur16(tempBits, ((char *)inDestBits + imgOffset), inBoxWidth, height,
              width, mBytesPerPix * height, mBytesPerRow, boxTemp, mBackColor);
  } else if (mBytesPerPix == 4) {
    BoxBlur32((mBits + imgOffset), tempBits, inBoxWidth, width, height,
              mBytesPerRow, mBytesPerPix * height, boxTemp, mBackColor);
    BoxBlur32(tempBits, ((char *)inDestBits + imgOffset), inBoxWidth, height,
              width, mBytesPerPix * height, mBytesPerRow, boxTemp, mBackColor);
  }
}
...
void PixPort::CopyBits(unsigned char *inOutVideo, const Rect *inSrce,
                       const Rect *inDest) {
...
void PixPort::CopyBits(GrafPtr inPort, const Rect *inSrce, const Rect *inDest) {
  ...
  if (inSrce->left <= inSrce->right && inSrce->top <= inSrce->bottom &&
    ...
    ::BitBlt(inPort, inDest->left, inDest->top, inDest->right - inDest->left,
             inDest->bottom - inDest->top, mWorld, inSrce->left, inSrce->top,
             SRCCOPY);
...
void PixPort::CopyBits(PixPort &inDestPort, const Rect *inSrce,
                       const Rect *inDest) {
...
#if EG_MAC
    ::CopyBits((BitMap *)*mBM, (BitMap *)*inDestPort.mBM, inSrce, inDest,
               srcCopy, 0);
#elif EG_WIN
    ::BitBlt(inDestPort.mWorld, inDest->left, inDest->top,
             inDest->right - inDest->left, inDest->bottom - inDest->top, mWorld,
             inSrce->left, inSrce->top, SRCCOPY);
#endif
  }
}
...
void PixPort::Line(int sx, int sy, int ex, int ey, const RGBColor &inS,
                   const RGBColor &inE) {
  ...
  // If the endpoints have the same color, run the faster line procs (that just
  // use one color)
  if (dR > -CLR_LINE_THR && dR < CLR_LINE_THR && dG > -CLR_LINE_THR &&
      dG < CLR_LINE_THR && dB > -CLR_LINE_THR && dB < CLR_LINE_THR) {
    int32_t color;
...
void PixPort::AssignFont(long inPixFontID, const char *inFontName, long inSize,
                         long inStyleFlags) {
  ...
  font->mOSFontID = (long)::CreateFont(
      height, 0, 0, 0, (inStyleFlags & PP_BOLD) ? FW_BOLD : FW_NORMAL,
      (inStyleFlags & PP_ITALIC) ? true : false,
      (inStyleFlags & PP_UNDERLINE) ? true : false, 0, DEFAULT_CHARSET,
...
#if EG_WIN
  ::SetTextColor(mWorld,
                 RGB(inColor.red >> 8, inColor.green >> 8, inColor.blue >> 8));
#endif
...
#if EG_WIN
  ::SetTextColor(mWorld,
                 RGB(inColor.rgbRed, inColor.rgbGreen, inColor.rgbBlue));
#endif
...
void PixPort::TextRect(const char *inStr, int32_t &outWidth,
                       int32_t &outHeight) {
  int32_t width, pos;
  char c;
...
#define HALFCORD                                                               \
  0x007F /* 16 bits per cord, 8 bits for fixed decimal, 8 bits for whole       \
            number */
#define FIXED_BITS 8

// Assembly note w/ branch prediction:  the first block is chosen to be more
// probable

#include <stdio.h>
void PixPort::Fade(const char *inSrce, char *inDest, int32_t inBytesPerRow,
                   int32_t inX, int32_t inY, uint32_t *grad) {
  ...
  // Setup the source row base address and offset to allow for negative grad
  // components
  ...
  for (int y = 0; y < inY; y++) {
    ...
    for (int x = 0; x < inX; x++) {
      ...
      if (u1 != 0xFFFFFFFF) {
        ...
        // Note that we use casting 3 times as an unsigned char to (smartly) get
        // the compiler to do masking for us
        // FIXME: BUG: Assumption that bytes per row equals width.  Also see
        // _G-Force\ Common/DeltaField.*
        srceMap = srce + (u1 >> 14);
        v = (u1 >> 7) & 0x7F; // frac part of x
        u = (u1)&0x7F;        // frac part of y

        // In the end, the pixel intensity will be 31/32 of its current
        // (interpolated) value
        v *= 31;
        ...
        /* We can now calc the intensity of the pixel (truncating the fraction
         * part of the pix value)  */
        /* We divide by (7+7+5) decimal places because p is units squared (7
         * places per decimal) and 5 more dec places cuz of the mult by 31 */
