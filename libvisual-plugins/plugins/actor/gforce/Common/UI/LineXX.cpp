...
#if CLR_INTERP
#if P_SZ != 1
void PixPort::_Line(int sx, int sy, int ex, int ey, const RGBColor &inS,
                    int32_t dR, int32_t dG, int32_t dB) {
#else
...
void PixPort::_Line(int sx, int sy, int ex, int ey, int32_t color) {
  ...
  if (mLineWidth > 3) {
    ...
    dy = dy * dy;
    if (dx > 0 && dx >= dy)
      lw = 128 + 55 * dy / dx; // 1/cos( atan( x ) ) is about 1+.43*x^2 from 0
                               // to 1 (55 == .43 * 128)
    else if (dy > 0 && dy > dx)
      lw = 128 + 55 * dx / dy; // 1/cos( atan( x ) ) is about 1+.43*x^2 from 0
                               // to 1 (55 == .43 * 128)
  ...
  // Clipping: Set the pen loc to a point that's in and stop drawing once/if the
  // pen moves out
  if (sx < mClipRect.left + penExtents || sx >= mClipRect.right - penExtents ||
      sy < mClipRect.top + penExtents || sy >= mClipRect.bottom - penExtents) {

    // Exit if both points are out of bounds (wimpy clipping, eh?)
    if (ex < mClipRect.left + penExtents ||
        ex >= mClipRect.right - penExtents || ey < mClipRect.top + penExtents ||
        ey >= mClipRect.bottom - penExtents)
      return;
...
// In Win32, everything's upside down
  ...
  if (lw > 1) {
    ...
    if (tw < 12) {
      const char *c_shape;
      __circ(tw, c_shape) for (j = 0; j < tw; j++) {
        c_x = c_shape[j];
