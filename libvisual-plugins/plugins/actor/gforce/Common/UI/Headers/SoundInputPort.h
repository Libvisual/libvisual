...
typedef struct {
  ...
  long mNumSampleSkip;
  long myA5, devBuffer, soundRefNum;
  short /*whichBuffer,*/
      numChannels,
      sampleSize;
  long mBufEnd;
...
public:
  ...
  // How we turn the port on and off.  Call StartRecording() before you make
  // calls to GetSamples() or GetSpectrum()
  ...
  // Tells the sound input how many samples are thrown away per sample taken.
  // The default value is 0.
  void SetSampleSkip(long inNumSamples) {
    mParams->mNumSampleSkip = inNumSamples;
  }

  // Retrieves a sound clip from the sound in and performs a fourier transform
  // to get a freq spectrum. See FourierAnalyzer.h for a description of the
  // params. Returns false if the data was not returned
