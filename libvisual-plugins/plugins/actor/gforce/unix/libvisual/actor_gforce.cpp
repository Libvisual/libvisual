...
static void lv_gforce_requisition(VisPluginData *plugin, int *width,
                                  int *height);
...
static void lv_gforce_render(VisPluginData *plugin, VisVideo *video,
                             VisAudio *audio);
...
const VisPluginInfo *get_plugin_info() {
  ...
  info.author =
      "Winamp version: Andy O'Meara, Unix port: Boris Gjenero, Libvisual port "
      "and cleanups: Dennis Smit <ds@nerds-incorporated.org";
  info.version = "0.1.0";
  info.about = N_("Libvisual G-Force plugin");
  info.help = N_("This plugin is a port of the well known G-Force winamp "
                 "plugin, based on an old unix port");
...
void lv_gforce_render(VisPluginData *plugin, VisVideo *video, VisAudio *audio) {
  ...
  audio->get_sample_mixed_simple(pcmbuf, 2, VISUAL_AUDIO_CHANNEL_LEFT,
  ...
  priv->gGF->RecordSample(time, gSoundBuf, .000043, NUMSAMPLES, gFFTBuf, 1,
                          FFT_BUF_SIZE);
}
