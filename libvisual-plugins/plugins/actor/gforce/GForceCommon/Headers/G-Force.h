...
public:
  ...
  //	void					RecordSample( long inCurTime, float* inFourier,
  //long inNumBins );
  void RecordSample(long inCurTime, float *inSound, float inScale,
  ...
  /* NewSong() tells GForce a new track has started and that the following four
   * strings may contain info. */
  UtilStr mArtist;
  UtilStr mAlbum;
  UtilStr mSongTitle;
  void NewSong();
