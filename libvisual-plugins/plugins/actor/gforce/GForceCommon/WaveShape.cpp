...
void WaveShape::SetFFTFcn(ExprUserFcn **inFFTFcn) {
...
#define VAL2(a, b, c) (((a) << 16) + ((b) << 8) + (c))
  ...
  // Compile and link all the temp exprs.  By their spec, A vars can be
  // evaluated now
  ...
  mPen_Dep_S = mIntensity.IsDependent("s") || mIntensity.IsDependent("c") ||
               mIntensity.IsDependent("rnd");
  mLineWidth_Dep_S = mLineWidth.IsDependent("s") ||
                     mLineWidth.IsDependent("c") ||
                     mLineWidth.IsDependent("rnd");
  ...
  _assignOrig(mConnectBins) _assignOrig(mConnectFirstLast)
...
void WaveShape::SetSize( long inWidth, long inHeight ) {
        ...
        if ( mAspect1to1 ) {
                ...
                // Keep the xy aspect ratio to 1, change the dim that will get
stretched if ( mYScale < mXScale ) mXScale = mYScale; else mYScale = mXScale;
        }
}
*/
...
#define __evalIntensity(var)                                                   \
  clr = 65535.0 * mIntensity.Evaluate() * inFader;                             \
  var = clr;                                                                   \
  if (clr < 0)                                                                 \
    var = 0;                                                                   \
  else if (clr > 0xFFFF)                                                       \
...
void WaveShape::Draw(long inNumSteps, PixPort &inDest, float inFader,
                     WaveShape *inWave2, float inMorphPct) {
  long i, x, y;
  ...
  // Calc linewidth, add a little to make .999 into 1.  If it's not dep on s, we
  // can evaluate it now
  if (!mLineWidth_Dep_S)
  ...
  for (sS = 0; sS <= 1.0; sS += stepSize) {
    ...
    for (i = 0; i < maxWaves; i++) {
      ...
      if (i < mNumWaves) {
        ...
        if (i < w2Waves) {
          tx = mShapeTrans * tx +
               (1.0 - mShapeTrans) * xscaleW2 * inWave2->mWaveX.Evaluate(i);
          ty = mShapeTrans * ty +
               (1.0 - mShapeTrans) * yscaleW2 * inWave2->mWaveY.Evaluate(i);
        } else {
          tx *= dialate;
          ty *= dialate;
        }
  ...
  if (mConnectFirstLast) {
    for (i = 0; i < maxWaves; i++)
      inDest.Line(sXY[2 * i], sXY[2 * i + 1], sStartXY[2 * i],
                  sStartXY[2 * i + 1], rgb, rgbStart);
  }

  // Make sure we restore a random seed (one of the virtual machines could be
  // seeding to the same value)
  srand(*((long *)mTPtr));
}

#define __weightFLT(field)                                                     \
  field = (inW * ((float)field##Orig) + w1 * ((float)inDest->field));
#define __weightINT(field)                                                     \
  field = (0.5 + inW * ((float)field##Orig) + w1 * ((float)inDest->field));
#define __weightBOL(field)                                                     \
  field = .5 < (inW * (field##Orig ? 1.0 : 0.0) +                              \
                w1 * (inDest->field ? 1.0 : 0.0));

void WaveShape::SetupFrame(WaveShape *inDest, float inW) {
  float w1 = 1.0 - inW;

  __weightBOL(mConnectBins) __weightBOL(mConnectFirstLast)
}
