...
GForce::GForce(void *inRefCon)
    :
      ...
      mLineExpireTimes(cOrderImportant), mT(0), mPal1(mT, mIntensityParam),
      ...
      mParticles(cNoDuplicates_CaseInsensitive, cSortLowToHigh), mWave1(mT),
      mWave2(mT) {
  // Do initting...
  mWind = 0;
  mOutPort = 0;
  ...
  mConsoleExpireTime = mLastCursorUpdate = mLastGetKeys = mT_MS = 0;
  mFrameCountStart = mNextPaletteUpdate = 0;
  ...
  mShapeSlideShow = mColorSlideShow = mFieldSlideShow = true;
  mAtFullScreen = false;
...
void GForce::SetNumSampleBins(long inNumBins) {
  ...
  if (inNumBins > 0 && inNumBins < 10000) {
    mSampleFcn = (ExprUserFcn *)mSamplesBuf.Dim(sizeof(float) * inNumBins +
                                                sizeof(ExprUserFcn) + 32);
...
void GForce::SetNumFFTBins(long inNumBins) {
  ...
  if (inNumBins > 0 && inNumBins < 1000) {
    mFFTFcn = (ExprUserFcn *)mFFTBuf.Dim(sizeof(float) * inNumBins +
                                         sizeof(ExprUserFcn) + 32);
...
void GForce::ShowHelp() {
  ...
  s.Assign("X     - Display track title");
  __setChar(1, cDispTrackTitle);
  Println(&s);
  s.Assign("X     - List configs");
  __setChar(1, cGetConfigInfo);
  Println(&s);
  s.Assign("X     - Frame Rate");
  __setChar(1, cFrameRate);
  Println(&s);
  s.Assign("X     - Particles on/off");
  __setChar(1, cToggleParticles);
  Println(&s);
  s.Assign("X     - Spawn new particle");
  __setChar(1, cSpawnNewParticle);
  Println(&s);
  s.Assign("X     - Fullscreen on/off");
  __setChar(1, cToggleFullsceen);
  Println(&s);
  s.Assign("X     - Show config titles on/off");
  __setChar(1, cToggleConfigName);
  Println(&s);
  s.Assign("X     - Normalize input on/off");
  __setChar(1, cToggleNormalize);
  Println(&s);

  s.Assign("X X   - Freeze/Continue slideshow");
  __setChar(1, cStopSlideshowAll);
  __setChar(3, cStartSlideshowAll);
  Println(&s);
  s.Assign("X X   - +/- sound amplitude (See Extras docs)");
  __setChar(1, cDecMagScale);
  __setChar(3, cIncMagScale);
  Println(&s);
  s.Assign("X X   - +/- num of bins (Stps) (See Extras docs)");
  __setChar(1, cDecNumSSteps);
  __setChar(3, cIncNumSSteps);
  Println(&s);
  s.Assign("X X X - Prev/Next/Hold DeltaField");
  __setChar(1, cPrevDeltaField);
  __setChar(3, cNextDeltaField);
  __setChar(5, cToggleFieldShow);
  Println(&s);
  s.Assign("X X X - Prev/Next/Hold ColorMap");
  __setChar(1, cPrevColorMap);
  __setChar(3, cNextColorMap);
  __setChar(5, cToggleColorShow);
  Println(&s);
  s.Assign("X X X - Prev/Next/Hold WaveShape");
  __setChar(1, cPrevWaveShape);
  __setChar(3, cNextWaveShape);
  __setChar(5, cToggleShapeShow);
  Println(&s);

  Println("");
  Println(
      "Press SHIFT and a number to store the current ColorMap, Waveshape, and");
...
bool GForce::HandleKey(long inChar) {
  ...
  if (inChar == '/' || inChar == '?')
    ...
    case cStartSlideshowAll:
      ...
      mNextFieldChange = mT;
      mColorSlideShow = true;
      mNextColorChange = mT;
      mShapeSlideShow = true;
      mNextShapeChange = mT;
    ...
    case cNextWaveShape:
      ...
      loadWaveShape(mShapePlayList.Fetch(1 + n % mShapePlayList.Count()),
                    false);
    ...
    case cSetPreset0:
      StoreConfigState(VAL('S', 'E', 'T', '0'));
      break;
    case cSetPreset1:
      StoreConfigState(VAL('S', 'E', 'T', '1'));
      break;
    case cSetPreset2:
      StoreConfigState(VAL('S', 'E', 'T', '2'));
      break;
    case cSetPreset3:
      StoreConfigState(VAL('S', 'E', 'T', '3'));
      break;
    case cSetPreset4:
      StoreConfigState(VAL('S', 'E', 'T', '4'));
      break;
    case cSetPreset5:
      StoreConfigState(VAL('S', 'E', 'T', '5'));
      break;
    case cSetPreset6:
      StoreConfigState(VAL('S', 'E', 'T', '6'));
      break;
    case cSetPreset7:
      StoreConfigState(VAL('S', 'E', 'T', '7'));
      break;
    case cSetPreset8:
      StoreConfigState(VAL('S', 'E', 'T', '8'));
      break;
    case cSetPreset9:
      StoreConfigState(VAL('S', 'E', 'T', '9'));
      break;

    case cPreset0:
      handled = RestoreConfigState(VAL('S', 'E', 'T', '0'));
      break;
    case cPreset1:
      handled = RestoreConfigState(VAL('S', 'E', 'T', '1'));
      break;
    case cPreset2:
      handled = RestoreConfigState(VAL('S', 'E', 'T', '2'));
      break;
    case cPreset3:
      handled = RestoreConfigState(VAL('S', 'E', 'T', '3'));
      break;
    case cPreset4:
      handled = RestoreConfigState(VAL('S', 'E', 'T', '4'));
      break;
    case cPreset5:
      handled = RestoreConfigState(VAL('S', 'E', 'T', '5'));
      break;
    case cPreset6:
      handled = RestoreConfigState(VAL('S', 'E', 'T', '6'));
      break;
    case cPreset7:
      handled = RestoreConfigState(VAL('S', 'E', 'T', '7'));
      break;
    case cPreset8:
      handled = RestoreConfigState(VAL('S', 'E', 'T', '8'));
      break;
    case cPreset9:
      handled = RestoreConfigState(VAL('S', 'E', 'T', '9'));
      break;
...
void GForce::ManageColorChanges() {
  ...
  if (mT > mNextPaletteUpdate) {
    ...
    // If in a ColorMap transition/morph then we must set mColorTrans, for it's
    // linked into mGF_Palette
    if (mColorTransTime > 0) {
...
void GForce::ManageFieldChanges() {
  ...
  if (mT > mNextFieldChange && mNextField->IsCalculated() && mFieldSlideShow) {
    ...
    // loadGradField() will initiate computation on mField with a new grad
    // field...
...
void GForce::ManageParticleChanges() {
  ...
  if (mT > mNextParticleCheck && mParticlesOn) {
    ...
    rndVar =
        ((float)LV::rand()) / ((float)std::numeric_limits<uint32_t>::max());

    // Comparing that to the evalated probability of a new particle being
    // spawned determines if a new one *should* be spawned
...
void GForce::DrawParticles(PixPort &inPort) {
  ...
  while (particle) {
    ...
    if (!particle->IsExpired())
      ...
      // Update the var that holds how many particles are running (and is
      // accessible in the PPrb expr)
...
void GForce::RecordSample( long inCurTime, float* inFourier, long inNumBins ) {
        ...
        for ( s = 0; s < mNum_S_Steps; s++ ) {
                ...
                for ( w = 0; w < inNumBins; w++ ) {
                        n = ( 2.42322211 * w + 1.9 ) * ((float) s)
+ 1.23231121211 * w; sample += inFourier[ w ] * mSine[ n % mNum_S_Steps ];
                }
...
void GForce::IdleMonitor() {
  ...
  // Calc time till next kybd poll (Don't waste time checking the kybd unless
  // we've been idle a while)
  if (IsFullscreen())
    pollDelay = .6;

  // Don't bother rapildly checking the kybd until we're really close to going
...
void GForce::RecordSample(long inCurTime, float *inSound, float inScale,
                          long inNumBins, float *inFFT, float inFFTScale,
                          long inFFTNumBins) {
  float mag, sum;
  int i, n;
  ExprUserFcn *fcn;
...
void GForce::RecordSample(long inCurTime) {
  ...
  // Do the blur operation, a fcn of what's oqn the screen, and the current
  // delta field
  ...
  // If we're not currently drawing track text, check to see if we start new
  // text
  ...
  // If we already have a t.t. draw in progress, draw the text in the (full)
  // foreground color
  if (mTrackTextDur > 0) {
  ...
  // Draw the console text to the offscreen image.  Then copy the image to the
  // OS out port
  ...
  // We need to avoid text all bluring together so we overwrite the foreground
  // text we just drew
...
void GForce::DrawFrame() {
  ...
  __setupPort
      ...
      // If we're fullscreen, follow the API (the screen may need to do
      // something to finish)
      if (mScreen.IsFullscreen()) mOutPort = mScreen.BeginFrame();
  ...
  // If we're fullscreen, follow the API (the screen may need to do something to
  // finish)
  ...
  __loadFolder(DATADIR "/GForceDeltaFields", mDeltaFields, mFieldPlayList)
      ...
      __loadFolder(DATADIR "/GForceWaveShapes", mWaveShapes, mShapePlayList)
          ...
          __loadFolder(DATADIR "/GForceColorMaps", mColorMaps, mColorPlayList)
              ...
              __loadFolder(DATADIR "/GForceParticles", mParticles,
                           mParticlePlayList)
}
...
void GForce::loadColorMap(long inColorMapNum, bool inAllowMorph) {
  ...
  // If first time load, don't do any transition/morph, otherwise set up the
  // morph
  if (mGF_Palette == 0 || !inAllowMorph) {
    ...
    mColorTransTime =
        EgOSUtils::Rnd(mTransitionLo * 1000, mTransitionHi * 1000);
...
void GForce::SetPort(GrafPtr inPort, const Rect &inRect, bool inFullScreen) {
  ...
  // mDispRect is the rect within inPort G-Force is drawing in (ex, the
  // letterbox) Change the disp rect if the desired size exceeds the pixel
  // ceiling
  mDispRect = inRect;
...
void GForce::GetWinRect(Rect &outRect) { SetRect(&outRect, 0, 0, 0, 0); }
