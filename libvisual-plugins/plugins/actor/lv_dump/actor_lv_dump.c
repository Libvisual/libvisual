...
static void lv_dump_render(VisPluginData *plugin, VisVideo *video,
                           VisAudio *audio);
static VisPalette *lv_dump_palette(VisPluginData *plugin);

const VisPluginInfo *get_plugin_info(void) {
  static VisActorPlugin actor = {.requisition = lv_dump_requisition,
  ...
  static VisPluginInfo info = {
      ...
      .help = N_(
          "A plugin that simply dumps floating-point PCM data for debugging"),
...
static int lv_dump_init(VisPluginData *plugin) {
  ...
  visual_param_list_add_many(
      ...
      visual_param_new_integer("samples_per_frame", "Samples per frame",
                               SAMPLES_PER_FRAME_DEFAULT, NULL),
      NULL);
...
static void lv_dump_requisition(VisPluginData *plugin, int *width,
                                int *height) {
  LVDumpPrivate *priv = visual_plugin_get_private(plugin);
...
static VisPalette *lv_dump_palette(VisPluginData *plugin) { return NULL; }
...
static void lv_dump_render(VisPluginData *plugin, VisVideo *video,
  ...
  VisBuffer *pcm_buffer =
      visual_buffer_new_allocate(priv->samples_per_frame * sizeof(float));
