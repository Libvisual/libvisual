...
static void lv_scope_requisition(VisPluginData *plugin, int *width,
                                 int *height);
...
static void lv_scope_render(VisPluginData *plugin, VisVideo *video,
                            VisAudio *audio);
static VisPalette *lv_scope_palette(VisPluginData *plugin);

const VisPluginInfo *get_plugin_info(void) {
  static VisActorPlugin actor = {.requisition = lv_scope_requisition,
...
static void lv_scope_render(VisPluginData *plugin, VisVideo *video,
  ...
  for (i = 0; i < video_width; i++) {
    ...
    const int y_tip =
        y_origin + (pcmbuf[(i >> 1) % PCM_SIZE] * (max_displacement));
