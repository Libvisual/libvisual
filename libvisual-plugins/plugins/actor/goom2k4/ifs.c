 ...
 * 11-Apr-2002: jeko@ios-software.com: Make ifs.c system-indendant. (ifs.h
 * added) 01-Nov-2000: Allocation checks 10-May-1997: jwz@jwz.org: turned into a
 * standalone program. Made it render into an offscreen bitmap and then copy
...
#define MAXRAND                                                                \
  (2147483648.0 / 127.0) /* unsigned 1<<31 / 127.0 (cf goom_tools) as a float  \
                          */
...
static void free_ifs(FRACTAL *Fractal) { free_ifs_buffers(Fractal); }
...
static void init_ifs(PluginInfo *goomInfo, IfsData *data) {
  ...
  if ((Fractal->Buffer1 =
           (IFSPoint *)calloc(Fractal->Max_Pt, sizeof(IFSPoint))) == NULL) {
    free_ifs(Fractal);
    return;
  }
  if ((Fractal->Buffer2 =
...
static void ifs_update(PluginInfo *goomInfo, Pixel *data, Pixel *back,
                       int increment, IfsData *fx_data) {
...
#define MOD_MERVER 2
  ...
  couleur = (col[ALPHA] << (ALPHA * 8)) | (col[BLEU] << (BLEU * 8)) |
            (col[VERT] << (VERT * 8)) | (col[ROUGE] << (ROUGE * 8));
}

/** VISUAL_FX WRAPPER FOR IFS */

static void ifs_vfx_apply(VisualFX *_this, Pixel *src, Pixel *dest,
                          PluginInfo *goomInfo) {
  ...
  /*TODO: trouver meilleur soluce pour increment (mettre le code de gestion de
   * l'ifs dans ce fichier: ifs_vfx_apply) */
}
