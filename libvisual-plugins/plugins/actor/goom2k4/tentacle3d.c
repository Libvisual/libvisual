...
static void tentacle_update(PluginInfo *goomInfo, Pixel *buf, Pixel *back,
                            int W, int H, short[2][512], float, int drawit,
                            TentacleFXData *data);
...
static void tentacle_fx_apply(VisualFX *_this, Pixel *src, Pixel *dest,
                              PluginInfo *goomInfo) {
...
static void tentacle_new(TentacleFXData *data) {
  ...
  for (tmp = 0; tmp < nbgrid; tmp++) {
    ...
    center.z = z;
    data->grille[tmp] =
        grid3d_new(x, definitionx, z, definitionz + visual_rand() % 10, center);
    center.y += 8;
  }
}
...
static int evolutecolor(unsigned int src, unsigned int dest, unsigned int mask,
                        unsigned int incr) {
...
static void pretty_move(PluginInfo *goomInfo, float cycle, float *dist,
                        float *dist2, float *rotangle,
  ...
  if (fx_data->happens)
    ...
    fx_data->happens = goom_irand(goomInfo->gRandom, 200)
                           ? 0
                           : 100 + goom_irand(goomInfo->gRandom, 60);
  ...
  *dist = fx_data->distt = (tmp + 3.0f * fx_data->distt) / 4.0f;
  ...
  if (!fx_data->happens) {
    ...
    fx_data->rotation = goom_irand(goomInfo->gRandom, 500)
                            ? fx_data->rotation
...
static void tentacle_update(PluginInfo *goomInfo, Pixel *buf, Pixel *back,
                            int W, int H, short data[2][512], float rapport,
  ...
  if (fx_data->lig > 1.01f) {
    ...
    fx_data->col = evolutecolor(fx_data->col, fx_data->colors[fx_data->dstcol],
                                0xff, 0x01);
    fx_data->col = evolutecolor(fx_data->col, fx_data->colors[fx_data->dstcol],
                                0xff00, 0x0100);
    fx_data->col = evolutecolor(fx_data->col, fx_data->colors[fx_data->dstcol],
    ...
    for (tmp = 0; tmp < nbgrid; tmp++) {
      for (tmp2 = 0; tmp2 < definitionx; tmp2++) {
        float val = (float)(ShiftRight(
                        data[0][goom_irand(goomInfo->gRandom, 511)], 10)) *
                    rapport;
