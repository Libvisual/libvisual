...
static void convolve_free(VisualFX *_this) { free(_this->fx_data); }

static void create_output_with_brightness(VisualFX *_this, Pixel *src,
  ...
  if (visual_cpu_has_mmx()) {
    ...
    for (y = info->screen.height; y--;) {
      ...
      for (x = info->screen.width; x--;) {
        __asm__ __volatile__(
            ...
            "\n\t movd      %%mm0, %[dest]"
            : [dest] "=g"(dest[i].val)
            : [src] "g"(src[i].val), [ifftab] "g"(&ifftab[0])
            : "eax", "ecx");
    ...
    for (y = info->screen.height; y--;) {
      ...
      for (x = info->screen.width; x--;) {
        ...
        iff2 = ifftab[(int)data->conv_motif[(ytex >> 16) & CONV_MOTIF_WMASK]
                                           [(xtex >> 16) & CONV_MOTIF_WMASK]];
...
#define sat(a) ((a) > 0xFF ? 0xFF : (a))
        ...
        dest[i].val = (sat(f1) << R_OFFSET) | (sat(f2) << G_OFFSET) |
                      (sat(f3) << B_OFFSET);
        /*
...
static void convolve_apply(VisualFX *_this, Pixel *src, Pixel *dest,
                           PluginInfo *info) {
  ...
  ff = (FVAL(data->factor_p) * FVAL(data->factor_adj_p) + FVAL(data->light)) /
       100.0f;
...
VisualFX convolve_create(void) {
  VisualFX vfx = {.init = convolve_init,
                  .free = convolve_free,
