...
static void choose_a_goom_line(PluginInfo *goomInfo, float *param1,
                               float *param2, int *couleur, int *mode,
...
PluginInfo *goom_init(uint32_t resx, uint32_t resy) {
  ...
  goomInfo->gmline1 =
      goom_lines_init(goomInfo, resx, goomInfo->screen.height, GML_HLINE,
                      goomInfo->screen.height, GML_BLACK, GML_CIRCLE,
                      0.4f * (float)goomInfo->screen.height, GML_VERT);
  goomInfo->gmline2 = goom_lines_init(
      goomInfo, resx, goomInfo->screen.height, GML_HLINE, 0, GML_BLACK,
...
*/
uint32_t *goom_update(PluginInfo *goomInfo, int16_t data[2][512], int forceMode,
                      float fps, const char *songTitle, const char *message) {
  Pixel *return_val;
  uint32_t pointWidth;
  ...
  if (goomInfo->curGState->drawPoints) {
    for (i = 1; i * 15 <= goomInfo->sound.speedvar * 80.0f + 15; i++) {
      ...
      pointFilter(goomInfo, goomInfo->p1, YELLOW,
                  ((pointWidth - 6.0f) * largfactor + 5.0f),
                  ((pointHeight - 6.0f) * largfactor + 5.0f), i * 152.0f,
                  128.0f, goomInfo->update.loopvar + i * 2032);
      pointFilter(goomInfo, goomInfo->p1, ORANGE,
                  ((pointWidth / 2) * largfactor) / i + 10.0f * i,
                  ((pointHeight / 2) * largfactor) / i + 10.0f * i, 96.0f,
                  i * 80.0f, goomInfo->update.loopvar / i);
      ...
      pointFilter(goomInfo, goomInfo->p1, BLACK,
                  ...
                  ((pointHeight / 3) * largfactor + 20.0f), 58.0f, i * 66.0f,
                  goomInfo->update.loopvar / i);
      pointFilter(goomInfo, goomInfo->p1, WHITE,
                  (pointHeight * largfactor + 10.0f * i) / i,
                  (pointHeight * largfactor + 10.0f * i) / i, 66.0f, 74.0f,
                  goomInfo->update.loopvar + i * 500);
    }
  }
  ...
  if (goomInfo->update.lockvar == 0) {
    ...
    if (goomInfo->sound.timeSinceLastGoom == 0) {
      ...
      for (i = 0; i < goomInfo->statesNumber; i++)
        if ((goomInfo->update.stateSelectionRnd >=
             goomInfo->states[i].rangemin) &&
            (goomInfo->update.stateSelectionRnd <=
      ...
      /* if (goomInfo->update.goomvar % 1 == 0) */
      {
        ...
        if ((goomInfo->update.zoomFilterData.reverse) &&
            (!(goomInfo->cycle % 13)) && (visual_rand() % 5 == 0)) {
          goomInfo->update.zoomFilterData.reverse = 0;
        ...
        if ((signed int)newvit <
            goomInfo->update.zoomFilterData.vitesse) /* on accelere */
        {
  ...
  if (pzfd != NULL) {
    ...
    dif = goomInfo->update.zoomFilterData.vitesse -
  ...
  if (goomInfo->update.lineMode != goomInfo->update.drawLinesDuration) {
    ...
    if (goomInfo->update.lineMode == -1)
      goomInfo->update.lineMode = 0;
             ...
             (goom_irand(goomInfo->gRandom, 5) == 0) &&
             goomInfo->update.lineMode)
    goomInfo->update.lineMode--;
  ...
  if ((goomInfo->cycle % 120 == 0) && (goom_irand(goomInfo->gRandom, 4) == 0) &&
    ...
    if (goomInfo->update.lineMode == 0)
      ...
      goomInfo->update.lineMode--;
      choose_a_goom_line(goomInfo, &param1, &param2, &couleur1, &mode,
                         &amplitude, goomInfo->update.stop_lines);
...
/* *** */
void choose_a_goom_line(PluginInfo *goomInfo, float *param1, float *param2,
...
void update_message(PluginInfo *goomInfo, const char *message) {
  ...
  if (goomInfo->update_message.affiche) {
    ...
    while (!fin) {
      ...
      goom_draw_text(goomInfo->p1, goomInfo->screen.width,
                     goomInfo->screen.height, goomInfo->screen.width / 2, pos,
                     message, ecart, 1);
      message = ++ptr;
      i++;
    }
