...
static void lv_goom_render(VisPluginData *plugin, VisVideo *video,
                           VisAudio *audio);
static VisPalette *lv_goom_palette(VisPluginData *plugin);

const VisPluginInfo *get_plugin_info(void) {
  static VisActorPlugin actor = {.requisition = lv_goom_requisition,
  ...
  static VisPluginInfo info = {
      ...
      .author = "Dennis Smit <ds@nerds-incorporated.org>, goom2k4 by: "
                "Jean-Christophe Hoelt <jeko@ios-software.com>",
      .version = "0.1",
      .about = N_("Libvisual goom2k4 plugin"),
      .help = N_("This plugin adds support for the supercool goom2k4 plugin "
...
static void lv_goom_requisition(VisPluginData *plugin, int *width,
                                int *height) {
...
static VisPalette *lv_goom_palette(VisPluginData *plugin) { return NULL; }
...
static void lv_goom_render(VisPluginData *plugin, VisVideo *video,
  ...
  VisSongInfo *songinfo =
      ((VisActorPlugin *)visual_plugin_get_specific(plugin))->songinfo;

  uint32_t *buf;

  /* FIXME goom should support setting a pointer, so we don't need that final
   * visual_mem_copy */
  if (songinfo != NULL && visual_songinfo_get_age(songinfo) <= 1 &&
      showinfo == TRUE) {
    VisSongInfoType songinfo_type = visual_songinfo_get_type(songinfo);

    if (songinfo_type == VISUAL_SONGINFO_TYPE_SIMPLE)
      buf = goom_update(priv->goominfo, pcmdata, 0, 0,
                        visual_songinfo_get_simple_name(songinfo), NULL);
    else if (songinfo_type == VISUAL_SONGINFO_TYPE_ADVANCED)
      buf = goom_update(priv->goominfo, pcmdata, 0, 0,
                        visual_songinfo_get_song(songinfo), NULL);
    else
  ...
  visual_mem_copy_pitch(
      vidbuf, buf, visual_video_get_pitch(video), visual_video_get_pitch(video),
      visual_video_get_pitch(video), visual_video_get_height(video));
}
