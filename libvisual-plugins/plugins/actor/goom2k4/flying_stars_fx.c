...
*/
static const int colval[] = {0x1416181a, 0x1419181a, 0x141f181a, 0x1426181a,
                             0x142a181a, 0x142f181a, 0x1436181a, 0x142f1819,
                             0x14261615, 0x13201411, 0x111a100a, 0x0c180508,
                             0x08100304, 0x00050101, 0x0};
...
static void fs_free(VisualFX *_this) { free(_this->fx_data); }

/**
 * Cree une nouvelle 'bombe', c'est a dire une particule appartenant a une fusee
 * d'artifice.
 */
static void addABomb(FSData *fs, int mx, int my, float radius, float vage,
                     float gravity, PluginInfo *info) {
...
static void fs_sound_event_occured(VisualFX *_this, PluginInfo *info) {
  ...
  int max =
      (int)((1.0f + info->sound.goomPower) * goom_irand(info->gRandom, 150)) +
      100;
  float radius = (1.0f + info->sound.goomPower) *
                 (float)(goom_irand(info->gRandom, 150) + 50) / 300;
  int mx;
  int my;
  ...
  case RAIN_FX:
    ...
    my = -(info->screen.height / 3) -
         goom_irand(info->gRandom, info->screen.width / 3);
    radius *= 1.5;
    vage = 0.002f;
    break;
  case FOUNTAIN_FX:
  ...
  radius *= info->screen.height /
            200.0f; /* why 200 ? because the FX was developped on 320x200 */
 ...
 */
static void fs_apply(VisualFX *_this, Pixel *src, Pixel *dest,
                     PluginInfo *info) {
...
VisualFX flying_star_create(void) {
  VisualFX vfx = {
      .init = fs_init, .free = fs_free, .apply = fs_apply, .fx_data = 0};
  return vfx;
}
