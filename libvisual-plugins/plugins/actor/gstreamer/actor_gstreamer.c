...
static void act_gstreamer_requisition(VisPluginData *plugin, int *width,
                                      int *height);
...
static void act_gstreamer_render(VisPluginData *plugin, VisVideo *video,
                                 VisAudio *audio);

static void handle_sink_handoff(GstElement *sink, GstBuffer *buffer,
                                GstPad *pad, GStreamerPrivate *data);
static void handle_bus_error_message(GstBus *bus, GstMessage *message,
                                     GStreamerPrivate *priv);
static void handle_bus_eos_message(GstBus *bus, GstMessage *message,
                                   GStreamerPrivate *priv);

const VisPluginInfo *get_plugin_info(void) {
  static VisActorPlugin actor = {.requisition = act_gstreamer_requisition,
  ...
  static VisPluginInfo info = {
      ...
      .help = N_("Capable of using gstreamer to play video, or do effects "
...
static int act_gstreamer_init(VisPluginData *plugin) {
  ...
  // - Element "capsfilter" limits the output to the width/height/format we
  // want.
  ...
  char *launch_str =
      g_strdup_printf("filesrc location=%s ! decodebin ! videoconvert ! "
  ...
  if (!priv->pipeline) {
    visual_log(VISUAL_LOG_ERROR, "Failed to create pipeline: %s",
               error->message);
    g_error_free(error);
    g_free(launch_str);
    return FALSE;
  ...
  GstCaps *caps = gst_caps_new_simple("video/x-raw", "width", G_TYPE_INT, 320,
                                      "height", G_TYPE_INT, 240, "format",
                                      G_TYPE_STRING, "BGR", NULL);
  ...
  g_signal_connect(priv->sink, "handoff", G_CALLBACK(handle_sink_handoff),
                   priv);
  ...
  g_signal_connect(bus, "message::error", G_CALLBACK(handle_bus_error_message),
                   priv);
  g_signal_connect(bus, "message::eos", G_CALLBACK(handle_bus_eos_message),
                   priv);
...
static void act_gstreamer_cleanup(VisPluginData *plugin) {
  ...
  if (priv->pipeline) {
    g_signal_handlers_disconnect_by_func(priv->sink, "handoff",
                                         handle_sink_handoff);
...
static void act_gstreamer_requisition(VisPluginData *plugin, int *width,
  ...
  gst_structure_get(structure, "width", width, "height", height, NULL);
  gst_caps_unref(caps);
}
...
static void act_gstreamer_resize(VisPluginData *plugin, int width, int height) {
  ...
  GstCaps *caps = gst_caps_new_simple("video/x-raw", "width", G_TYPE_INT, width,
                                      "height", G_TYPE_INT, height, "format",
                                      G_TYPE_STRING, "BGR", NULL);
  g_object_set(priv->capsfilter, "caps", caps, NULL);
  gst_caps_unref(caps);
}
...
static int act_gstreamer_events(VisPluginData *plugin, VisEventQueue *events) {
  ...
  while (visual_event_queue_poll(events, &ev)) {
    ...
    case VISUAL_EVENT_RESIZE:
      act_gstreamer_resize(plugin, ev.event.resize.width,
...
static VisPalette *act_gstreamer_palette(VisPluginData *plugin) { return NULL; }
...
static void act_gstreamer_render(VisPluginData *plugin, VisVideo *video,
  ...
  if (state != GST_STATE_PLAYING && pending_state != GST_STATE_PLAYING) {
    ...
    /* Check pipeline state, make sure it's already playing, or going to be set
     * playing */
    status = gst_element_get_state(priv->pipeline, &state, &pending_state,
                                   10 * GST_MSECOND);

    switch (status) {
    case GST_STATE_CHANGE_ASYNC:
      visual_log(VISUAL_LOG_INFO,
                 "Waiting for pipeline to get ready (Current state: %s)",
  ...
  g_main_context_iteration(g_main_loop_get_context(priv->glib_main_loop),
                           FALSE);
  ...
  if (priv->buffer) {
    ...
    const int expected_buffer_size =
        expected_row_pitch * visual_video_get_height(video);
    ...
    if (gst_buffer_get_size(priv->buffer) == expected_buffer_size) {
      ...
      if (gst_buffer_map(priv->buffer, &info, GST_MAP_READ)) {
        VisVideo *const source = visual_video_new_wrap_buffer(
            info.data, FALSE, visual_video_get_width(video),
            visual_video_get_height(video), VISUAL_VIDEO_DEPTH_24BIT,
...
static void handle_sink_handoff(GstElement *sink, GstBuffer *buffer,
                                GstPad *pad, GStreamerPrivate *priv) {
  g_mutex_lock(priv->mutex);
