...
void _oink_gfx_palette_save_old(OinksiePrivate *priv) {
  ...
  if (priv->pal_data.pal_new == 1 &&
      priv->pal_data.fade_stepsdone == priv->pal_data.fade_steps)
...
void _oink_gfx_palette_transform(OinksiePrivate *priv) {
  ...
  if (priv->pal_data.fade_start == 1) {
    ...
    for (i = 0; i < 256; i++) {
      priv->pal_fades[i].r =
          (float)(pal_cur_colors[i].r - pal_old_colors[i].r) /
          (float)priv->pal_data.fade_steps;
      priv->pal_fades[i].g =
          (float)(pal_cur_colors[i].g - pal_old_colors[i].g) /
          (float)priv->pal_data.fade_steps;
      priv->pal_fades[i].b =
          (float)(pal_cur_colors[i].b - pal_old_colors[i].b) /
          (float)priv->pal_data.fade_steps;
...
void _oink_gfx_palette_build(OinksiePrivate *priv, uint8_t funky) {
  ...
  if (priv->pal_data.fade_poststop == 1 && priv->pal_startup == 0) {
    ...
    case 0:
    case 1:
      priv->pal_data.fade_poststeps = visual_random_context_int_range(
          priv->rcontext, 60, priv->pal_data.fade_steps);
      break;

    case 2:
      priv->pal_data.fade_poststeps = visual_random_context_int_range(
          priv->rcontext, 40, priv->pal_data.fade_steps / 2);
      break;

    case 3:
      priv->pal_data.fade_poststeps = visual_random_context_int_range(
          priv->rcontext, 20, priv->pal_data.fade_steps / 3);
      break;
    case 4:
      priv->pal_data.fade_poststeps = visual_random_context_int_range(
          priv->rcontext, 80, priv->pal_data.fade_steps);
      break;
