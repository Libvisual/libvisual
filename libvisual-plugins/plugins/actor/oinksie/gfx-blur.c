...
void _oink_gfx_blur_fade(OinksiePrivate *priv, uint8_t *buf, int fade) {
  ...
  if (visual_cpu_has_mmx()) {
    ...
    __asm __volatile("\n\t movd %[fade], %%mm3"
                     "\n\t movd %[fade], %%mm4"
                     "\n\t psllq $32, %%mm3"
                     "\n\t por %%mm4, %%mm3" ::[fade] "m"(fadeflag));

    for (i = 0; i < priv->screen_size; i += 8) {
      __asm __volatile("\n\t movq %[buf], %%mm0"
                       "\n\t psubsb %%mm3, %%mm0"
                       "\n\t movq %%mm0, %[buf]" ::[buf] "m"(*buf));
    }
...
void _oink_gfx_blur_simple(OinksiePrivate *priv, uint8_t *buf) {
  ...
  for (i = 0; i < (priv->screen_size - priv->screen_width - 1); i++) {
    buf[i] = (buf[i + 1] + buf[i + 2] + buf[i + priv->screen_width] +
              buf[i + 1 + (priv->screen_width)]) >>
             2;
  }

  for (i = (priv->screen_size - priv->screen_width - 1);
       i < priv->screen_size - 2; i++) {
...
void _oink_gfx_blur_middle(OinksiePrivate *priv, uint8_t *buf) {
  ...
  if (visual_cpu_has_mmx()) {
    ...
    for (i = 0; i < scrsh; i++) {
      *(buf + i) = (*(buf + i) + *(buf + i + priv->screen_width) +
...
void _oink_gfx_blur_midstrange(OinksiePrivate *priv, uint8_t *buf) {
  ...
  if (visual_cpu_has_mmx()) {
    ...
    for (i = scrsh; i > 0; i--) {
      buf[i] =
          (buf[i] + buf[i + priv->screen_width] +
           buf[i + priv->screen_width + 1] + buf[i + priv->screen_width - 1]) >>
          2;
    }
