...
void _oink_gfx_line(OinksiePrivate *priv, uint8_t *buf, int color, int x0,
  ...
  if (x0 < 0 || x0 > priv->screen_width - 1 || x1 < 0 ||
      x1 > priv->screen_width - 1 || y0 < 0 || y0 > priv->screen_height - 1 ||
      y1 < 0 || y1 > priv->screen_height - 1) {
    if (x0 < 0)
      x0 = 0;
...
void _oink_gfx_hline(OinksiePrivate *priv, uint8_t *buf, int color, int y,
                     int x1, int x2) {
...
void _oink_gfx_circle(OinksiePrivate *priv, uint8_t *buf, int color, int xsize,
  ...
  for (i = 0; i < size; i++) {
    _oink_gfx_pixel_set(priv, buf, color,
                        (int)(_oink_table_sin[i * tabx] * xsize) + x,
