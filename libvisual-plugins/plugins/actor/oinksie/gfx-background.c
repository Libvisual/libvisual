...
void _oink_gfx_background_circles_sine(OinksiePrivate *priv, uint8_t *buf,
                                       int color, int rotate, int scroll,
  ...
  for (i = 0; i < priv->screen_width; i += 20) {
    ...
    sden = _oink_table_sin[abs((tab + OINK_TABLE_NORMAL_SIZE / 2) %
    ...
    _oink_gfx_circle_filled(priv, buf, color - fabs(sden * 20),
                            15 - fabs(sden * 10), x + priv->screen_halfwidth,
                            y + priv->screen_halfheight);
    _oink_gfx_circle_filled(priv, buf, color - fabs(sden * 20),
                            15 - fabs(sden * 10), x1 + priv->screen_halfwidth,
                            y1 + priv->screen_halfheight);

    tab += stretch;
  }
}

void _oink_gfx_background_circles_filled(OinksiePrivate *priv, uint8_t *buf,
                                         int color, int size, int number,
                                         int distance, int turn, int x, int y) {
  int i;
  int xi;
  int yi;
...
// In C, `-8 % 5` yields -3 (and `-8 % 5u` yields +3) rather than 2. This
...
void _oink_gfx_background_floaters(OinksiePrivate *priv, uint8_t *buf,
                                   int color, int size, int number, int xturn,
                                   int yturn, int x, int badd1, int badd2) {
  int i;
  int xi;
  int yi;
  int xb;
  ...
  for (i = 0; i < number; i++) {
    xi = (_oink_table_sin[mod(xturn + add1, OINK_TABLE_NORMAL_SIZE)] *
          (float)(priv->screen_width / (number + 1)));
    yi = (_oink_table_cos[mod(yturn + add2, OINK_TABLE_NORMAL_SIZE)] *
          (float)(priv->screen_height / 5));

    xb = xi + (i * mul) + 20;
    yb = yi + x;

    if (xb > 0 + size || xb < priv->screen_width - size || yb > 0 + size ||
        yb < priv->screen_height - size)
...
void _oink_gfx_background_ball_shooting(OinksiePrivate *priv, uint8_t *buf,
                                        int color, int distance, int xb, int yb,
                                        int x, int y) {
...
void _oink_gfx_background_ball_whirling(OinksiePrivate *priv, uint8_t *buf,
                                        int color, int size, int dia, int rot,
                                        int x, int y) {
  int xd;
  int yd;
...
void _oink_gfx_background_fill(OinksiePrivate *priv, uint8_t *buf, int color) {
  visual_mem_set(buf, color, priv->screen_size);
}

void _oink_gfx_background_circles_star(OinksiePrivate *priv, uint8_t *buf,
                                       int color, int size, int tentnr,
                                       int ballnr, int badd, int turn, int x,
                                       int y) {
  int i;
  int i2;
  int sizedef = 0;
