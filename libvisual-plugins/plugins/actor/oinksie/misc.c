...
int _oink_line_x_next_rad(int x, int length, float rad) {
  /*	return (_oink_table_sinlarge[(int) (rad * (OINK_TABLE_LARGE_SIZE / (PI *
   * 2)))] * length) + x; */
  return (sin(rad) * length) + x;
}

int _oink_line_y_next_rad(int y, int length, float rad) {
  /*	return (_oink_table_coslarge[(int) (rad * (OINK_TABLE_LARGE_SIZE / (PI *
   * 2)))] * length) + y; */
...
void _oink_pixel_rotate(int *x, int *y, int rot) {
  ...
  *x = (tx * _oink_table_cos[rot % OINK_TABLE_NORMAL_SIZE]) +
       (*y * _oink_table_sin[rot % OINK_TABLE_NORMAL_SIZE]);
  *y = (tx * _oink_table_sin[rot % OINK_TABLE_NORMAL_SIZE]) -
       (*y * _oink_table_cos[rot % OINK_TABLE_NORMAL_SIZE]);
}
