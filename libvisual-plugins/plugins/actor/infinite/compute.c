...
static t_complex _inf_fct(InfinitePrivate *priv, t_complex a, int n, int p1,
                          int p2) // p1 et p2:0-4
{
  t_complex b;
  float fact;
...
static void _inf_generate_sector(InfinitePrivate *priv, int g, int f, int p1,
                                 int p2, int debut, int step,
  ...
  for (c.y = debut; c.y < fin; c.y++)
    for (c.x = 0; c.x < priv->plugwidth; c.x++) {
      ...
      w3 = (int)(fpy * lw);
      w1 = lw - w3;
      vector_field[b_add + add].weight =
          (w1 << 24) | (w2 << 16) | (w3 << 8) | w4;
    }
}

void _inf_generate_vector_field(InfinitePrivate *priv,
                                t_interpol *vector_field) {
  int f;
  int i, p1, p2;
