...
void _inf_generate_colors(InfinitePrivate *priv) {
  ...
  for (k = 0; k < NB_PALETTES; k++) {
    ...
    for (i = 0; i < 128; i++) {
      priv->color_table[k][i + 128].r =
          colors[k][0][0] * 127 + colors[k][1][0] * i;
      priv->color_table[k][i + 128].g =
          colors[k][0][1] * 127 + colors[k][1][1] * i;
      priv->color_table[k][i + 128].b =
          colors[k][0][2] * 127 + colors[k][1][2] * i;
    }
  }
}
...
void _inf_change_color(InfinitePrivate *priv, int t2, int t1, int w) {
  ...
  for (i = 0; i < 256; i++) {
    int r, g, b;
    r = (priv->color_table[t1][i].r * w +
         priv->color_table[t2][i].r * (256 - w)) >>
        8;
    g = (priv->color_table[t1][i].g * w +
         priv->color_table[t2][i].g * (256 - w)) >>
        8;
    b = (priv->color_table[t1][i].b * w +
         priv->color_table[t2][i].b * (256 - w)) >>
        8;
...
static void _inf_compute_surface(InfinitePrivate *priv,
   ...
   * every buffer containing one element, the different weights, the upper and
   * lower part of the interpolation coords etc, with this setup it's MUCH
   * easier to vectorize it damn hard.
   */

  /* FIXME ok this sucked, is een mislukte actie geweest zie ik, omdat ik mmx
   * regs te kort kwam zat ik met restore problemen, waardoor het uiteindelijk
   * even snel werd, moet de hele blur routine van deze plugin from scratch
   * doen... */
  if (0) {
#if 0
...
*/
    ...
    const uint8_t *ptr_pix_end =
        priv->surface1 + (priv->plugwidth * priv->plugheight);
...
void _inf_display(InfinitePrivate *priv, uint8_t *surf, int pitch) {
  ...
  for (i = 0; i < priv->plugheight; i++) {
    ...
    visual_mem_copy(surf, priv->surface1 + (i * priv->plugwidth),
                    priv->plugwidth);
...
static void _inf_line(InfinitePrivate *priv, int x1, int y1, int x2, int y2,
                      int c) {
  int dx, dy, cxy, dxy;
...
void _inf_spectral(InfinitePrivate *priv, t_effect *current_effect,
                   float data[2][512]) {
  int i, halfheight, halfwidth;
  float old_y1, old_y2;
  float y1 = (((data[0][0] + data[1][0]) * 128) *
              current_effect->spectral_amplitude * priv->plugheight) *
             (1.0 / 4096.0);
  float y2 = (((data[0][0] + data[1][0]) * 128) *
              current_effect->spectral_amplitude * priv->plugheight) *
             (1.0 / 4096.0);
  ...
  for (i = step; i < priv->plugwidth; i += step) {
    ...
    case 0:
      _inf_line(priv, i - step, halfheight + shift + old_y2, i,
                halfheight + shift + y2, current_effect->spectral_color);
      break;
    case 1:
      _inf_line(priv, i - step, halfheight + shift + old_y1, i,
                halfheight + shift + y1, current_effect->spectral_color);
      _inf_line(priv, i - step, halfheight - shift + old_y2, i,
                halfheight - shift + y2, current_effect->spectral_color);
      break;
    case 2:
      ...
      _inf_line(priv, halfwidth + shift + old_y2, i - step,
                halfwidth + shift + y2, i, current_effect->spectral_color);
      _inf_line(priv, halfwidth - shift + old_y2, i - step,
                halfwidth - shift + y2, i, current_effect->spectral_color);
      break;
    case 3:
...
void _inf_curve(InfinitePrivate *priv, t_effect *current_effect) {
  ...
  for (j = 0; j < 2; j++) {
    ...
    for (i = 0; i < 64; i++) {
      ...
      y = sin((float)(k) / (1.756 * (v + v * j * 0.93))) * priv->plugheight *
          amplitude;
      _inf_plot2(priv,
                 x * cos((float)k * vr) + y * sin((float)k * vr) +
                     priv->plugwidth / 2,
                 x * sin((float)k * vr) - y * cos((float)k * vr) +
                     priv->plugheight / 2,
