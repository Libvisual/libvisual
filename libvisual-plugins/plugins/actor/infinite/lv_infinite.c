...
static void act_infinite_requisition(VisPluginData *plugin, int *width,
                                     int *height);
static int act_infinite_events(VisPluginData *plugin, VisEventQueue *events);
static void act_infinite_render(VisPluginData *plugin, VisVideo *video,
                                VisAudio *audio);
static VisPalette *act_infinite_palette(VisPluginData *plugin);

const VisPluginInfo *get_plugin_info(void) {
  static VisActorPlugin actor = {.requisition = act_infinite_requisition,
  ...
  static VisPluginInfo info = {
      ...
      .author = N_("Original by: Julien Carme <julien.carme@acm.org>, Port by: "
...
int act_infinite_init(VisPluginData *plugin) {
  ...
  priv->color =
      visual_random_context_int_range(priv->rcontext, 0, NB_PALETTES - 1);
...
static int act_infinite_events(VisPluginData *plugin, VisEventQueue *events) {
  ...
  while (visual_event_queue_poll(events, &ev)) {
    ...
    case VISUAL_EVENT_RESIZE:
      act_infinite_resize(plugin, ev.event.resize.width,
...
static void act_infinite_render(VisPluginData *plugin, VisVideo *video,
  ...
  VisBuffer *pcm_left =
      visual_buffer_new_wrap_data(priv->pcm_data[0], sizeof(float) * 512, 0);
  visual_audio_get_sample(audio, pcm_left, VISUAL_AUDIO_CHANNEL_LEFT);
  visual_buffer_unref(pcm_left);

  VisBuffer *pcm_right =
      visual_buffer_new_wrap_data(priv->pcm_data[1], sizeof(float) * 512, 0);
