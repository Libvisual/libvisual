...
public:
  ...
  void ApplyMatrix(FloatPoint &p, FloatPoint &C) {
    FloatPoint P(p);
    p[0] = (P[2] - C[2]) * v[0][2] + (P[1] - C[1]) * v[0][1] +
           (P[0] - C[0]) * v[0][0] + C[0];
    p[1] = (P[2] - C[2]) * v[1][2] + (P[1] - C[1]) * v[1][1] +
           (P[0] - C[0]) * v[1][0] + C[1];
    p[2] = (P[2] - C[2]) * v[2][2] + (P[1] - C[1]) * v[2][1] +
           (P[0] - C[0]) * v[2][0] + C[2];
  }
...
#define b axe[1]
#define c axe[2]
    double aa = a * a, bb = b * b, cc = c * c, cosa = cos(angle),
           sina = sin(angle), abc = aa + bb + cc, rabc = fastsqrt(abc);
