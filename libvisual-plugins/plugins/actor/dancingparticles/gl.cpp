...
void init_gl(void) {
  ...
  gluPerspective(45.0f, (GLfloat)640 / (GLfloat)480, 0.1f,
...
void draw_gl(void) {
  ...
  for (int i = 0; i < ptsNum; i++) {
    GLfloat colors[][3] = {{1.0, 0.2, 0.2}, {0.2, 1.0, 0.2}, {0.4, 0.4, 1.0}};
    ...
    if (p.mode < 50) {
      ...
      if (pts[(i + 1) % ptsNum][0] > pts[i][0]) {
        /*	      if(speed[i][0]>0)
                                {
                                glColor4f(colors[0][0], colors[0][1],
           colors[0][2], 1.0f); glBegin(GL_QUADS);
                                glVertex3f(pts[i][0]/100,1,pts[i][2]/100);
                                glVertex3f(pts[i][0]/100+SIZE2,1,pts[i][2]/100);
                                glVertex3f(pts[i][0]/100+SIZE2, 1+SIZE2,
           pts[(i+1)%ptsNum][2]/100);
        ...
        */
        ...
        GLfloat vertices[4][2] = {
            ...
            {pts[(i + 1) % ptsNum][0] / 100 + SIZE,
             pts[(i + 1) % ptsNum][1] / 100},
