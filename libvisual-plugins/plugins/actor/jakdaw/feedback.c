...
static void blur_then(JakdawPrivate *priv, int x, int y,
  ...
  for (b = 0; b < priv->yres; b++)
    for (a = 0; a < priv->xres; a++) {
      ...
      case FEEDBACK_ZOOMRIPPLE:
        blur_then(priv, a, b, zoom_ripple);
        break;
      case FEEDBACK_BLURONLY:
        blur_then(priv, a, b, nothing);
        break;
      case FEEDBACK_ZOOMROTATE:
        blur_then(priv, a, b, zoom_rotate);
        break;
      case FEEDBACK_SCROLL:
        blur_then(priv, a, b, scroll);
        break;
      case FEEDBACK_INTOSCREEN:
        blur_then(priv, a, b, into_screen);
        break;
      case FEEDBACK_NEWRIPPLE:
        blur_then(priv, a, b, zoom_ripplenew);
        break;
      default:
        blur_then(priv, a, b, nothing);
      }
...
static void init_table(JakdawPrivate *priv) {
  priv->table = visual_mem_malloc0(priv->xres * priv->yres * sizeof(uint32_t) *
                                   4); // 4 points / pixel
  priv->tableptr = 0;
}
...
static uint32_t zoom_rotate(JakdawPrivate *priv, int x, int y) {
  ...
  if (nx < 0 || nx >= priv->xres || ny < 0 || ny >= priv->yres) {
    nx = priv->xres >> 1;
    ny = priv->yres >> 1;
  }
...
static uint32_t zoom_ripple(JakdawPrivate *priv, int x, int y) {
  ...
  dist *= 3.14 * priv->zoom_ripplesize /
          sqrt((priv->xres * priv->xres) + (priv->yres * priv->yres));
