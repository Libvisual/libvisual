...
static void fuzzydot(int x, int y, int add) {
  ...
  for (yy = -4; yy <= 4; yy++) {
    for (xx = -4, point = &IMG_PIXEL(x + xx, y + yy); xx <= 4; xx++, point++) {
      sum = (xx * xx + yy * yy + 1) * 4;
      if (sum <= add) {
        sum = add - sum;
...
static void plussign(int x, int y, int add) {
  ...
  if (x < extent || x >= img_width - extent || y < extent ||
      y >= img_height - extent)
    return;
  extent -= 1; /* <-- for safety */
...
static void edges(int x, int y, int thick) {
  ...
  if (img_width < 30 || img_height < 30 || x < 0 || x >= img_width || y < 0 ||
      y >= img_height)
    return;
...
static void plot(int x, int y) {
  ...
  case 'M': /* Mirrored */
    if (!plotfirst) {
      line(img_width - 1 - fromx, img_height - 1 - fromy, img_width - 1 - x,
           img_height - 1 - y, plotcolor);
      if (plotthick >= 2)
        line(img_width - 1 - fromx, img_height - fromy, img_width - 1 - x,
             img_height - y, plotcolor);
      if (plotthick >= 3)
        line(img_width - fromx, img_height - 1 - fromy, img_width - x,
             img_height - 1 - y, plotcolor);
    }
    /* fall through... */
  ...
  case 'R': /* Radar */
    ...
    line((int)(x * plotcos) + img_width / 2,
         ...
         (int)(x * plotprevsin) + img_height / 2, y);
    break;
...
static int condition_data(int max, int ndata, int16_t **dataref) {
  ...
  if (ndata > max) {
    for (step = (ndata + max - 1) / max, i = 0, j = step / 2; j < ndata;
         j += step, i++) {
  ...
  while (ndata < max / 2) {
    /* interpolate */
    for (j = ndata - 1, i = ndata * 2 - 2; j >= 0; j--, i -= 2) {
      renderdata[i] = (*dataref)[j];
    ...
    *dataref = renderdata;
    ...
    /* smooth */
    for (i = 1; i < ndata - 2; i++)
      renderdata[i] = (int16_t)((renderdata[i - 1] + renderdata[i + 1]) * 3 +
                                renderdata[i] * 10) >>
                      4;
  }
  return ndata;
...
static void render_radial(int thick, int center, int ndata, int16_t *data) {
  ...
  for (theta = 0.0, step = 2.0 * VISUAL_MATH_PI / ndata, i = 0; i < ndata;
       theta += step, i++) {
...
char *render_plotname(int i) {
  static char *name[] = {"Line",       "Mirrored", "Dots",      "Fuzzy dots",
                         "Plus signs", "Airbrush", "Starburst", "Inertia",
                         "Radar",      "Edges",    "None"};
...
char *signal_style_name(int i) {
  static char *names[] = {
      ...
      "High/Low plot", "Stereo spectrum", "Mono spectrum", NULL};
  return names[i];
}
