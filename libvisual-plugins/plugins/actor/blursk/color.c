...
static int32_t hsv_to_rgb(hsv_t *hsv) {
  ...
  return ((((int32_t)r & 0xff) << 16) | (((int32_t)g & 0xff) << 8) |
          ((int32_t)b & 0xff));
}
...
static int32_t dimming(int32_t i) {
  return (((int32_t)(i * red / 256) << 16) | ((int32_t)(i * green / 256) << 8) |
          ((int32_t)(i * blue / 256)) | ((255 - i) << 24));
}
...
static int32_t cloud(int32_t i) {
  ...
  if (i == 128 && *config.overall_effect == 'B') /* "Bump effect" */
  {
    ...
    k = 0;
             ...
             *config.overall_effect == 'B') /* "Bump effect" */
  {
...
static int32_t colorlayers(int32_t i) {
  ...
  /* set this color */
  return (((int32_t)(tmp * r / 256) << 16) | ((int32_t)(tmp * g / 256) << 8) |
          ((int32_t)(tmp * b / 256)) | k << 26);
}
...
static int32_t stripes(int32_t i) {
  ...
  return (((int32_t)(tmp * red / 256) << 16) |
          ((int32_t)(tmp * green / 256) << 8) | ((int32_t)(tmp * blue / 256)) |
          (k << 26));
}

static int32_t colorstripes(int32_t i) {
  int32_t r, g, b, k, tmp;
  static int32_t brightness[] = {0,   64,  128, 192, 254, 254, 254, 254,
                                 254, 254, 254, 254, 254, 192, 128, 64};
...
static struct colorstyles {
  ...
  int good_for_bump;
} colorstyles[17] = {{"Dimming", dimming, TRUE},
 ...
 */
void color_transition(BlurskPrivate *priv,
...
void color_bg(BlurskPrivate *priv, int ndata, int16_t *data) {
   ...
   * then do nothing
   */
  if (bgletter != 'F' && bgred == tored && bggreen == togreen &&
      bgblue == toblue) {
    if (!tonew)
      return;
    tonew = FALSE;
  ...
  for (i = 0; i < 256; i++) {
    ...
    bg = (((bgr * k) << 8) & 0x00ff0000) | ((bgg * k) & 0x0000ff00) |
         (((bgb * k) >> 8) & 0x000000ff);
...
char *color_background_name(int i) {
  static char *names[] = {"Black bkgnd",  "White bkgnd", "Dark bkgnd",
                          "Shift bkgnd",  "Color bkgnd", "Flash bkgnd",
                          "Random bkgnd", NULL};
  return names[i];
}
