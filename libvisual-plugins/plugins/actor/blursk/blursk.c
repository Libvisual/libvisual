...
char *floaters_name(int i) {
  static char *names[] = {"No floaters",   "Dots",           "Slow floaters",
                          "Fast floaters", "Retro floaters", NULL};
  return names[i];
}
...
static void drawfloaters(int beat) {
  ...
  for (i = 0; i < nfloaters; i++) {
    ...
    if (*config.floaters == 'D' || i >= prevfloaters ||
        floater[i].age++ > 80 + i * 13 || beat || floater[i].x < 0 ||
        floater[i].x >= img_width || floater[i].y < 0 ||
        floater[i].y >= img_height) {
     ...
     */
    if (delta == 0 || floater[i].x < 0 || floater[i].x >= img_width ||
...
static unsigned char *show_info(unsigned char *img, int height, int bpl) {
  ...
  if (songinfo == NULL ||
      visual_songinfo_get_type(songinfo) == VISUAL_SONGINFO_TYPE_NULL)
    return img;
    ...
    if (pos != prevpos) {
      ...
      case VISUAL_SONGINFO_TYPE_SIMPLE:
        if (config.show_timestamp) {
          sprintf(buf, "{%s/%s} %s", posstr, lenstr,
                  visual_songinfo_get_simple_name(songinfo));
          break;
        } else {
      ...
      case VISUAL_SONGINFO_TYPE_ADVANCED:
        if (config.show_timestamp) {
          if (strcmp(visual_songinfo_get_artist(songinfo), "(null)") == 0) {
            if (length >= 0)
              sprintf(buf, "{%s/%s} %s", posstr, lenstr,
                      visual_songinfo_get_song(songinfo));
            else
              sprintf(buf, "(%s) %s", posstr,
                      visual_songinfo_get_song(songinfo));
          } else {
            if (length >= 0)
              sprintf(buf, "{%s/%s} %s by %s", posstr, lenstr,
                      visual_songinfo_get_song(songinfo),
                      visual_songinfo_get_artist(songinfo));
            else
          ...
          if (strcmp(visual_songinfo_get_artist(songinfo), "(null)") == 0) {
            ...
            sprintf(buf, "%s by %s", visual_songinfo_get_song(songinfo),
                    visual_songinfo_get_artist(songinfo));
          }
        }
        break;
 ...
 */
static void update_image(BlurskPrivate *priv, int32_t loudness, int ndata,
                         int16_t *data) {
...
static void blursk_render_pcm(BlurskPrivate *priv, int16_t *data) {
   ...
   */
  loudness =
      (((int32_t)data[imax] - (int32_t)data[imin]) * 60 + delta_sum) / 75;
