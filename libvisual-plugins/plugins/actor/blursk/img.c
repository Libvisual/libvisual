...
void img_copyback(void) {
  ...
  for (i = img_height, src = img_tmp, dst = img_buf; --i >= 0;
...
uint8_t *img_ripple(int *widthref, int *heightref, int *bplref) {
  ...
  for (i = QTY(tbl); --i >= 0;) {
    tbl[i] =
        i +
        (uint8_t)((double)((QTY(tbl) / 2 - abs((int)QTY(tbl) / 2 - i)) >> 1) *
                  sin((double)(i + img_rippleshift) / 10.0));
  }
  ...
  case 'F': /* Fast CPU */
    ...
    for (src = img_buf, dst = img_tmp, i = img_chunks; --i >= 0;) {
      *dst++ = tbl[*src++];
      *dst++ = tbl[*src++];
      *dst++ = tbl[*src++];
      *dst++ = tbl[*src++];
