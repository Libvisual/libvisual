...
#define BLUR                                                                   \
  src = *srcref++;                                                             \
  *dest++ = (src[-bpl] + src[0] + src[bpl - 1] + src[bpl + 1]) >> 2;           \
  bpl = -bpl;
...
#define SMEAR                                                                  \
  src = *srcref++;                                                             \
  pix = (src[-bpl - 1] + src[bpl - 1] + src[0] + src[1]) >> 2;                 \
  if (pix < *orig++)                                                           \
    pix = orig[-1];                                                            \
  *dest++ = pix;                                                               \
  bpl = -bpl;
...
#define MELT                                                                   \
  ...
  pix = *orig++;                                                               \
  if (pix < 160)                                                               \
    pix = (src[-bpl] + src[0] + src[bpl - 1] + src[bpl + 1]) >> 2;             \
  *dest++ = pix;                                                               \
  bpl = -bpl;
...
void loopreduced1(void) {
  ...
  srcref = img_source;
  do {
    BLUR SHARP SHARP SHARP BLUR SHARP SHARP SHARP
  } while (--i != 0);
}
...
void loopreduced2(void) {
  ...
  do {
    SHARP
    BLUR SHARP SHARP SHARP BLUR SHARP SHARP
...
void loopreduced3(void) {
  ...
  do {
    SHARP
    SHARP
    BLUR SHARP SHARP SHARP BLUR SHARP
