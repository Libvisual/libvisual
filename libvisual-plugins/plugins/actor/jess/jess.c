...
static void act_jess_requisition(VisPluginData *plugin, int *width,
                                 int *height);
...
static void act_jess_render(VisPluginData *plugin, VisVideo *video,
                            VisAudio *audio);
...
const VisPluginInfo *get_plugin_info(void) {
  static VisActorPlugin actor = {.requisition = act_jess_requisition,
  ...
  static VisPluginInfo info = {
      ...
      .plugname = "jess",
      .name = "jess plugin",
      .author = N_("Original by: Remi Arquier <arquier@crans.org>, Port by: "
                   "Dennis Smit <ds@nerds-incorporated.org>"),
      .version = "0.1",
...
static int act_jess_init(VisPluginData *plugin) {
  ...
  priv->pcm_data1 = visual_buffer_new_wrap_data(priv->pcm_data[0],
                                                512 * sizeof(float), FALSE);
  priv->pcm_data2 = visual_buffer_new_wrap_data(priv->pcm_data[1],
                                                512 * sizeof(float), FALSE);
...
static void jess_init(JessPrivate *priv) {
  ...
  priv->table1 =
      (uint32_t *)visual_mem_malloc0(priv->resx * priv->resy * sizeof(int));
  priv->table2 =
      (uint32_t *)visual_mem_malloc0(priv->resx * priv->resy * sizeof(int));
  priv->table3 =
      (uint32_t *)visual_mem_malloc0(priv->resx * priv->resy * sizeof(int));
