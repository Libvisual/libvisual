...
void analyser(JessPrivate *priv, uint8_t *buffer) {
  ...
  for (i = -priv->xres2; i < -priv->xres2 + 5; i++) {
    ...
    droite(priv, buffer, i + 5, 0, i + 5, (int)(priv->lys.dEdt_moyen * 25000),
           230);
...
void fusee(JessPrivate *priv, uint8_t *buffer, int new) {
  ...
  if (new == NEW) /* on creer une nouvelle fusee*/
  {
    ...
    priv->xi[i] =
        visual_random_context_int(priv->rcontext) % priv->resx - priv->xres2;
...
void super_spectral_balls(JessPrivate *priv, uint8_t *buffer) {
  ...
  for (i = 0; i < 256; i++) /* pour chaque frequence */
  {
    if (priv->lys.dbeat[i] == NEW) /*creation d'une nouvelle ligne */
    {
      ...
      priv->vx[i][j] =
          RESFACTXF(0.025 * ((float)i - 128.0) * 32 +
                    0 * (1 - visual_random_context_int(priv->rcontext) /
                                 (float)UINT32_MAX));
      priv->vy[i][j] = RESFACTYF((10 + i) * i * priv->lys.Ed_moyen[i] * 5000 *
                                 ((float)j + 1) / 4);
      priv->x[i][j] = RESFACTXF(2 * (i - 128)) + ((float)j * (i - 128)) / 2;
      priv->y[i][j] =
          0 * RESFACTXF(yres2 / 2 - (float)((i - 128) * (i - 128)) / 256) -
          20 * j;
    }
        ...
        if ((priv->y[i][j] < resy) && (priv->y[i][j] > -resy)) /*oui !*/
        {
          if (i > 128)
            droite(priv, buffer, priv->xres2, (int)priv->y[i][j] / 32,
                   (int)priv->x[i][j], (int)priv->y[i][j],
                   50 * (LINE_VIE - priv->lifev[i][j]) / LINE_VIE);
          else
...
void super_spectral(JessPrivate *priv, uint8_t *buffer) {
  ...
  for (i = 0; i < 256; i++) /* pour chaque frequence */
  {
    if (priv->lys.dbeat[i] == NEW) /*creation d'une nouvelle ligne */
    {
      ...
      priv->ssvx[i][j] =
          0 * RESFACTXF(0.025 * ((float)i - 128.0) * 32 +
                        visual_random_context_int(priv->rcontext) /
                            (float)UINT32_MAX * 60);
      priv->ssvy[i][j] =
          0 * RESFACTYF(64 + visual_random_context_int(priv->rcontext) /
                                 (float)UINT32_MAX * 64);
  ...
  quit:; /* pas de nouvelle ligne */
    for (j = 0; j < LINE_MAX; j++)
      if (priv->lifet[i][j] > 0) {
        ...
        taille = RESFACTXF(70) * (0 + 2 * (LINE_VIE - priv->lifet[i][j])) /
                 LINE_VIE * (j + 1) / 6;
        nx = taille * sin(priv->sstheta[i][j]);
        ny = taille * cos(priv->sstheta[i][j]);

        droite(priv, buffer, (int)priv->ssx[i][j] + nx,
               (int)priv->ssy[i][j] + ny, (int)priv->ssx[i][j],
               (int)priv->ssy[i][j],
               50 * (LINE_VIE - priv->lifet[i][j]) / LINE_VIE);

        if (priv->video == 8)
          cercle(priv, buffer, (int)priv->ssx[i][j] + nx,
                 (int)priv->ssy[i][j] + ny, 3 * j,
                 150 * (LINE_VIE - priv->lifet[i][j]) / LINE_VIE);
        else
          cercle_32(priv, buffer, (int)priv->ssx[i][j] + nx,
                    (int)priv->ssy[i][j] + ny, 3 * j,
                    150 * (LINE_VIE - priv->lifet[i][j]) / LINE_VIE);
...
void grille_3d(JessPrivate *priv, uint8_t *buffer, float data[2][512],
               float alpha, float beta, float gamma, int persp, int dist_cam) {
...
void l2_grilles_3d(JessPrivate *priv, uint8_t *buffer, float data[2][512],
                   float alpha, float beta, float gamma, int persp,
                   int dist_cam) {
  ...
  for (i = 0; i < nb_x; i++) {
    /* Optimisation par Karl Soulabaille. Factorisation de calcul & expression
     * commune */
...
void burn_3d(JessPrivate *priv, uint8_t *buffer, float data[2][512],
             float alpha, float beta, float gamma, int persp, int dist_cam,
             int mode) {
  float x, y, z, xres2 = (float)(priv->resx >> 1),
                 yres2 = (float)(priv->resy >> 1);
  short ix, iy, i, j, nb_x, nb_y, color, taille;

  /* Variable temporaire pour la factorisation de constantes */
  float temp1, temp2, temp3, temp3b, temp4, temp4b, temp5, temp6, temp7, temp8,
      temp9;
  ...
  case 1:
    ...
    for (i = 0; i < nb_x; i++) {
      ...
      for (j = 0; j < nb_y; j++) {
        ...
        z = RESFACTXF((float)100 * cos((float)i / nb_x * PI) *
                      (1 + priv->lys.dEdt_moyen * 1000));
        rotation_3d(&x, &y, &z, alpha, beta, gamma);
        perspective(&x, &y, &z, persp, dist_cam);
        if (!(x >= xres2 || x <= -xres2 || y >= yres2 || y <= -yres2)) {
          ix = (int)x;
  ...
  case 2:
    ...
    for (i = 0; i < nb_x; i++) {
      ...
      for (j = 0; j < nb_y; j++) {
        ...
        z = -RESFACTXF(130 * cos((float)i / nb_x * PI) * priv->lys.dEdt_moyen *
                       1000);
        rotation_3d(&x, &y, &z, alpha, beta, gamma);
        perspective(&x, &y, &z, persp, dist_cam);
        if (!(x >= xres2 || x <= -xres2 || y >= yres2 || y <= -yres2)) {
...
void stars_create_state(JessPrivate *priv, float pos[3][STARS_MAX], int mode) {
  ...
  case 1:
    for (i = 0; i < STARS_MAX; i++) {
      for (j = 0; j < 3; j++) {
        pos[j][i] =
            (visual_random_context_int(priv->rcontext) / (float)UINT32_MAX -
             0.5);
      }
    }
    break;
...
void stars_manage(JessPrivate *priv, uint8_t *buffer, int new, float alpha,
                  float beta, float gamma, int persp, int dist_cam) {
  ...
  } else if (new == NEW) /* on creer une nouvelle forme */
  {
    ...
    stars_create_state(priv, priv->smpos[priv->smselect],
                       visual_random_context_int(priv->rcontext) % 2 + 1);
  } else /* on gere */
  {
    priv->smmorpheur +=
        (2 * (float)priv->smselect - 1) * MORPH_SPEED * priv->conteur.dt;
    ...
    for (i = 0; i < STARS_MAX; i++) {
      x[i] = (priv->smpos[1][0][i] * priv->smmorpheur +
              (1 - priv->smmorpheur) * priv->smpos[0][0][i]);
      y[i] = (priv->smpos[1][1][i] * priv->smmorpheur +
              (1 - priv->smmorpheur) * priv->smpos[0][1][i]);
      z[i] = (priv->smpos[1][2][i] * priv->smmorpheur +
              (1 - priv->smmorpheur) * priv->smpos[0][2][i]);
      ...
      if ((int)x_t >= xres2 || (int)x_t <= -xres2 || (int)y_t >= yres2 ||
          (int)y_t <= -yres2 || z_t > 2 * dist_cam)
        return;
...
void courbes(JessPrivate *priv, uint8_t *buffer, float data[2][512],
             uint8_t color, int type) {
