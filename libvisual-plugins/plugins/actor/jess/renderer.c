...
void draw_mode(JessPrivate *priv, int mode) {
  ...
  case NON:                               /* bruit calme */
    ...
    if (mode == 0)
      ...
      l2_grilles_3d(priv, priv->pixel, priv->pcm_data,
                    priv->conteur.angle2 / 200, 0, priv->conteur.angle2 / 30,
                    200, 130);
    else if (mode == 2)
  ...
  case OUI: /* bruit modere */
    ...
    if (mode == 0)
      grille_3d(priv, priv->pixel, priv->pcm_data, priv->conteur.angle / 200, 0,
                priv->conteur.angle / 30, 100, -priv->lys.E_moyen * 20 + 130);
    else if (mode == 1)
      l2_grilles_3d(priv, priv->pixel, priv->pcm_data,
                    priv->conteur.angle2 / 200, 0, priv->conteur.angle2 / 30,
                    200, -priv->lys.E_moyen * 20 + 130);
    else if (mode == 2)
      burn_3d(priv, priv->pixel, priv->pcm_data, priv->conteur.angle2 / 400, 0,
              priv->conteur.angle2 / 60, 200, 130, priv->conteur.burn_mode);
    else if ((mode == 3) && (priv->conteur.k3 > 700)) /* mode 3 */
  ...
  if (priv->conteur.k3 < 300) /* Ici c'est les boules qui se barrent */
    burn_3d(priv, priv->pixel, priv->pcm_data, priv->conteur.angle2 / 200, 0,
            priv->conteur.angle2 / 200, 200, -50 + 3 * priv->conteur.k3,
            priv->conteur.burn_mode);
...
void on_beat(JessPrivate *priv, int beat) {
  if (priv->lys.beat == OUI) {
    ...
    /* vitesse a l angle 2 */
    priv->conteur.v_angle2 +=
        (visual_random_context_int(priv->rcontext) % 2 - 0.5) * 16 * 32;
...
void on_reprise(JessPrivate *priv) {
  ...
  if (priv->lys.reprise == OUI) {
    if (priv->conteur.last_flash > 5 * priv->conteur.fps) {
      if (priv->conteur.draw_mode == 5)
        stars_manage(priv, priv->pixel, NEW_SESSION, priv->conteur.angle2 / 400,
                     0, priv->conteur.angle2 / 60, 200, 130);
      ...
      if ((priv->conteur.freeze_mode == 0) && (priv->conteur.mix_reprise > 5) &&
          (priv->conteur.draw_mode != 2)) {
        priv->conteur.blur_mode = visual_random_context_int(priv->rcontext) % 5;
      }
    }
  }
    ...
    if (visual_cpu_has_mmx()) {
      ...
      for (pix = priv->pixel; pix < (uint8_t *)bmax - 9; pix += 8) {
        __asm __volatile(
            ...
            "\n\t paddb %%mm2, %%mm1"
            "\n\t paddb %%mm3, %%mm1"
            "\n\t movq %%mm1, %[pix1]" ::[pix1] "m"(*pix),
            [pix2] "m"(*(pix + 1)), [pix3] "m"(*(pix + priv->resx)),
    ...
    if (visual_cpu_has_mmx()) {
      ...
      for (pix = priv->pixel; pix < (uint8_t *)bmax - 12;) {
        __asm __volatile(
            ...
            "\n\t paddb %%mm3, %%mm1"
            "\n\t movq %%mm1, %[pix1]" ::[pix1] "m"(*pix),
            [pix2] "m"(*(pix + 4)), [pix3] "m"(*(pix + priv->pitch)),
            [pix4] "m"(*(pix + pitch_4)));
