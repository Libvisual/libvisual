  ...
  for (k = 1; k < 5; k++) {
    for (i = 0; i < priv->resy; i++) {
      for (j = 0; j < priv->resx; j++) {
        ...
        switch (k) {
        case 1:
          rot_hyperbolic_radial(&n_fx, &n_fy, -PI / 5, 0.001, 0, RESFACTY(50));
          rot_hyperbolic_radial(&n_fx, &n_fy, PI / 2, 0.004, RESFACTX(200),
                                RESFACTY(-30));
          rot_hyperbolic_radial(&n_fx, &n_fy, PI / 5, 0.001, RESFACTX(-150),
                                RESFACTY(-30));
...
void noize(JessPrivate *priv, float *n_fx, float *n_fy, float intensity) {
  ...
  *n_fy +=
      2 * (visual_random_context_float(priv->rcontext) - 0.5) * intensity - 5;
}
