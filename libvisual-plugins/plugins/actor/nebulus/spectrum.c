...
static void draw_rectangle(GLfloat x1, GLfloat y1, GLfloat z1, GLfloat x2,
...
static void draw_bar(GLfloat x_offset, GLfloat z_offset, GLfloat height,
                     GLfloat red, GLfloat green, GLfloat blue) {
  GLfloat width = 0.08f;
  glColor3f(red - 0.1f, 0.0f, blue);
  draw_rectangle(x_offset, height, z_offset, x_offset + width, height,
                 z_offset + 0.1);
  draw_rectangle(x_offset, 0, z_offset, x_offset + width, 0, z_offset + 0.1);
  glColor3f(0.5 * red, 0.0f, 0.6 * blue);
  draw_rectangle(x_offset, 0.0, z_offset + 0.1, x_offset + width, height,
                 z_offset + 0.1);
  ...
  draw_rectangle(x_offset + width, 0.0, z_offset, x_offset + width, height,
                 z_offset + 0.1);
}
...
static void createbars(void) {
  ...
  for (y = 0; y < 16; y++) {
    ...
    for (x = 0; x < 16; x++) {
      x_offset = -1.6 + (x * 0.2);
      draw_bar(x_offset, z_offset, heights[y][x], r_base, y * (r_base / 10.0f),
               b_base);
    }
  }
  glEnd();
