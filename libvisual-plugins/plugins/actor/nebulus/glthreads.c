...
static void createglthreads(void) {
  int i, j;
  glcoord p;
  GLfloat r, g, b;
  short val_pcm;
  ...
  xval += speed_xval;
  xabs += speed_xabs;
  p.x = ((GLfloat)point_general->WIDTH / 2) /
            (GLfloat)(point_general->WIDTH / 8) -
        (fabs(sin(xval) * 4) * 2) + (sin(xabs) / 1.5f);
  p.y = -2.0f;
  p.z = (GLfloat)((point_general->HEIGHT / 2) +
                  ((GLfloat)val_pcm * (0.11f * point_general->HEIGHT) / 1800)) /
            (GLfloat)(point_general->HEIGHT / 6) -
        3.0f;
  ...
  for (j = 0; j < MAXTHREADS; j++) {
    for (i = 0; i < THREADLENGTH - 1; i++) {
      ...
      threads[j][i].y =
          (threads[j][i + 1].y * (2 + (GLfloat)j / 1000) + threads[j][i].y) /
          2.9f;
...
static void drawglthreads(void) {
  glPushMatrix();
...
static void render_glthreads(void) {
