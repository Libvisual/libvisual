...
static void initknot(int scaling_factor1, int scaling_factor2, GLfloat radius1,
  ...
  for (count2 = 0; count2 < scaling_factor2; count2++) {
    ...
    for (count1 = 0; count1 < scaling_factor1; count1++) {
      ...
      knotobject.vertices[knotobject.numverts].x =
          x -
          radius3 * (cos(beta) * dz - sin(beta) * dx * dy / modulus) / value;
      knotobject.vertices[knotobject.numverts].y =
          y - radius3 * sin(beta) * value / modulus;
      knotobject.vertices[knotobject.numverts].z =
          z +
          radius3 * (cos(beta) * dx + sin(beta) * dy * dz / modulus) / value;

      dist = sqrt(knotobject.vertices[knotobject.numverts].x *
                      knotobject.vertices[knotobject.numverts].x +
                  knotobject.vertices[knotobject.numverts].y *
                      knotobject.vertices[knotobject.numverts].y +
                  knotobject.vertices[knotobject.numverts].z *
  ...
  for (count1 = 0; count1 < scaling_factor2; count1++) {
    ...
    rotation = 0;
    mindistance =
        (knotobject.vertices[index1].x - knotobject.vertices[index2].x) *
            (knotobject.vertices[index1].x - knotobject.vertices[index2].x) +
        (knotobject.vertices[index1].y - knotobject.vertices[index2].y) *
            (knotobject.vertices[index1].y - knotobject.vertices[index2].y) +
        (knotobject.vertices[index1].z - knotobject.vertices[index2].z) *
            (knotobject.vertices[index1].z - knotobject.vertices[index2].z);
...
static void recalcknot(int scaling_factor1, int scaling_factor2,
                       GLfloat radius1, GLfloat radius2, GLfloat radius3) {
  int count1, count2;
  GLfloat alpha, beta;
  GLfloat x, y, z, dx, dy, dz;
...
static void draw_the_knot(void) {
  int i, j, num;
  ...
  for (i = 0; i < j; i++) {
    ...
    glColor3f(knotobject.colors[num].r * 2.5, knotobject.colors[num].g * 2,
              knotobject.colors[num].b * 1.5);
...
static void createknot(void) {
  recalcknot(16, 64, 3.4 * sin(0.02 * knot_time) - 1,
             4 * cos(0.006 * knot_time) - 2, 1.1);
...
static void drawknot(void) {
  glPushMatrix();
  createknot();
  glPopMatrix();
}

static void render_knot(void) {
