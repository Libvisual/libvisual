...
static void lv_nebulus_requisition(VisPluginData *plugin, int *width,
                                   int *height);
...
static void lv_nebulus_render(VisPluginData *plugin, VisVideo *video,
                              VisAudio *audio);
...
short pcm_data[512];
nebulus general = {32,    32,    0,     4,     4,     100,
                   100,   0,     FALSE, FALSE, TRUE,  FALSE,
                   FALSE, FALSE, FALSE, FALSE, FALSE, FALSE};
nebulus *point_general = &general;

effect my_effect[EFFECT_NUMBER] = {
    {"Knot", 100},      {"Spectrum", 100}, {"Face blur", 100},
    {"Glthreads", 100}, {"Tunnel", 100},   {"Tentacles", 100},
    {"Twist", 100},     {"Child", 100},    {"Energy", 100}};
...
const VisPluginInfo *get_plugin_info(void) {
  static VisActorPlugin actor = {.requisition = lv_nebulus_requisition,
  ...
  static VisPluginInfo info = {
      ...
      .plugname = "nebulus",
      .name = "Nebulus",
      .author = N_("Original by: Pascal Brochart <pbrochart@tuxfamily.org> and "
                   "many others, Port and maintaince by: Dennis Smit "
...
static int lv_nebulus_init(VisPluginData *plugin) {
  ...
  if (maxtexsize < 256) {
    visual_log(VISUAL_LOG_CRITICAL,
               _("Nebulus: max texture size is lower than 256"));
    return FALSE;
  }
...
static VisPalette *lv_nebulus_palette(VisPluginData *plugin) { return NULL; }
...
static int nebulus_detect_beat(int loudness) {
  ...
  if (elapsed > 15 && aged > 2000 && loudness * 4 > aged * 5) {
    for (i = BEAT_MAX / elapsed, total = 0; --i > 0;
...
static int nebulus_sound(NebulusPrivate *priv, VisAudio *audio) {
  ...
  int xscale[] = {0,  1,  2,  3,  5,   7,   10,  14, 20,
  ...
  for (i = 0; i < 512; i++) {
    pcm_data[i] = fbuf[i] * 32767; // FIXME pull the 32767 as a constant from
                                   // somewhere, also in goom2.c
  }
