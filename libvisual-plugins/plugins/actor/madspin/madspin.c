...
static void lv_madspin_requisition(VisPluginData *plugin, int *width,
                                   int *height);
...
static void lv_madspin_render(VisPluginData *plugin, VisVideo *video,
                              VisAudio *audio);
...
const VisPluginInfo *get_plugin_info(void) {
  static VisActorPlugin actor = {.requisition = lv_madspin_requisition,
  ...
  static VisPluginInfo info = {
      ...
      .author = N_("Original by: Andrew Birck <birck@uiuc.edu>, Port by: "
...
static int lv_madspin_init(VisPluginData *plugin) {
  ...
  visual_param_list_add_many(
      params,
      visual_param_new_integer("num_stars", N_("Number of stars"), 512, NULL),
      visual_param_new_integer("speed", N_("Speed"), 715, NULL), NULL);
...
static void bind_texture(GLuint texture, VisVideo *image) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, visual_video_get_width(image),
...
static VisPalette *lv_madspin_palette(VisPluginData *plugin) { return NULL; }
...
static int madspin_sound(MadspinPrivate *priv, VisAudio *audio) {
  ...
  visual_audio_get_sample_mixed_simple(
      audio, pcmb, 2, VISUAL_AUDIO_CHANNEL_LEFT, VISUAL_AUDIO_CHANNEL_RIGHT);
...
static int madspin_draw(MadspinPrivate *priv, VisVideo *video) {
  ...
  for (line = priv->maxlines; line > 0; line--) {
    for (point = 0; point <= priv->num_stars; point++) {
      ...
      x = sa * snapb * ampl * csab +
          ca * (csapb * ampl * b * csab + s5 * (ampl * (csab + 2 * snapb)));
      y = sa * csapb * ampl * csab +
          ca * (csapb * ampl * b * snab + s5 * (ampl * (csab + 2 * csapb)));
      z = ca * csapb * ampl * csab +
          ca * (snapb * ampl * b * snab + s5 * (ampl * (snab + 2 * snapb)));
      ...
      s1a = ((priv->gdata[(int)(point / priv->num_stars * 220)] +
              (priv->total / 200.0f)) /
             4.0f);
      ...
      if (s1a > 0.008f) {
        ...
        priv->texsize =
            (((priv->gdata[(int)(point / priv->num_stars * 220)])) /
             (2048.01f - (point * 4.0f))) *
                (((point - priv->num_stars) / (-priv->num_stars)) * 18.0f) +
            0.15f;
      ...
      if (s2a > 0.005f) {
        ...
        priv->texsize *=
            ((visual_random_context_int(priv->rcontext) % 100) / 100.0f) * 2.0f;
