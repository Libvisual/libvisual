...
static void act_bumpscope_requisition(VisPluginData *plugin, int *width,
                                      int *height);
...
static void act_bumpscope_render(VisPluginData *plugin, VisVideo *video,
                                 VisAudio *audio);
static VisPalette *act_bumpscope_palette(VisPluginData *plugin);

const VisPluginInfo *get_plugin_info(void) {
  static VisActorPlugin actor = {.requisition = act_bumpscope_requisition,
  ...
  static VisPluginInfo info = {
      ...
      .author = N_("Original by: Zinx Verituse <zinx@xmms.org>, Port by: "
...
static int act_bumpscope_init(VisPluginData *plugin) {
  ...
  visual_param_list_add_many(
      params,
      visual_param_new_color_rgb("color", N_("The light's color"), 122, 204,
                                 255, NULL),
      visual_param_new_integer("light_size", N_("The size of the light"), 256,
                               visual_param_in_range_integer(0, 1000)),
      visual_param_new_bool("color_cycle", N_("Whether to cycle colors"), TRUE,
                            NULL),
      visual_param_new_bool("moving_light",
                            N_("Whether the light moves with the mouse"), TRUE,
                            NULL),
      visual_param_new_bool(
          "diamond", N_("Whether to use a diamond shape light"), FALSE, NULL),
      NULL);
...
static int act_bumpscope_events(VisPluginData *plugin, VisEventQueue *events) {
  ...
  while (visual_event_queue_poll(events, &ev)) {
    ...
    case VISUAL_EVENT_RESIZE:
      act_bumpscope_resize(plugin, ev.event.resize.width,
...
static void act_bumpscope_render(VisPluginData *plugin, VisVideo *video,
  ...
  visual_mem_copy(visual_video_get_pixels(video), priv->rgb_buf2,
  ...
  if (priv->colorchanged == TRUE && priv->colorupdate == 0) {
    ...
    visual_param_set_value_color(
        visual_param_list_get(visual_plugin_get_params(plugin), "color"),
        &priv->color);
  }
}
