...
static void lv_analyzer_requisition(VisPluginData *plugin, int *width,
                                    int *height);
static int lv_analyzer_events(VisPluginData *plugin, VisEventQueue *events);
static void lv_analyzer_render(VisPluginData *plugin, VisVideo *video,
                               VisAudio *audio);
static VisPalette *lv_analyzer_palette(VisPluginData *plugin);

const VisPluginInfo *get_plugin_info(void) {
  static VisActorPlugin actor = {.requisition = lv_analyzer_requisition,
...
static int lv_analyzer_init(VisPluginData *plugin) {
  ...
  visual_param_list_add_many(
      params,
      visual_param_new_integer("bars", "Number of bars in graph", BARS_DEFAULT,
                               NULL),
      visual_param_new_integer("bar-space",
                               "Amount of pixels between bars in graph",
                               BARS_DEFAULT_SPACE, NULL),
      NULL);
...
static void _change_bar_space(VisPluginData *plugin, VisParam *p,
                              int (*validator)(VisPluginData *plugin,
                                               void *value)) {
  AnalyzerPrivate *priv = visual_plugin_get_private(plugin);
...
static void _change_bars(VisPluginData *plugin, VisParam *p,
  ...
  if (!validator || validator(plugin, &integer)) {
    ...
    priv->pcm_buffer =
        visual_buffer_new_allocate(priv->bars * 2 * sizeof(float));
    priv->freq_buffer = visual_buffer_new_allocate(priv->bars * sizeof(float));
    return;
  }
  /* reset to previous value */
...
static void _change_param(VisPluginData *plugin, VisParam *p) {
  ...
  struct {
    ...
    void (*change)(VisPluginData *plugin, VisParam *parameter,
                   int (*validator)(void *value));
    ...
    void (*postchange)(VisPluginData *plugin);
      ...
      {"bar-space", (void *)_validate_bar_space, (void *)_change_bar_space,
       NULL},
  };
...
static inline void draw_bar(VisVideo *video, int x, int width,
                            float amplitude) {
  /* NOTES:
...
static void lv_analyzer_render(VisPluginData *plugin, VisVideo *video,
  ...
  visual_audio_get_spectrum_for_sample(priv->freq_buffer, priv->pcm_buffer,
                                       TRUE);
