...
static void lv_morph_flash_apply(VisPluginData *plugin, float progress,
                                 VisAudio *audio, VisVideo *dest,
                                 VisVideo *src1, VisVideo *src2);
static void lv_morph_flash_palette(VisPluginData *plugin, float progress,
                                   VisAudio *audio, VisPalette *pal,
                                   VisVideo *src1, VisVideo *src2);

static void replacetable_generate_24(FlashPrivate *priv, float progress);
static void flash_8(FlashPrivate *priv, float progress, VisVideo *dest,
                    VisVideo *src1, VisVideo *src2);
static void flash_24(FlashPrivate *priv, float progress, VisVideo *dest,
                     VisVideo *src1, VisVideo *src2);
...
const VisPluginInfo *get_plugin_info(void) {
  static VisMorphPlugin morph = {
      ...
      .vidoptions.depth = VISUAL_VIDEO_DEPTH_8BIT | VISUAL_VIDEO_DEPTH_16BIT |
  ...
  static VisPluginInfo info = {
      ...
      .help = N_("This morph plugin morphs between two video sources using a "
                 "bright flash"),
...
static void replacetable_generate_24(FlashPrivate *priv, float progress) {
  ...
  for (i = 0; i < 256; i++) {
    if (progress < 0.5)
      priv->replacetable[i] =
          i + (((255.00 - i) / 100.00) * ((progress * 2) * 100));
    else
      priv->replacetable[i] = i + (((255.00 - i) / 100.00) *
                                   ((1.0 - ((progress - 0.5) * 2)) * 100));
  }
}
...
static void flash_8(FlashPrivate *priv, float progress, VisVideo *dest,
                    VisVideo *src1, VisVideo *src2) {
  if (progress < 0.5)
    visual_mem_copy(visual_video_get_pixels(dest),
                    visual_video_get_pixels(src1), visual_video_get_size(src1));
  else
    visual_mem_copy(visual_video_get_pixels(dest),
                    visual_video_get_pixels(src2), visual_video_get_size(src2));
}
