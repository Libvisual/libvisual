...
void lv_morph_slide_apply(VisPluginData *plugin, float progress,
  ...
  case SLIDE_LEFT:
    for (i = 0; i < dest_height; i++) {
      visual_mem_copy(destbuf + (i * dest_pitch),
                      srcbuf2 + (i * dest_pitch) + diff2, diff1);
      visual_mem_copy(destbuf + (i * dest_pitch) + (diff1),
                      srcbuf1 + (i * dest_pitch), diff2);
    }
  ...
  case SLIDE_RIGHT:
    for (i = 0; i < dest_height; i++) {
      visual_mem_copy(destbuf + (i * dest_pitch),
                      srcbuf1 + (i * dest_pitch) + diff2, diff1);
      visual_mem_copy(destbuf + (i * dest_pitch) + (diff1),
                      srcbuf2 + (i * dest_pitch), diff2);
    }

    break;

  case SLIDE_DOWN:
    visual_mem_copy(destbuf, srcbuf1 + (hadd * dest_pitch),
                    (dest_height - hadd) * dest_pitch);
    visual_mem_copy(destbuf + ((dest_height - hadd) * dest_pitch), srcbuf2,
                    hadd * dest_pitch);

    break;

  case SLIDE_UP:
    visual_mem_copy(destbuf, srcbuf2 + (hadd * dest_pitch),
                    (dest_height - hadd) * dest_pitch);
    visual_mem_copy(destbuf + ((dest_height - hadd) * dest_pitch), srcbuf1,
                    hadd * dest_pitch);
  ...
  default:
    visual_log(VISUAL_LOG_WARNING,
               "Slide plugin is initialized with an impossible slide "
