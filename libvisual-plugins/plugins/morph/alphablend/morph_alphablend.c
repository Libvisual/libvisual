...
static void lv_morph_alpha_apply(VisPluginData *plugin, float progress,
                                 VisAudio *audio, VisVideo *dest,
                                 VisVideo *src1, VisVideo *src2);

static inline void alpha_blend_buffer(uint8_t *dest, uint8_t *src1,
                                      uint8_t *src2, int size, int depth,
                                      float alpha);
...
const VisPluginInfo *get_plugin_info(void) {
  ...
  static VisPluginInfo info = {
      ...
      .help = N_("This morph plugin morphs between two video sources using the "
                 "alphablend method"),
...
static void lv_morph_alpha_apply(VisPluginData *plugin, float progress,
  ...
  alpha_blend_buffer(visual_video_get_pixels(dest),
                     ...
                     visual_video_get_pixels(src2), visual_video_get_size(dest),
                     visual_video_get_depth(dest), progress);
}
