...
int main (int argc, char *argv[])
{
	...
	while (1) {
		...
		memset (sdlvid->pixels, 0, video->size * video->bpp);
		...
		if (dismode == 0) {
			...
			visual_rectangle_set (&srect, 10, 10, sxsize, sysize);
		} else {
			visual_rectangle_set (&drect, 256, 0, sxsize, sysize);
			visual_rectangle_set (&srect, xoff, yoff, sxsize, sysize);
		}
		
		visual_video_blit_overlay_rectangle (sdlvid, &drect, video, &srect, FALSE);
